...
/**
 * Craftable item searcher, responsible for scanning all server recipes and building the craftable panel.
 *
 * <p>Core responsibilities:
 * <ul>
 *   <li><b>Recipe scanning</b> ({@link #requestCraftables}) — Iterates over all server {@link net.minecraft.world.item.crafting.CraftingRecipe},
 *   filters to 3x3 workbench recipes, evaluates available materials, builds a list of craftable candidates</li>
 *   <li><b>Search filtering</b> — Supports item ID, display name, pinyin ({@code pinyinSearchEnabled}),
 *   mod namespace ({@code @modid}) and other search methods</li>
 *   <li><b>Group sorting</b> — Groups craft plans by output item, craftable first, label-based sorting</li>
 *   <li><b>Refresh</b> ({@link #refreshCraftables}) — Rescans reusing the search state from the session</li>
 * </ul>
 *
 * <p>Search state is stored in {@link com.rtsbuilding.rtsbuilding.server.storage.session.RtsBrowserState},
 * supporting pagination. Each recipe group ({@link CraftableGroupEntry}) contains multiple candidate plans
 * (e.g., different recipe IDs producing the same item), the client can switch between them.
 */
public final class RtsCraftingSearch {
    ...
    /**
     * Scans all server recipes, finds those craftable from linked storage, groups by result item.
     */
    ...
    /**
     * Refreshes the currently visible craftable panel (reusing search state from session).
     */
    ...
    /**
     * Checks whether the recipe is supported by the workbench craft panel (3x3 grid only).
     */
