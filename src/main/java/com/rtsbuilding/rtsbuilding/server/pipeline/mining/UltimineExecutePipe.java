 ...
 *
 * <p>此 Pipe 是 {@link RtsWorkflowType#ULTIMINE}、 {@link RtsWorkflowType#AREA_MINE} 和 {@link
 * RtsWorkflowType#AREA_DESTROY} 的"执行"阶段。 它从管道上下文（由上游 {@link ToolBorrowPipe} 和 {@link
 * WorkflowStartPipe} 设置） 读取工具租约和工作流条目，将它们存储在玩家会话中， 并将实际工作委托给 {@link RtsUltimineProcessor}。
 *
 * <p>预期的上下文参数（按操作类型变化）：
 *
 * <p><b>ULTIMINE:</b>
 *
 * <ul>
 *   <li>{@code "pos"} —— {@link BlockPos} 种子位置
 *   <li>{@code "face"} —— {@link Direction} 挖掘面（可选）
 *   <li>{@code "requestedLimit"} —— {@code int} 最大挖掘方块数
 *   <li>{@code "mode"} —— {@code byte} 连锁挖掘模式
 * </ul>
 *
 * <p><b>AREA_MINE:</b>
 *
 * <ul>
 *   <li>{@code "minX"}, {@code "maxX"}, {@code "minY"}, {@code "maxY"}, {@code "minZ"}, {@code
 *       "maxZ"} —— {@code int} 区域边界
 *   <li>{@code "shapeType"} —— {@code byte} 形状类型
 *   <li>{@code "fillType"} —— {@code byte} 填充类型
 * </ul>
 *
 * <p><b>AREA_DESTROY:</b>
 *
 * <ul>
 *   <li>{@code "positions"} —— {@code List<BlockPos>} 要破坏的显式位置列表
 * </ul>
 */
public record UltimineExecutePipe(RtsWorkflowType type) implements PipelinePipe<MiningContext> {

  public static final TypedKey<BlockPos> ARG_POS = new TypedKey<>("pos", BlockPos.class);
  public static final TypedKey<Direction> ARG_FACE = new TypedKey<>("face", Direction.class);
  public static final TypedKey<Integer> ARG_REQUESTED_LIMIT =
      new TypedKey<>("requestedLimit", Integer.class);
  public static final TypedKey<Byte> ARG_MODE = new TypedKey<>("mode", Byte.class);
  public static final TypedKey<Integer> ARG_MIN_X = new TypedKey<>("minX", Integer.class);
  public static final TypedKey<Integer> ARG_MAX_X = new TypedKey<>("maxX", Integer.class);
  public static final TypedKey<Integer> ARG_MIN_Y = new TypedKey<>("minY", Integer.class);
  public static final TypedKey<Integer> ARG_MAX_Y = new TypedKey<>("maxY", Integer.class);
  public static final TypedKey<Integer> ARG_MIN_Z = new TypedKey<>("minZ", Integer.class);
  public static final TypedKey<Integer> ARG_MAX_Z = new TypedKey<>("maxZ", Integer.class);
  public static final TypedKey<Byte> ARG_SHAPE_TYPE = new TypedKey<>("shapeType", Byte.class);
  public static final TypedKey<Byte> ARG_FILL_TYPE = new TypedKey<>("fillType", Byte.class);
  ...
  public static final TypedKey<Boolean> KEY_SELECTED_TOOL_REQUESTED =
      ToolBorrowPipe.KEY_SELECTED_TOOL_REQUESTED;
   ...
   * 紧凑构造函数，验证批处理挖掘类型。
   *
   * @param type 批处理挖掘类型（{@link RtsWorkflowType#ULTIMINE}、 {@link RtsWorkflowType#AREA_MINE} 或
  ...
  public UltimineExecutePipe {
    if (type != RtsWorkflowType.ULTIMINE
      ...
      throw new IllegalArgumentException(
          "UltimineExecutePipe only supports ULTIMINE, AREA_MINE, and AREA_DESTROY");
    }
  }
  ...
  public PipelineResult execute(MiningContext ctx) {
    ...
    if (!queueMode) {
      session.mining.miningToolLease =
          mctx.hasToolLease() ? mctx.getToolLease() : RtsToolLease.empty();
      session.mining.miningSelectedToolRequested = mctx.isSelectedToolRequested();
    }

    RtsbuildingMod.LOGGER.debug(
        "[UltimineExecutePipe] Executing {} for player={}, queueMode={}, toolSlot={}",
        type,
      ...
      case ULTIMINE:
        {
          ...
          int requestedLimit =
              mctx.hasArg(ARG_REQUESTED_LIMIT)
                  ? Objects.requireNonNull(
                      mctx.getArg(ARG_REQUESTED_LIMIT),
                      "ULTIMINE missing required arg: requestedLimit")
                  : Integer.MAX_VALUE;
          byte mode =
              mctx.hasArg(ARG_MODE)
                  ? Objects.requireNonNull(
                      mctx.getArg(ARG_MODE), "ULTIMINE missing required arg: mode")
                  : (byte) 0;

          if (queueMode) {
            int queuedCount =
                RtsUltimineProcessor.queueStartUltimine(
                    mctx.player(),
                    session,
                    pos,
                    face,
            ...
            RtsbuildingMod.LOGGER.debug(
                "[UltimineExecutePipe] ULTIMINE queued {} blocks for {}",
                queuedCount,
                mctx.player().getGameProfile().getName());
            if (queuedCount > 0 && mctx.hasWorkflowEntryId()) {
              RtsWorkflowEngine.getInstance()
                  .from(mctx.player(), mctx.getWorkflowEntryId())
          ...
          boolean started =
              RtsUltimineProcessor.startUltimine(
                  mctx.player(),
                  session,
                  pos,
                  face,
      ...
      case AREA_MINE:
        {
          int minX =
              Objects.requireNonNull(
                  mctx.getArg(ARG_MIN_X), "AREA_MINE missing required arg: minX");
          int maxX =
              Objects.requireNonNull(
                  mctx.getArg(ARG_MAX_X), "AREA_MINE missing required arg: maxX");
          int minY =
              Objects.requireNonNull(
                  mctx.getArg(ARG_MIN_Y), "AREA_MINE missing required arg: minY");
          int maxY =
              Objects.requireNonNull(
                  mctx.getArg(ARG_MAX_Y), "AREA_MINE missing required arg: maxY");
          int minZ =
              Objects.requireNonNull(
                  mctx.getArg(ARG_MIN_Z), "AREA_MINE missing required arg: minZ");
          int maxZ =
              Objects.requireNonNull(
                  mctx.getArg(ARG_MAX_Z), "AREA_MINE missing required arg: maxZ");
          byte shapeType =
              mctx.hasArg(ARG_SHAPE_TYPE)
                  ? Objects.requireNonNull(
                      mctx.getArg(ARG_SHAPE_TYPE), "AREA_MINE missing required arg: shapeType")
                  : (byte) 0;
          byte fillType =
              mctx.hasArg(ARG_FILL_TYPE)
                  ? Objects.requireNonNull(
                      mctx.getArg(ARG_FILL_TYPE), "AREA_MINE missing required arg: fillType")
                  : (byte) 0;

          if (queueMode) {
            int queuedCount =
                RtsUltimineProcessor.queueAreaMine(
                    mctx.player(),
                    session,
                    minX,
                    maxX,
                    ...
                    mctx.getWorkflowEntryId());
            RtsbuildingMod.LOGGER.debug(
                "[UltimineExecutePipe] AREA_MINE queued {} blocks for {}",
                queuedCount,
                mctx.player().getGameProfile().getName());
            if (queuedCount > 0 && mctx.hasWorkflowEntryId()) {
              RtsWorkflowEngine.getInstance()
      ...
      case AREA_DESTROY:
        {
          ...
          RtsbuildingMod.LOGGER.debug(
              "[UltimineExecutePipe] AREA_DESTROY enqueuing {} positions for {}",
              requestSize,
              mctx.player().getGameProfile().getName());

          boolean enqueued =
              RtsDestructionBatch.enqueueDestroyBatch(
                  mctx.player(),
                  session,
                  positions,
    ...
    if (mctx.hasWorkflowEntryId()) {
      int totalTargets =
          com.rtsbuilding.rtsbuilding.server.task.RtsTaskEngine.INSTANCE.workflowTaskTotalUnits(
              mctx.player(), mctx.getWorkflowEntryId());
      RtsWorkflowEngine.getInstance()
          .from(mctx.player(), mctx.getWorkflowEntryId())
          .ifPresent(token -> token.setTotalBlocks(totalTargets));
    }
  ...
  private static void completeWithoutTask(
      MiningContext ctx, RtsStorageSession session, boolean queueMode) {
