...
public final class RtsClientInputGate {
  ...
  public static JeiOverlayIngredient getJeiOverlayIngredientUnderMouse(
      double mouseX, double mouseY) {
    Minecraft minecraft = Minecraft.getInstance();
    VisibleOverlayLayout visible =
        resolveVisibleOverlayLayout(minecraft == null ? null : minecraft.screen);
    ...
    int index =
        resolveOverlaySlotIndex(
            overlayMouseX, overlayMouseY, layout.gridX(), layout.gridY(), layout.storageRows());
    if (index < 0) {
      return null;
    }
    ...
    return new JeiOverlayIngredient(
        stack.copy(), toGuiRect(slotX, slotY, SLOT_SIZE, SLOT_SIZE, scale));
  }
  ...
  public static void onScreenRenderPost(RtsScreenEvent.Render.Post event) {
    ...
    drawOverlayWindowFrame(
        g,
        minecraft.font,
        layout.storagePanelX(),
        layout.storagePanelY(),
        STORAGE_PANEL_W,
        layout.storagePanelH());
    drawMiniButton(
        g,
        minecraft.font,
        layout.dragX(),
    ...
    String display =
        trimToWidth(
            minecraft.font, searchText, Math.max(8, layout.searchW() - OVERLAY_SEARCH_CLEAR_W - 5));
    g.drawString(
        minecraft.font,
        display,
        layout.searchX() + 2,
    ...
    RtsClientUiUtil.drawCenteredStringNoShadow(
        ...
        minecraft.font,
        "x",
        layout.clearX() + OVERLAY_SEARCH_CLEAR_W / 2,
        layout.headerY() + 2,
        (searchText.isEmpty()
                ? ContainerOverlayStyle.SEARCH_CLEAR_EMPTY
                : ContainerOverlayStyle.BUTTON_TEXT)
            .toArgb());
    ...
    for (int i = 0; i < visibleStorageSlots; i++) {
      ...
      g.fill(cx, cy, cx + SLOT_SIZE, cy + SLOT_SIZE, ContainerOverlayStyle.STORAGE_SLOT.toArgb());
      if (i < maxSlots) {
      ...
      for (int i = 0; i < RETURN_SLOTS; i++) {
        ...
        if (!preview.isEmpty()) {
          ...
          g.drawString(
              ...
              cx + 6,
              cy + 5,
              ContainerOverlayStyle.RETURN_EMPTY_TEXT.toArgb(),
              false);
        }
      }
    }
    ...
    if (!OVERLAY_CRAFT_DIALOG.isOpen()) {
      int hoveredStorage =
          resolveOverlaySlotIndex(
              mouseX, mouseY, layout.gridX(), layout.gridY(), visibleStorageRows);
      ...
      if (hoveredCraft >= 0 && hoveredCraft < controller.getCraftableEntries().size()) {
        ...
        g.renderTooltip(minecraft.font, entry.stack(), (int) mouseX, (int) mouseY);
        String detail =
            entry.craftable() ? "Right click: choose recipe/count" : entry.missingSummary();
        if (detail != null && !detail.isBlank()) {
          g.drawString(
              ...
              (entry.craftable()
                      ...
                      : ContainerOverlayStyle.TOOLTIP_MISSING)
                  .toArgb(),
              false);
        }
      }
      ...
      int hoveredQuick =
          layout.overlayCollapsed()
              ? -1
              : resolveQuickbarSlotIndex(mouseX, mouseY, layout.quickbarX(), layout.quickbarY());
      if (hoveredQuick >= 0) {
      ...
      int hoveredReturn =
          resolveReturnSlotIndex(mouseX, mouseY, layout.returnX(), layout.returnY());
      if (hoveredReturn >= 0) {
    ...
    RtsCraftFeedbackPopup.render(
        g, minecraft.font, minecraft.getWindow().getGuiScaledWidth(), controller);
  }
