 ...
 *
 * <p>该实现类作为薄代理层，将所有传输操作委托给 {@link
 * com.rtsbuilding.rtsbuilding.server.service.transfer.RtsTransferPlayerIntegration}
 * 处理。负责协调会话获取和链接存储查询。
 */
public final class RtsTransferServiceImpl implements TransferService {
  ...
  public long countLinkedItemsMatching(ServerPlayer player, Predicate<ItemStack> predicate) {
    ...
    for (LinkedHandler linkedHandler : linked) {
      ...
      for (int slot = 0; slot < handler.getSlots(); slot++) {
        ...
        total =
            RtsCountUtil.saturatedAdd(
                total, RtsStoragePageBuilder.getHandlerReportedCount(handler, slot, stack));
      }
    }
    return total;
  }
  ...
  public void returnCarriedToLinked(ServerPlayer player, String itemId, int amount) {
    RtsTransferPlayerIntegration.returnCarriedToLinked(
        player, registry.session().getIfPresent(player), itemId, amount);
  }

  @Override
  public void quickDropLinkedItem(
      ServerPlayer player, String itemId, byte amount, double dropX, double dropY, double dropZ) {
    RtsTransferPlayerIntegration.quickDropLinkedItem(
        player, registry.session().getIfPresent(player), itemId, amount, dropX, dropY, dropZ);
  }

  @Override
  public void importMenuSlotToLinked(ServerPlayer player, int menuSlot) {
    RtsTransferPlayerIntegration.importMenuSlotToLinked(
  ...
  public void bulkStorageOperation(
      ServerPlayer player, byte action, ItemStack prototype, int amount) {
  ...
  public void pickupLinkedToCarried(ServerPlayer player, ItemStack prototype, int amount) {
    RtsTransferPlayerIntegration.pickupLinkedToCarried(
        player, registry.session().getIfPresent(player), prototype, amount);
  }

  @Override
  public void quickMoveLinkedItem(ServerPlayer player, ItemStack prototype) {
    RtsTransferPlayerIntegration.quickMoveLinkedItem(
        player, registry.session().getIfPresent(player), prototype);
  }

  @Override
  public void fillPlayerInventoryFromLinked(ServerPlayer player) {
    RtsTransferPlayerIntegration.fillPlayerInventoryFromLinked(
        player, registry.session().getIfPresent(player));
  }
}
