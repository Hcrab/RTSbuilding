 ...
 * 挖掘操作的强类型管道上下文。
 *
 * <p>提供对挖掘特定参数和共享数据的类型安全访问器， 消除了整个挖掘管道实现中的 {@code ctx.<BlockPos>getArg(ARG_POS)} 强制转换。
 *
 * <p>属于挖掘管道（MINE_SINGLE、ULTIMINE、 AREA_MINE、AREA_DESTROY）的 Pipe 应在 {@link
 * PipelinePipe#execute(PipelineContext)} 开头调用 {@link #require(PipelineContext)}:
 *
 * <pre>{@code
...
public class MiningContext extends PipelineContext {
  ...
  /**
   * 创建一个新的 {@link Builder}，用于构建 {@link MiningContext}， 提供类型安全的流式 setter，消除 {@code Map<String,
   * Object>} 样板代码。
   */
   ...
   *
   * <p>使用此方法代替原始的 {@code (MiningContext) ctx} 转换。 如果上下文不是 {@code MiningContext}，会抛出携带描述性消息的 {@link
   * IllegalArgumentException}，这比裸的 {@link ClassCastException} 更容易诊断配置错误的管道。
   *
   * @param ctx 要转换的管道上下文
   * @return 相同的上下文，类型化为 {@code MiningContext}
   * @throws IllegalArgumentException 如果 {@code ctx} 不是 {@code MiningContext} 实例
   */
  public static MiningContext require(PipelineContext ctx) {
    ...
    throw new IllegalArgumentException(
        "Expected MiningContext but got "
   ...
   * 返回挖掘面。
   *
   * @return 面方向，如果未提供则返回 {@code null} （默认为 {@link Direction#DOWN}）
   */
  @Nullable
  ...
  /** 返回是否启用了已放置方块恢复。 如果参数不存在则默认为 {@code false}。 */
  ...
  /** 返回是否启用了工具保护。 如果参数不存在则默认为 {@code true}。 */
  ...
  public static final class Builder {
    ...
    public Builder operationTrace(RtsOperationTraceContext trace) {
      args.put(
          PipelineContext.ARG_OPERATION_TRACE.name(),
  ...
  /** 返回来自 {@link SessionValidatePipe} 解析的存储会话， 如果尚未设置则返回 {@code null}。 */
  @Nullable
  public RtsStorageSession getResolvedSession() {
    return getData(SessionValidatePipe.KEY_SESSION);
  }

  /** 返回来自 {@link ToolBorrowPipe} 的借用工具租约， 如果未设置（创造模式快速路径）则返回 {@code null}。 */
  @Nullable
  ...
  /** 返回来自 {@link WorkflowStartPipe} 的工作流条目 ID， 如果未设置则返回 {@code -1}。 */
