...
/**
 * 按当前存档中的玩家与维度保存范围剔除区域。
 *
 * <p>剔除只影响客户端视觉，但坐标属于世界数据，不能放入全局客户端设置。玩家数据随存档
...
public final class RtsCullingPersistence {
    ...
    static State decode(CompoundTag root, String dimensionKey) {
        ...
        CompoundTag dimensions = root.getCompoundOrEmpty(NBT_DIMENSIONS);
        CompoundTag dimension = dimensions.getCompoundOrEmpty(dimensionKey);

        ListTag boxTags = dimension.getListOrEmpty(NBT_BOXES);
        ...
        for (int index = 0; index < boxTags.size() && boxes.size() < MAX_BOXES; index++) {
            CompoundTag tag = boxTags.getCompoundOrEmpty(index);
            if (!tag.contains(NBT_MIN) || !tag.contains(NBT_MAX)) {
                continue;
            }
            boxes.add(new RtsCullingBoxSnapshot(
                    BlockPos.of(tag.getLong(NBT_MIN).orElse(0L)),
                    BlockPos.of(tag.getLong(NBT_MAX).orElse(0L))));
        }

        long[] revealedValues = dimension.getLongArray(NBT_REVEALED).orElseGet(() -> new long[0]);
