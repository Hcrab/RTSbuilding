 ...
 * 1.21.1 连锁破坏会话适配器。
 *
 * <p>Core 只决定当前预览能否确认以及会话应进入哪个阶段；本类保留真实方块坐标、 工具槽、撤回记录和网络发包。它不绘制独立 Ultimine 窗口，因为当前产品入口已经是 Quick
 * Build → 范围破坏 → 连锁。
 */
public final class UltimineUiAdapter {
  ...
  public static UltimineUiState snapshot(BuilderScreen screen, int previewBlocks) {
    ...
    UltimineUiPhase phase =
        processed >= 0 && total > 0 && processed < total
            ? UltimineUiPhase.RUNNING
            : previewBlocks > 0 ? UltimineUiPhase.PREVIEW : UltimineUiPhase.IDLE;
    return new UltimineUiState(
        enabled,
        enabled ? "" : "chain_not_active",
        phase,
        previewBlocks > 0,
        previewBlocks,
        0,
  ...
  public static boolean confirmPreview(
      BuilderScreen screen, BlockHitResult hit, List<BlockPos> preview) {
    return confirmPreview(screen, hit, preview, RtsTraceInputKind.UNKNOWN);
  }
  ...
  public static boolean confirmPreview(
    ...
    UltimineUiTransition transition =
        UltimineUiReducer.apply(
    ...
    screen
        .getShapeController()
        .recordPendingBreakForUndo(preview, hit.getDirection(), screen.getSelectedToolSlot());
    screen
        .uiController()
        .startUltimine(
            hit.getBlockPos(),
