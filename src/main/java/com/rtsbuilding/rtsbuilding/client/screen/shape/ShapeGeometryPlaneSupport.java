...
// ======================== 实用方法 ========================
    ...
    public static Set<PlaneCell> buildCircleCells(int radius, boolean fill) {
        ...
        cells.addAll(emitted);
        return fill ? fillPlaneInteriorHoles(cells) : cells;
    }

    static void emitCircleCells(List<PlaneCell> cells, int radius, boolean fill) {
        ...
        for (long a = -radius; a <= radius; a++) {
            long outerRowSquared = outerSquared - a * a;
            if (outerRowSquared < 0L) continue;
            int outer = floorSqrt(outerRowSquared);
            if (fill) {
                for (int b = -outer; b <= outer; b++) cells.add(new PlaneCell(toInt(a), b));
                continue;
            }
            ...
            for (int b = -outer; b <= -inner; b++) cells.add(new PlaneCell(toInt(a), b));
            for (int b = inner; b <= outer; b++) cells.add(new PlaneCell(toInt(a), b));
        }
    }

    static void emitCircleCells(
            List<BlockPos> positions, BlockPos origin, Direction axisA, Direction axisB,
    ...
    static long circleCellCount(int radius, boolean fill, int limit) {
        ...
        if ((long) safeRadius * 2L + 1L > budget) return budget + 1L;
        ...
        for (long a = -safeRadius; a <= safeRadius; a++) {
            ...
            if (rowSquared < 0L) continue;
            int outer = floorSqrt(rowSquared);
            long rowCount;
            if (fill) {
                rowCount = outer * 2L + 1L;
            } else {
                long innerRowSquared = innerSquared - a * a;
                if (innerRowSquared <= 0L) rowCount = outer * 2L + 1L;
                else {
                    int inner = ceilSqrt(innerRowSquared);
            ...
            if (count > budget) return budget + 1L;
        }
        return count;
    }
    ...
    static int targetLimit(Set<BlockPos> targets) {
        return targets instanceof ShapeGeometryBudget.TargetSet bounded ? bounded.limit : MiningLimits.MAX_VOLUME;
    }
    ...
    static long roundedSpatialRadius(long dx, long dy, long dz) {
        return Math.round(Math.sqrt((double) dx * dx + (double) dy * dy + (double) dz * dz));
    }

    static int clampRadius(long radius, int maxRadius) {
        long safeMax = Math.max(0L, maxRadius);
        return toInt(Math.max(0L, Math.min((long) Integer.MAX_VALUE, Math.min(radius, safeMax))));
    }

    static int floorSqrt(long value) {
        if (value <= 0L) return 0;
        long root = (long) Math.sqrt((double) value);
        while (root < Integer.MAX_VALUE && (root + 1L) * (root + 1L) <= value) root++;
    ...
    static long saturatedProduct(long left, long middle, long right) {
        if (left <= 0L || middle <= 0L || right <= 0L) return 0L;
        if (left > Long.MAX_VALUE / middle) return Long.MAX_VALUE;
    ...
    static long boxTargetCount(long width, long height, long depth, ShapeFillMode fillMode) {
        ...
        if (fillMode == ShapeFillMode.FILL) return volume;
        if (fillMode == ShapeFillMode.HOLLOW) {
            if (volume == Long.MAX_VALUE) return Long.MAX_VALUE;
            long interior = saturatedProduct(Math.max(0L, width - 2L),
                    Math.max(0L, height - 2L), Math.max(0L, depth - 2L));
    ...
    static void addBoxHollowLayer(Set<BlockPos> targets, BlockPos start,
            int minX, int maxX, int minZ, int maxZ, long iy, boolean yBoundary) {
        if (yBoundary) {
            for (long x = minX; x <= maxX; x++) for (long z = minZ; z <= maxZ; z++) addOffsetTarget(targets, start, x, iy, z);
            return;
        }
        for (long z = minZ; z <= maxZ; z++) {
            addOffsetTarget(targets, start, minX, iy, z);
            if (maxX != minX) addOffsetTarget(targets, start, maxX, iy, z);
        }
        for (long x = (long) minX + 1L; x < maxX; x++) {
            addOffsetTarget(targets, start, x, iy, minZ);
            if (maxZ != minZ) addOffsetTarget(targets, start, x, iy, maxZ);
        }
    }

    static void addBoxSkeletonLayer(Set<BlockPos> targets, BlockPos start,
            int minX, int maxX, int minZ, int maxZ, long iy, boolean yBoundary) {
        if (yBoundary) {
            for (long z = minZ; z <= maxZ; z++) {
                addOffsetTarget(targets, start, minX, iy, z);
    ...
    static void addOffsetTarget(Set<BlockPos> targets, BlockPos start, long x, long y, long z) {
        targets.add(new BlockPos(toInt((long) start.getX() + x),
                toInt((long) start.getY() + y), toInt((long) start.getZ() + z)));
    }
