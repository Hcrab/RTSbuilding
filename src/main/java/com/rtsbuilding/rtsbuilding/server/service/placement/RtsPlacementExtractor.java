...
public final class RtsPlacementExtractor {
     ...
     * 如果提供了原型则优先匹配。
     */
    public static ItemStack extractSelectedFromNetwork(List<RtsItemHandler> handlers, ServerPlayer player, Item item,
     ...
     */
    public static ItemStack extractSelectedFromNetworkCached(ServerPlayer player, List<RtsItemHandler> handlers, Item item,
                                                              ItemStack preferredStack) {
     ...
     * 如果提供了原型则优先匹配。
     */
    public static ItemStack extractSelectedFromLinked(List<RtsItemHandler> handlers, Item item, ItemStack preferredStack) {
        if (preferredStack != null && !preferredStack.isEmpty()) {
     ...
     */
    public static ItemStack extractSelectedFromLinkedCached(ServerPlayer player, List<RtsItemHandler> handlers, Item item, ItemStack preferredStack) {
        RtsAggregateStorage aggregate = RtsStorageTickService.INSTANCE.getStorage(player);
        if (aggregate != null && !aggregate.isEmpty()) {
            ItemStack extracted;
            if (preferredStack != null && !preferredStack.isEmpty()) {
