...
public record S2CRtsServerConfigResultPayload(byte[] data) implements CustomPacketPayload {
    public static final Type<S2CRtsServerConfigResultPayload> TYPE = new Type<>(
            new ResourceLocation(RtsbuildingMod.MODID, "s2c_rts_server_config_result"), S2CRtsServerConfigResultPayload.class);
    public static final StreamCodec<RegistryFriendlyByteBuf, S2CRtsServerConfigResultPayload> STREAM_CODEC = StreamCodec.of(
    ...
    public S2CRtsServerConfigResultPayload { RtsServerConfigWire.decodeResponse(data); data = data.clone(); }
