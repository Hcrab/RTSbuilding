...
/**
 * Compact status window for range culling.
 *
 * <p>The window shows only the current step, selected box size, and deletion entry. Main editing stays on
 * world-space axis arrows, keeping the panel small and avoiding the old oversized empty-box feel.
 */
public final class RtsCullingPanel extends RtsWindowPanel {
    ...
    private boolean isDeleteButtonHovered(int mouseX, int mouseY) {
        if (!isInsideWindow(mouseX, mouseY) || manager.selectedId() < 0) {
            return false;
        }
