...
public final class PlacedBlockTrackerData extends SavedData {
    ...
    public static PlacedBlockTrackerData get(ServerLevel level) {
        return level.getDataStorage().computeIfAbsent(
                PlacedBlockTrackerData::load, PlacedBlockTrackerData::new, DATA_NAME);
    }

    /** 在实际方块已经落地后写入一条新的 V2 凭据。 */
    public @Nullable CredentialSnapshot markPlaced(
            BlockPos pos, UUID owner, BlockState actualState) {
        if (actualState == null || actualState.isAir()) {
            clear(pos);
            return null;
        }
        return markPlaced(pos, owner, BuiltInRegistries.BLOCK.getKey(actualState.getBlock()));
    }
    ...
    public void clear(@Nullable BlockPos pos) {
        if (pos != null && credentials.remove(pos.asLong()) != null) setDirty();
    }
    ...
    /** 恢复历史事务携带的凭据快照；不会把当前操作者 UUID 写入快照。 */
    public void restoreSnapshot(
            @Nullable BlockPos pos, @Nullable CredentialSnapshot snapshot) {
        if (pos == null) return;
        long packed = pos.asLong();
        if (snapshot == null) {
            clear(pos);
    ...
    public boolean isPlaced(@Nullable BlockPos pos) {
        return pos != null && credentials.containsKey(pos.asLong());
    }

    /** 按当前已加载世界状态判定一次回收资格，并惰性迁移 legacy。 */
    public RecoveryCheck checkRecovery(
            ServerLevel level, BlockPos pos, @Nullable UUID actor) {
    ...
    public CompoundTag save(CompoundTag tag) {
        ...
        for (String id : blockPalette) blocks.add(StringTag.valueOf(id));
    ...
    static PlacedBlockTrackerData load(CompoundTag tag) {
    ...
    private void loadV2(CompoundTag tag) {
        ...
        for (int i = 0; i < positions.length; i++) {
            ...
            ResourceLocation blockId = blockIndex < 0 ? null
                    : ResourceLocation.tryParse(blockPalette.get(blockIndex));
            if (blockIndex >= 0 && blockId == null) {
                throw new IllegalArgumentException("placed tracker block ID 无效");
            }
        ...
        nextGeneration = persistedNext > maxGeneration
                ? persistedNext : incrementGeneration(maxGeneration);
    }

    private static List<String> readBlockPalette(ListTag encoded) {
        if (encoded.size() > MAX_PALETTE_ENTRIES
                || (!encoded.isEmpty() && encoded.getElementType() != Tag.TAG_STRING)) {
