...
final class ClientRtsCommandOwner {
  ...
  void rotateBlockStep(BlockPos pos, Direction axisDirection, int quarterTurns) {
    controller.buildPlacementService.rotateBlockStep(pos, axisDirection, quarterTurns);
  }
  ...
  void applyStoragePage(S2CRtsStoragePagePayload payload) {
    controller.storageStateManager.applyStoragePage(
        payload, controller::refreshSelectedItemPreviewFromStorage);
  }
  ...
  void refreshSelectedItemPreviewFromStorage() {
    controller.buildPlacementService.syncSelectedPreviewFromStorage(
        ...
        controller.storageStateManager.getStorageTotalCount(
  ...
  void applyRemoteMenuHint(S2CRtsRemoteMenuHintPayload payload) {
    RtsRemoteMenuClientDiagnostics.receiveServerHint(
        payload == null ? null : payload.pos(), ClientRtsController.REMOTE_MENU_OPEN_GRACE_TICKS);
    controller.beginRemoteMenuOpenGrace();
  }
  ...
  void applyProgressionState(S2CRtsProgressionStatePayload payload) {
    controller.progressionStateManager.applyProgressionState(
        payload, () -> controller.homeSelectionMode = false);
  }
  ...
  void setSurvivalProgressionEnabled(boolean enabled) {
    controller.progressionStateManager.setSurvivalProgressionEnabled(
        enabled, () -> controller.homeSelectionMode = false);
  }
