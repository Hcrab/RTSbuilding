...
/**
 * 将网络 trace、Pipeline op、Workflow 与 durable Task 串成一条有界诊断链。 该表不拥有业务对象，不参与包去重、任务准入或取消判断；容量淘汰不会改变游戏行为。
 */
public final class RtsServerTraceRegistry {
  ...
  public static synchronized RtsOperationTraceContext acceptNetwork(
    ...
    RtsOperationTraceContext trace =
        new RtsOperationTraceContext(
            traceId,
            sequence,
            clientTick,
            heldMs,
            RtsTraceInputKind.fromWire(inputKind),
            RtsMiningStopOrigin.fromWire(stopOrigin),
            packet,
            operationId,
            receivedNanos,
            receiveTick,
  ...
  public static synchronized TraceState bindWorkflow(
    ...
    state.lastServerTick =
        player == null ? state.lastServerTick : player.serverLevel().getGameTime();
  ...
  public static synchronized void bindTask(TaskSnapshot snapshot) {
    ...
    WorkflowKey workflowKey = new WorkflowKey(snapshot.ownerId(), snapshot.workflowEntryId());
    TraceState state = BY_WORKFLOW.get(workflowKey);
    if (state == null) {
      state = recoveredState(snapshot);
      BY_WORKFLOW.put(workflowKey, state);
      trim(BY_WORKFLOW, MAX_WORKFLOWS);
    }
  ...
  public static synchronized void onTaskSlice(
    ...
    if (!state.everExecuted) {
      ...
      infoDiag(
          ...
          "first_slice_tick",
          tick,
          "submit_to_first_slice_ticks",
          waitTicks(state.createdTick, tick),
    ...
    if (!after.state().terminal()) return;
    String outcome =
        switch (after.state()) {
    ...
    String reason =
        after.state() == TaskLifecycleState.CANCELLED
  ...
  public static synchronized void markCancelOrigin(
    ...
    infoDiag(
        ...
        state.everExecuted,
        "first_slice_tick",
        state.firstSliceTick,
        "age_ticks",
        waitTicks(state.createdTick, player.serverLevel().getGameTime()));
  }
  ...
  public static synchronized void externalTaskTerminal(
    ...
    terminal(
        ...
        snapshot.updatedGameTime());
  }

  public static synchronized void workflowTerminal(
      UUID playerId, int workflowId, String outcome, String reason, int completed, int failed) {
    ...
    String terminalReason =
        "CANCELLED".equalsIgnoreCase(outcome) && !"NONE".equals(state.cancelOrigin)
            ? state.cancelOrigin
            : reason;
    terminal(
        state,
  ...
  public static synchronized void terminalWithoutWorkflow(
    ...
    TraceState state = stateFor(player, trace);
    if (state == null) return;
    if (type == RtsWorkflowType.STOP_MINING && state.workflowId >= 0 && !state.terminalSent) {
      infoDiag(
          "STOP_ACCEPTED",
          state,
          "stop_origin",
    ...
    state.type = type;
    terminal(
        state, outcome, reason, 0, 0, player == null ? -1L : player.serverLevel().getGameTime());
  }
  ...
  private static void maybeLogWait(TraceState state, TaskSnapshot before, TaskSnapshot after) {
    String wait =
        after.waitKey() == null
            ? "NONE"
            : safe(after.waitKey().kind() + ':' + after.waitKey().value(), "UNKNOWN");
    ...
    if (!"NONE".equals(wait))
      ...
      infoDiag(
          ...
          before.waitKey().kind() + ":" + before.waitKey().value(),
          "cursor",
          after.cursorUnits());
  }
  ...
  private static void maybeLogProgress(
    ...
    boolean verboseSample =
        level() == RtsDiagnosticLevel.VERBOSE && now - state.lastProgressLogNanos >= 1_000_000_000L;
  ...
  private static boolean crossedThreshold(int previous, int current) {
    for (int threshold : new int[] {10, 25, 50, 75, 100})
      if (previous < threshold && current >= threshold) return true;
    return false;
  }

  private static void terminal(
      TraceState state, String outcome, String reason, int completed, int failed, long serverTick) {
    ...
    infoDiag(
        ...
        "submit_to_terminal_ticks",
        waitTicks(state.createdTick, serverTick),
        "server_stall_overlap",
        recentGap > 0L,
        "recent_tick_gap_ms",
        recentGap);
    ...
    if (state.traceId != RtsTraceIds.NONE && player != null) {
      int ackSequence =
          state.lastSequence == Integer.MAX_VALUE ? Integer.MAX_VALUE : state.lastSequence + 1;
      RtsClientboundPackets.sendToPlayer(
          player,
          new S2CRtsOperationTerminalPayload(
              state.traceId,
              ackSequence,
              safe(outcome, "UNKNOWN"),
              safe(reason, "UNKNOWN"),
  ...
  private static TraceState recoveredState(TaskSnapshot snapshot) {
    TraceState state =
        new TraceState(
            snapshot.ownerId(), null, RtsOperationTraceContext.legacy("PERSIST_RESTORE"));
    ...
    infoDiag(
        ...
        snapshot.type(),
        "restore_decision",
        snapshot.state().terminal() ? "KEEP_VISIBLE_ONLY" : "RESUME");
    return state;
  }
  ...
  private static void infoTrace(String event, RtsOperationTraceContext trace, Object... fields) {
    ...
    RtsbuildingMod.LOGGER.info(
        ...
        trace.sequence(),
        value(trace.operationId()),
        suffix(fields));
    appendStructured(
        "TRACE_" + event, trace.traceId(), trace.sequence(), trace.operationId(), fields);
  }
  ...
  private static void infoDiag(String event, TraceState state, Object... fields) {
    ...
    RtsbuildingMod.LOGGER.info(
        ...
        RtsTraceIds.runId(),
        event,
        RtsTraceIds.format(state.traceId),
        state.lastSequence,
        value(state.lastOperationId),
        value(state.workflowId),
        state.taskId,
        state.type == null ? "-" : state.type,
        value(state.lastServerTick),
        suffix(fields));
    appendStructured(
        event,
        state.traceId,
