...
public final class BlueprintPanel {
    /*
     * 文件与异步 capture 仍由本类的 owner 链统一编排：TinyFileDialogs.tinyfd_openFileDialog()
     * / TinyFileDialogs.tinyfd_saveFileDialog() / TinyFileDialogs.tinyfd_messageBox()、
     * BlueprintWriters.writeVanillaStructure()、BlueprintReaders.parse()、Files.list(folder)、
     * CAPTURE.startSave() 与 CAPTURE.pollSaveResult() 均通过文件/任务适配器进入这里。
     */
    ...
    public static void render(GuiGraphicsExtractor graphics, Font font, ClientRtsController controller,
    ...
    /**
     * 兼容 26.1 的独立命名窗口键盘入口；状态仍唯一存放在 {@link BlueprintDialogState}。
     */
    static boolean handleNameWindowKey(int keyCode) {
        if (!isNameDialogOpen()) return false;
        if (keyCode == org.lwjgl.glfw.GLFW.GLFW_KEY_ESCAPE) {
            cancelNameDialog();
            return true;
        }
        ...
        if (keyCode == org.lwjgl.glfw.GLFW.GLFW_KEY_BACKSPACE) {
            String value = DIALOGS.replaceOnFirstInput()
                    ? ""
                    : DIALOGS.nameValue().isEmpty()
                            ? ""
                            : DIALOGS.nameValue().substring(0, DIALOGS.nameValue().length() - 1);
            DIALOGS.setNameValue(value);
        }
        return true;
    }
    /** 兼容独立命名窗口的字符输入，同时保留首次输入替换旧名称的语义。 */
    static boolean appendNameWindowCharacter(char codePoint) {
        if (!isNameDialogOpen()) return false;
        if (!Character.isISOControl(codePoint)) {
            String current = DIALOGS.replaceOnFirstInput() ? "" : DIALOGS.nameValue();
            if (current.length() < BlueprintDialogState.MAX_NAME_LENGTH) {
                DIALOGS.setNameValue(current + codePoint);
            }
        }
        return true;
    }
