 ...
 *
 * <p>共享底部栏仍可使用 MOD 排序；合成终端打开时会把这个不受支持的字段 规范化为数量排序。转换集中在这里，避免屏幕渲染、tooltip 和点击各自维护 一套不一致的 switch。
 */
public final class CraftTerminalSortAdapter {
  ...
  public static RtsStorageSort toStorage(CraftTerminalSortField field) {
    ...
    return field == CraftTerminalSortField.NAME ? RtsStorageSort.NAME : RtsStorageSort.QUANTITY;
  }
}
