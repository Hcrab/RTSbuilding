 ...
 * RTS 主屏幕的顶层编排壳。
 *
 * <p>这里只保留 Screen 边界、构造装配、缩放入口和稳定公共门面；具体输入、绘制、窗口、 世界动作与查询分别由独立 owner 承担。
 */
public final class BuilderScreen extends BuilderScreenComponentState {
  ...
  private final BuilderScreenPointerActionOwner pointerActionOwner =
      new BuilderScreenPointerActionOwner(this);
  private final BuilderScreenPointerGestureOwner pointerGestureOwner =
      new BuilderScreenPointerGestureOwner(this);
  private final BuilderScreenKeyboardActionOwner keyboardActionOwner =
      new BuilderScreenKeyboardActionOwner(this);
  private final BuilderScreenKeyboardSessionOwner keyboardSessionOwner =
      new BuilderScreenKeyboardSessionOwner(this);
  private final BuilderScreenRenderOwner renderOwner = new BuilderScreenRenderOwner(this);
  private final BuilderScreenWindowActionOwner windowActionOwner =
      new BuilderScreenWindowActionOwner(this);
  private final BuilderScreenModeSessionOwner modeSessionOwner =
      new BuilderScreenModeSessionOwner(this);
  private final BuilderScreenWorldQueryOwner worldQueryOwner =
      new BuilderScreenWorldQueryOwner(this);
  private final BuilderScreenPreviewQueryOwner previewQueryOwner =
      new BuilderScreenPreviewQueryOwner(this);

  public BuilderScreen(ClientRtsController controller) {
    super(Component.literal("RTS Builder"));
    this.controller = controller;
    BuilderScreenInputHost inputHost = new BuilderScreenInputHost(this);
    this.pointerClickRouter =
        new BuilderScreenPointerClickRouter(inputHost, this.placementStateWheel, this.modeWheel);
    this.primaryActionRouter =
        new BuilderScreenPrimaryActionRouter(
            ...
            this.cursorPicker);
    this.keyPressRouter =
        new BuilderScreenKeyPressRouter(inputHost, this.placementStateWheel, this.modeWheel);
    this.leftDockedTooltipRenderer = new LeftDockedTooltipRenderer(this, this.bottomPanel);
    this.uiStateManager =
        new RtsScreenUiStateManager(this.controller, this.shapeController, this.quickBuildPanel);
    this.guiScaleCoordinator =
        new RtsGuiScaleCoordinator(
            () -> this.minecraft,
            () -> this.width,
            () -> this.height,
    ...
    this.overlayRenderer =
        new RtsScreenOverlayRenderer(this, this.controller, this.cursorPicker, this.bottomPanel);
    this.playerStatusRenderer = new PlayerStatusRenderer(this);
    this.storageLinkDetailHandler =
        new StorageLinkDetailHandler(
            this, this.controller, this.topBarPanel, this.linkedStoragePanel);
    this.floatingWindowLayer =
        new RtsFloatingWindowLayer(
            this.storageLinkDetailHandler,
    ...
    this.uiStateManager.registerWindowPanel(
        "blueprint_materials", this.blueprintMaterialWindowPanel);
  ...
  public void toggleContainerOverlayEnabled() {
    this.uiStateManager.toggleContainerOverlayEnabled();
  }

  public void toggleOverlayShiftImportEnabled() {
    this.uiStateManager.toggleOverlayShiftImportEnabled();
  }
  ...
  public void toggleShowWorkflowPanelEnabled() {
    this.uiStateManager.toggleShowWorkflowPanelEnabled();
  }

  public void toggleJadePanelTrackMouse() {
    this.uiStateManager.toggleJadePanelTrackMouse();
  }

  public void toggleJadePanelHidden() {
    this.uiStateManager.toggleJadePanelHidden();
  }

  public void toggleStorageRefreshQuietEnabled() {
    this.uiStateManager.toggleStorageRefreshQuietEnabled();
  }

  public void toggleStorageAutoRefreshEnabled() {
    this.uiStateManager.toggleStorageAutoRefreshEnabled();
  }
  ...
  public boolean canUseQuickBuild() {
    return !this.controller.isProgressionEnabled()
        || this.controller.hasInstalledPlugin(
  ...
  public void syncQuickBuildActiveState() {
    this.lifecycleOwner.syncQuickBuildActiveState();
  }
  ...
  */
  public boolean mouseClicked(double mouseX, double mouseY, int button) {
    try (RtsGuiScaleCoordinator.InputFrame frame = this.guiScaleCoordinator.beginInput()) {
  ...
  void selectPlacementStateFromWheel(PlacementStateWheel.PlacementChoice choice, int button) {
    this.pointerActionOwner.selectPlacementStateFromWheel(choice, button);
  }

  void closePlacementStateWheelFromPointer(int button) {
    this.pointerActionOwner.closePlacementStateWheelFromPointer(button);
  }

  void selectModeFromWheelPointer(BuilderMode selectedMode, int button) {
    this.pointerActionOwner.selectModeFromWheelPointer(selectedMode, button);
  }

  void closeModeWheelFromPointer(int button) {
    this.pointerActionOwner.closeModeWheelFromPointer(button);
  }
  ...
  boolean handleBlueprintCaptureClicks(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleBlueprintCaptureClicks(mouseX, mouseY, button);
  }

  boolean handleHomeSelectionClicks(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleHomeSelectionClicks(mouseX, mouseY, button);
  }

  boolean handleOverlayClicks(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleOverlayClicks(mouseX, mouseY, button);
  }

  boolean handleAreaMineClickBlock(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleAreaMineClickBlock(mouseX, mouseY, button);
  }

  boolean handleLeftClickInteractions(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleLeftClickInteractions(mouseX, mouseY, button);
  }

  boolean handleWorldClickActions(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleWorldClickActions(mouseX, mouseY, button);
  }

  boolean handleAdvancedShapeHandleClick(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleAdvancedShapeHandleClick(mouseX, mouseY, button);
  }

  boolean handleBatchConfirmMouse(double mouseX, double mouseY, int button) {
    return this.pointerActionOwner.handleBatchConfirmMouse(mouseX, mouseY, button);
  }

  boolean handleStorageBatchWorldClick(double mouseX, double mouseY) {
    return this.pointerActionOwner.handleStorageBatchWorldClick(mouseX, mouseY);
  }

  @Override
  public boolean mouseReleased(double mouseX, double mouseY, int button) {
    return this.pointerGestureOwner.mouseReleased(mouseX, mouseY, button);
  }

  @Override
  public boolean mouseDragged(
      double mouseX, double mouseY, int button, double dragX, double dragY) {
    return this.pointerGestureOwner.mouseDragged(mouseX, mouseY, button, dragX, dragY);
  }

  @Override
  public void mouseMoved(double mouseX, double mouseY) {
    this.pointerGestureOwner.mouseMoved(mouseX, mouseY);
  }

  public boolean isCameraUpActionHeld() {
    return this.pointerGestureOwner.isCameraUpActionHeld();
  }

  public boolean isCameraDownActionHeld() {
    return this.pointerGestureOwner.isCameraDownActionHeld();
  }

  boolean runPrimaryActionAt(double mouseX, double mouseY) {
    return this.pointerGestureOwner.runPrimaryActionAt(mouseX, mouseY);
  }

  boolean runPrimaryActionAt(double mouseX, double mouseY, int mouseButton) {
    return this.pointerGestureOwner.runPrimaryActionAt(mouseX, mouseY, mouseButton);
  }

  boolean openPlacementStateWheel(double mouseX, double mouseY) {
    return this.pointerGestureOwner.openPlacementStateWheel(mouseX, mouseY);
  }

  void closePlacementStateWheel() {
    this.pointerGestureOwner.closePlacementStateWheel();
  }

  void closePlacementStateWheelImmediately() {
    this.pointerGestureOwner.closePlacementStateWheelImmediately();
  }

  void releasePlacementWheelPointer() {
    this.pointerGestureOwner.releasePlacementWheelPointer();
  }

  boolean tryUseMainHandItemInAir(boolean shiftDown) {
    return this.worldQueryOwner.tryUseMainHandItemInAir(shiftDown);
  }

  boolean canUseMainHandItemInAir() {
    return this.pointerGestureOwner.canUseMainHandItemInAir();
  }

  @Override
  /**
   * Handles mouse scroll with RTS GUI scale remapping. Routes scroll to open dialogs, gear menu,
   * wheel panels, guide panel, bottom panel, shape height previews, rotation mode, and item slot
   * scrolling.
   */
  public boolean mouseScrolled(double mouseX, double mouseY, double scrollX, double scrollY) {
    ...
    return this.scrollRouter.mouseScrolled(mouseX, mouseY, scrollX, scrollY);
  }

  @Override
  /**
   * Handles key press events. Dispatches to dialogs, blueprint, overlay, world interaction, search
   * box, tool slot, and sensitivity handlers in priority order.
   */
  ...
  void closePlacementStateWheelFromKey() {
    this.keyboardActionOwner.closePlacementStateWheelFromKey();
  }
  ...
  boolean handleBlueprintKeys(int keyCode, int scanCode, int modifiers) {
    return this.keyboardActionOwner.handleBlueprintKeys(keyCode, scanCode, modifiers);
  }

  boolean handleHomeSelectionKey(int keyCode) {
    return this.keyboardActionOwner.handleHomeSelectionKey(keyCode);
  }

  boolean handleOverlayKeys(int keyCode, int scanCode, int modifiers) {
    return this.keyboardActionOwner.handleOverlayKeys(keyCode, scanCode, modifiers);
  }

  boolean handleWorldInteractionKeys(int keyCode, int scanCode, int modifiers) {
    return this.keyboardActionOwner.handleWorldInteractionKeys(keyCode, scanCode, modifiers);
  }

  boolean handleStorageBatchSelectionKey(int keyCode) {
    return this.keyboardActionOwner.handleStorageBatchSelectionKey(keyCode);
  }

  boolean handlePlacedBlockRotationKey(int keyCode) {
    return this.keyboardActionOwner.handlePlacedBlockRotationKey(keyCode);
  }

  boolean handleSelectionBoxKeys(int keyCode, int scanCode, int modifiers) {
    return this.keyboardActionOwner.handleSelectionBoxKeys(keyCode, scanCode, modifiers);
  }

  boolean handleBatchConfirmKey(int keyCode, int scanCode) {
    return this.keyboardActionOwner.handleBatchConfirmKey(keyCode, scanCode);
  }

  boolean handleSearchFocusKeys(int keyCode, int scanCode, int modifiers) {
    return this.keyboardActionOwner.handleSearchFocusKeys(keyCode, scanCode, modifiers);
  }

  boolean handleToolSlotKeys(int keyCode, int scanCode, int modifiers) {
    return this.keyboardActionOwner.handleToolSlotKeys(keyCode, scanCode, modifiers);
  }

  boolean handleSensitivityKeys(int keyCode, int scanCode) {
    return this.keyboardActionOwner.handleSensitivityKeys(keyCode, scanCode);
  }

  @Override
  public boolean keyReleased(int keyCode, int scanCode, int modifiers) {
    return this.keyboardSessionOwner.keyReleased(keyCode, scanCode, modifiers);
  }

  void handleRtsFlightToggle() {
    this.keyboardSessionOwner.handleRtsFlightToggle();
  }

  boolean handleModeKeyPressed(int keyCode, int scanCode) {
    return this.keyboardSessionOwner.handleModeKeyPressed(keyCode, scanCode);
  }

  boolean switchToModeFromKey(BuilderMode mode, boolean funnelEnabled) {
    return this.keyboardSessionOwner.switchToModeFromKey(mode, funnelEnabled);
  }
  ...
  public boolean charTyped(char codePoint, int modifiers) {
    return this.keyboardSessionOwner.charTyped(codePoint, modifiers);
  }

  @Override
  public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
    this.renderOwner.render(guiGraphics, mouseX, mouseY, partialTick);
  }

  void resetHoverStates() {
    this.renderOwner.resetHoverStates();
  }

  void renderHoveredItemTooltips(GuiGraphics g, int mouseX, int mouseY) {
    this.renderOwner.renderHoveredItemTooltips(g, mouseX, mouseY);
  }

  public void renderTopGuideHint(GuiGraphics g, List<TopBarTypes.TopBarButtonLayout> topButtons) {
    this.renderOwner.renderTopGuideHint(g, topButtons);
  }

  void drawGuiBindCursor(GuiGraphics g, int mouseX, int mouseY) {
    this.renderOwner.drawGuiBindCursor(g, mouseX, mouseY);
  }
  ...
  public void persistUiState() {
    this.windowActionOwner.persistUiState();
  }

  public void adjustRtsGuiScale(double delta) {
    this.windowActionOwner.adjustRtsGuiScale(delta);
  }

  public double getRtsGuiScale() {
    return this.windowActionOwner.getRtsGuiScale();
  }

  public String rtsGuiScaleLabel() {
    return this.windowActionOwner.rtsGuiScaleLabel();
  }

  public RtsFloatingWindowLayer getFloatingWindowLayer() {
    return this.windowActionOwner.getFloatingWindowLayer();
  }

  public ClientRtsController uiController() {
    return this.windowActionOwner.uiController();
  }

  public boolean isQuickBuildRangeDestroyMode() {
    return this.windowActionOwner.isQuickBuildRangeDestroyMode();
  }

  public boolean isQuickBuildSmartFillMode() {
    return this.windowActionOwner.isQuickBuildSmartFillMode();
  }

  public ShapeDataRecords.GhostPreview getSmartFillGhostPreview() {
    return this.windowActionOwner.getSmartFillGhostPreview();
  }

  boolean handleQuickBuildSmartFillClick() {
    return this.windowActionOwner.handleQuickBuildSmartFillClick();
  }

  boolean cancelQuickBuildSmartFillAnchor() {
    return this.windowActionOwner.cancelQuickBuildSmartFillAnchor();
  }

  public boolean isQuickBuildRangeDestroyChainMode() {
    return this.windowActionOwner.isQuickBuildRangeDestroyChainMode();
  }

  public boolean isQuickBuildConvenienceDestroyMode() {
    return this.windowActionOwner.isQuickBuildConvenienceDestroyMode();
  }
  ...
  public boolean isQuickBuildCreativeOverwriteEnabled() {
    return this.windowActionOwner.isQuickBuildCreativeOverwriteEnabled();
  }

  public boolean isAdvancedRangeDestroyBoxMode() {
    return this.windowActionOwner.isAdvancedRangeDestroyBoxMode();
  }

  public boolean isAdvancedRangeDestroyShapeMode() {
    return this.windowActionOwner.isAdvancedRangeDestroyShapeMode();
  }

  public boolean isAdvancedShapeMode() {
    return this.windowActionOwner.isAdvancedShapeMode();
  }

  public boolean isRoundShapeVertical(BuildShape shape) {
    return this.windowActionOwner.isRoundShapeVertical(shape);
  }

  public String activeQuickBuildShapeLabel() {
    return this.windowActionOwner.activeQuickBuildShapeLabel();
  }
  ...
  public void setQuickBuildMode(QuickBuildMode mode) {
    this.windowActionOwner.setQuickBuildMode(mode);
  }

  public int getUltimineLimit() {
    return this.windowActionOwner.getUltimineLimit();
  }

  public boolean isAreaMineHeightPreview() {
    return this.windowActionOwner.isAreaMineHeightPreview();
  }

  public int getShapeUndoSize() {
    return this.windowActionOwner.getShapeUndoSize();
  }

  public int getPendingGuiBindSlot() {
    return this.windowActionOwner.getPendingGuiBindSlot();
  }

  public void setPendingGuiBindSlot(int slot) {
    this.windowActionOwner.setPendingGuiBindSlot(slot);
  }

  public void clearPendingGuiBind() {
    this.windowActionOwner.clearPendingGuiBind();
  }

  public void toggleQuickBuild() {
    this.windowActionOwner.toggleQuickBuild();
  }

  public void openCraftQuantityWindow(CraftableEntry entry) {
    this.windowActionOwner.openCraftQuantityWindow(entry);
  }

  public void submitCraftQuantityWindowIfReady() {
    this.windowActionOwner.submitCraftQuantityWindowIfReady();
  }

  boolean handleFloatingWindowClick(double mouseX, double mouseY, int button) {
    return this.windowActionOwner.handleFloatingWindowClick(mouseX, mouseY, button);
  }

  boolean handleFloatingWindowDrag(
      double mouseX, double mouseY, int button, double dragX, double dragY) {
    return this.windowActionOwner.handleFloatingWindowDrag(mouseX, mouseY, button, dragX, dragY);
  }

  boolean handleFloatingWindowRelease(double mouseX, double mouseY, int button) {
    return this.windowActionOwner.handleFloatingWindowRelease(mouseX, mouseY, button);
  }

  public boolean isMouseOverFloatingWindow(double mouseX, double mouseY) {
    return this.windowActionOwner.isMouseOverFloatingWindow(mouseX, mouseY);
  }

  public void closeGearMenu() {
    this.windowActionOwner.closeGearMenu();
  }

  public void toggleGearMenu() {
    this.windowActionOwner.toggleGearMenu();
  }

  public void toggleTopGuide(int x, int y) {
    this.windowActionOwner.toggleTopGuide(x, y);
  }

  public void openAiChat() {
    this.windowActionOwner.openAiChat();
  }

  public boolean canUseRangeCulling() {
    return this.modeSessionOwner.canUseRangeCulling();
  }

  public boolean isRangeCullingManagementActive() {
    return this.modeSessionOwner.isRangeCullingManagementActive();
  }

  public void toggleRangeCullingManagement() {
    this.modeSessionOwner.toggleRangeCullingManagement();
  }

  public void openBottomGuide(int x, int y) {
    this.modeSessionOwner.openBottomGuide(x, y);
  }

  public boolean isGuideOpen() {
    return this.modeSessionOwner.isGuideOpen();
  }

  public boolean isGearMenuOpen() {
    return this.modeSessionOwner.isGearMenuOpen();
  }

  public boolean isCraftQuantityDialogOpen() {
    return this.modeSessionOwner.isCraftQuantityDialogOpen();
  }

  void activateFunnelHotkey() {
    this.modeSessionOwner.activateFunnelHotkey();
  }

  void deactivateFunnelHotkey() {
    this.modeSessionOwner.deactivateFunnelHotkey();
  }

  void beginFunnelMouseHold(int button) {
    this.modeSessionOwner.beginFunnelMouseHold(button);
  }

  void endFunnelMouseHold(int button) {
    this.modeSessionOwner.endFunnelMouseHold(button);
  }

  void syncFunnelHoldState() {
    this.modeSessionOwner.syncFunnelHoldState();
  }

  void updateModeWheelAltState() {
    this.modeSessionOwner.updateModeWheelAltState();
  }

  boolean canOpenModeWheel() {
    return this.modeSessionOwner.canOpenModeWheel();
  }

  void selectModeFromWheel(BuilderMode mode) {
    this.modeSessionOwner.selectModeFromWheel(mode);
  }

  boolean handleRangeCullingSelectionClick(double mouseX, double mouseY, int button) {
    return this.worldQueryOwner.handleRangeCullingSelectionClick(mouseX, mouseY, button);
  }

  boolean handleRangeCullingWorldAction(double mouseX, double mouseY) {
    return this.worldQueryOwner.handleRangeCullingWorldAction(mouseX, mouseY);
  }

  boolean handleBoxHandleDrag(int button, double dragX, double dragY) {
    return this.modeSessionOwner.handleBoxHandleDrag(button, dragX, dragY);
  }

  double[] screenAxisForDirection(Direction direction) {
    return this.modeSessionOwner.screenAxisForDirection(direction);
  }

  void updateRangeCullingHover(double mouseX, double mouseY) {
    this.modeSessionOwner.updateRangeCullingHover(mouseX, mouseY);
  }

  void updateAdvancedRangeDestroyHover(double mouseX, double mouseY) {
    this.modeSessionOwner.updateAdvancedRangeDestroyHover(mouseX, mouseY);
  }

  public boolean isBlueprintPlacementModeLocked() {
    return this.modeSessionOwner.isBlueprintPlacementModeLocked();
  }

  void enforceBlueprintPlacementModeLock() {
    this.modeSessionOwner.enforceBlueprintPlacementModeLock();
  }

  void quickDropSelectedAtCursor() {
    this.modeSessionOwner.quickDropSelectedAtCursor();
  }

  public void blurSearchFocus() {
    this.worldQueryOwner.blurSearchFocus();
  }

  public void focusStorageSearchBox() {
    this.worldQueryOwner.focusStorageSearchBox();
  }

  public void focusCraftSearchBox() {
    this.worldQueryOwner.focusCraftSearchBox();
  }

  public boolean isWorldArea(double mouseX, double mouseY) {
    return this.worldQueryOwner.isWorldArea(mouseX, mouseY);
  }

  public int getBottomY() {
    return this.worldQueryOwner.getBottomY();
  }

  public int getFloatingPanelAvailableHeight(int panelY) {
    return this.worldQueryOwner.getFloatingPanelAvailableHeight(panelY);
  }

  boolean isInsideBottomPanel(double mouseX, double mouseY) {
    return this.worldQueryOwner.isInsideBottomPanel(mouseX, mouseY);
  }

  public boolean isSearchFocused() {
    return this.worldQueryOwner.isSearchFocused();
  }

  public int getSelectedToolSlot() {
    return this.worldQueryOwner.getSelectedToolSlot();
  }

  ItemStack getSelectedToolStack() {
    return this.worldQueryOwner.getSelectedToolStack();
  }

  String resolveGuiBindingItemId(BlockHitResult hit) {
    return this.worldQueryOwner.resolveGuiBindingItemId(hit);
  }

  public boolean canUseToolSlotShapeSource() {
    return this.worldQueryOwner.canUseToolSlotShapeSource();
  }

  boolean tryAssignQuickSlotFromToolSelection(int pinIndex) {
    return this.worldQueryOwner.tryAssignQuickSlotFromToolSelection(pinIndex);
  }

  public void setSelectedToolSlot(int slot) {
    this.worldQueryOwner.setSelectedToolSlot(slot);
  }

  public BlueprintGhostPreview getBlueprintGhostPreview() {
    return this.previewQueryOwner.getBlueprintGhostPreview();
  }

  public List<BlockPos> collectUltiminePreviewBlocks() {
    return this.previewQueryOwner.collectUltiminePreviewBlocks();
  }

  public List<BlockPos> filterToBounds(List<BlockPos> blocks) {
    return this.previewQueryOwner.filterToBounds(blocks);
  }

  boolean isMovePlayerActionMouse(int button) {
    return this.previewQueryOwner.isMovePlayerActionMouse(button);
  }

  boolean isMovePlayerActionKey(int keyCode, int scanCode) {
    return this.previewQueryOwner.isMovePlayerActionKey(keyCode, scanCode);
  }

  boolean handleMovePlayerActionAt(double mouseX, double mouseY) {
    return this.previewQueryOwner.handleMovePlayerActionAt(mouseX, mouseY);
  }

  public void enableRtsScissor(GuiGraphics g, int x1, int y1, int x2, int y2) {
    this.previewQueryOwner.enableRtsScissor(g, x1, y1, x2, y2);
  }

  public String trimToWidth(String text, int maxWidth) {
    return this.previewQueryOwner.trimToWidth(text, maxWidth);
  }

  public String text(String key, Object... args) {
    return this.previewQueryOwner.text(key, args);
  }

  public String selectedItemStatusLabel() {
    return this.previewQueryOwner.selectedItemStatusLabel();
  }

  boolean hasMainHandItem() {
    return this.worldQueryOwner.hasMainHandItem();
  }

  ItemStack resolveCursorPreview() {
    return this.previewQueryOwner.resolveCursorPreview();
  }

  boolean shouldRenderFunnelCursor() {
    return this.previewQueryOwner.shouldRenderFunnelCursor();
  }

  public Vec3 computeCursorRayDirection() {
    return this.previewQueryOwner.computeCursorRayDirection();
  }

  public Vec3 currentRayOrigin() {
    return this.previewQueryOwner.currentRayOrigin();
  }

  public Direction currentCameraHorizontalDirection() {
    return this.previewQueryOwner.currentCameraHorizontalDirection();
  }

  public PlacedBlockRotationHandles getRotationHandles() {
    return this.previewQueryOwner.getRotationHandles();
  }

  public BlockHitResult pickBlockHit() {
    return this.previewQueryOwner.pickBlockHit();
  }

  public InteractionTypes.InteractionTarget pickInteractionTarget(boolean includeFluidSource) {
    return this.previewQueryOwner.pickInteractionTarget(includeFluidSource);
  }

  public ScreenShapeController getShapeController() {
    return this.previewQueryOwner.getShapeController();
  }

  public String fillModeLabel(ShapeFillMode mode) {
    return this.previewQueryOwner.fillModeLabel(mode);
  }
  ...
  public String currentShapeSizeText() {
    return this.previewQueryOwner.currentShapeSizeText();
  }

  public String currentShapeCostText() {
    return this.previewQueryOwner.currentShapeCostText();
  }

  public String pendingShapeStatusText() {
    return this.previewQueryOwner.pendingShapeStatusText();
  }

  public String shapeLabel(BuildShape shape) {
    return this.previewQueryOwner.shapeLabel(shape);
  }

  boolean isAltDown() {
    return this.previewQueryOwner.isAltDown();
  }

  boolean isAltDownForInput() {
    return this.worldQueryOwner.isAltDownForInput();
  }

  double currentMouseX() {
    return this.previewQueryOwner.currentMouseX();
  }

  double currentMouseY() {
    return this.previewQueryOwner.currentMouseY();
  }
  ...
  boolean forwardUnhandledMouseReleased(double x, double y, int button) {
    return super.mouseReleased(x, y, button);
  }

  boolean forwardUnhandledMouseDragged(double x, double y, int button, double dx, double dy) {
  ...
  boolean forwardUnhandledKeyReleased(int key, int scan, int modifiers) {
    return super.keyReleased(key, scan, modifiers);
  }

  boolean forwardUnhandledCharTyped(char codePoint, int modifiers) {
