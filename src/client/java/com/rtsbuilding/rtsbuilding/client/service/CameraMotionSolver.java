...
final class CameraMotionSolver {
    ...
    static Pose solve(Pose pose, Bounds bounds, Input input) {
        ...
        float pitch = Mth.clamp(pose.pitchDeg + input.rotateY * ROTATE_GAIN_Y, -90.0F, 90.0F);
        ...
        double x = pose.x + dx;
        double y = pose.y + Mth.clamp(input.vertical, -4.0F, 4.0F) * (input.fast ? FAST_VERTICAL_SPEED : VERTICAL_SPEED);
        double z = pose.z + dz;
        ...
        x = Mth.clamp(x, bounds.anchorX - bounds.maxRadius, bounds.anchorX + bounds.maxRadius);
        z = Mth.clamp(z, bounds.anchorZ - bounds.maxRadius, bounds.anchorZ + bounds.maxRadius);
        y = Mth.clamp(y, bounds.anchorY + MIN_HEIGHT_OFFSET, bounds.anchorY + MAX_HEIGHT_OFFSET);
