...
/**
 * 蓝图材料恢复窗口的 Minecraft 薄绘制适配层。
 *
 * <p>共享 Kit 负责行、列和动作框；本类只恢复真实物品图标、材料统计和翻译文本。</p>
 */
final class BlueprintResumePanelRenderer {
    ...
    static void render(
        ...
        WorkflowResumeChromeRenderer.renderBlueprint(
                new MinecraftUiCanvas(graphics, font), geometry, canResume, actionHover);
        renderHeader(graphics, font, geometry, data);
        renderRows(graphics, font, geometry, data, scrollOffset);
        WorkflowResumeRenderSupport.drawActionText(graphics, font, geometry.action,
                canResume ? "screen.rtsbuilding.workflow.blueprint_resume.restart"
                        : "screen.rtsbuilding.workflow.blueprint_resume.insufficient_materials",
                WorkflowResumeStyle.action(WorkflowResumeStyle.ActionKind.RESUME,
                        canResume, actionHover).text.toArgb());
    }

    static boolean allMaterialsEnough(S2CRtsBlueprintResumeScanPayload data) {
        if (data == null) {
            return false;
        }
    ...
    private static void renderHeader(
        ...
        WorkflowResumeRenderSupport.draw(graphics, font,
                ...
                geometry.x, geometry.y, WorkflowResumeStyle.PROGRESS_TEXT.toArgb());
        ...
        WorkflowResumeRenderSupport.draw(graphics, font,
                ...
                geometry.x, headerY, WorkflowResumeStyle.LABEL_TEXT.toArgb());
        WorkflowResumeRenderSupport.draw(graphics, font,
                WorkflowResumeRenderSupport.text(
                        "screen.rtsbuilding.workflow.blueprint_resume.required"),
                geometry.requiredColumnX, headerY, WorkflowResumeStyle.LABEL_TEXT.toArgb());
        WorkflowResumeRenderSupport.draw(graphics, font,
                WorkflowResumeRenderSupport.text(
                        "screen.rtsbuilding.workflow.blueprint_resume.available"),
                geometry.availableColumnX, headerY, WorkflowResumeStyle.LABEL_TEXT.toArgb());
    }
    ...
    private static void renderRows(
            ...
            S2CRtsBlueprintResumeScanPayload data,
            int scrollOffset) {
        for (WorkflowResumeWindowLayout.BlueprintRowGeometry row : geometry.rows) {
            int sourceIndex = scrollOffset + row.visibleIndex;
            renderRow(graphics, font, geometry, row,
                    data.itemIds().get(sourceIndex), data.itemLabels().get(sourceIndex),
                    data.required().get(sourceIndex), data.available().get(sourceIndex));
        }
    }
    ...
    private static void renderRow(
        ...
        ItemStack displayStack = WorkflowResumeRenderSupport.item(itemId);
        if (!displayStack.isEmpty()) {
            graphics.item(displayStack, (int) row.itemIcon.getX(),
                    (int) row.itemIcon.getY());
            graphics.nextStratum();
            WorkflowResumeRenderSupport.draw(graphics, font,
                    WorkflowResumeRenderSupport.truncate(itemLabel, font, 100),
                    (int) row.itemIcon.getX() + 18,
                    (int) row.itemIcon.getY() + 4,
                    WorkflowResumeStyle.ITEM_TEXT.toArgb());
        } else {
            WorkflowResumeRenderSupport.draw(graphics, font, itemLabel,
                    (int) row.itemIcon.getX(), (int) row.itemIcon.getY() + 4,
                    WorkflowResumeStyle.ITEM_TEXT.toArgb());
        }
        WorkflowResumeRenderSupport.draw(graphics, font, String.valueOf(required),
                geometry.requiredColumnX, (int) row.row.getY() + 4,
                WorkflowResumeStyle.PRIMARY_TEXT.toArgb());
        WorkflowResumeRenderSupport.draw(graphics, font,
                enough ? String.valueOf(available)
                        : WorkflowResumeRenderSupport.text(
                                "screen.rtsbuilding.workflow.blueprint_resume.missing", missing),
                geometry.availableColumnX, (int) row.row.getY() + 4,
