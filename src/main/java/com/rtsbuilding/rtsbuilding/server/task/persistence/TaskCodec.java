...
public final class TaskCodec {
    ...
    private CompoundTag encodeSnapshotUnchecked(TaskSnapshot snapshot) {
        ...
        if (snapshot.reason() != RtsOperationReason.UNKNOWN) tag.putInt("reason_id", snapshot.reason().wireId());
    ...
    public TaskSnapshot decodeSnapshot(CompoundTag tag) {
        ...
        if (tag.contains("reason_id")) {
            if (!tag.contains("reason_id", Tag.TAG_INT)) throw new TaskCodecException("reason_id 类型错误");
