...
public final class RtsGuiCompatProbe {
  ...
  private static final int REQUIRED_STABLE_TICKS =
      resolveInt("rtsbuilding.guiCompatStableTicks", "RTSBUILDING_GUI_COMPAT_STABLE_TICKS", 40);
  private static final int TARGET_SEARCH_RADIUS =
      resolveInt(
          "rtsbuilding.guiCompatTargetSearchRadius",
          "RTSBUILDING_GUI_COMPAT_TARGET_SEARCH_RADIUS",
          32);

  private static final Path REPORT_PATH = resolveReportPath();
  private static final String CASE_ID =
      resolveConfig(
          "rtsbuilding.guiCompatCaseId", "RTSBUILDING_GUI_COMPAT_CASE_ID", "vanilla_chest");
  private static final String TARGET_BLOCK =
      resolveConfig(
          "rtsbuilding.guiCompatTargetBlock",
          "RTSBUILDING_GUI_COMPAT_TARGET_BLOCK",
          "minecraft:chest");
  private static final String EXPECTED_MENU_REGEX =
      resolveConfig(
          "rtsbuilding.guiCompatExpectedMenuRegex",
          "RTSBUILDING_GUI_COMPAT_EXPECTED_MENU_REGEX",
          defaultExpectedMenuRegex(CASE_ID));
  private static final String EXPECTED_SCREEN_REGEX =
      resolveConfig(
  ...
  private static final String SETUP_COMMAND =
      stripLeadingSlash(
          resolveConfig(
              "rtsbuilding.guiCompatSetupCommand",
              "RTSBUILDING_GUI_COMPAT_SETUP_COMMAND",
              "rtsbuilding_gui_compat_setup vanilla_chest"));
  private static final boolean AUTO_RUN =
      resolveBoolean("rtsbuilding.guiCompatAutoRun", "RTSBUILDING_GUI_COMPAT_AUTO_RUN");
  private static final boolean AUTO_EXIT =
      resolveBoolean("rtsbuilding.guiCompatAutoExit", "RTSBUILDING_GUI_COMPAT_AUTO_EXIT");
  private static final Path SUITE_PATH =
      resolveOptionalPath("rtsbuilding.guiCompatSuite", "RTSBUILDING_GUI_COMPAT_SUITE");
  ...
  private static final RtsGuiCompatProbeReport REPORT =
      new RtsGuiCompatProbeReport(
          REPORT_PATH,
          SUITE.suiteId(),
          resolveConfig(
              "rtsbuilding.guiCompatBaselineSha", "RTSBUILDING_GUI_COMPAT_BASELINE_SHA", "unknown"),
          resolveConfig(
              "rtsbuilding.guiCompatManifestHash",
              "RTSBUILDING_GUI_COMPAT_MANIFEST_HASH",
              "unknown"));
  ...
  private static AutoRun autoRun =
      AUTO_RUN && REPORT_PATH != null && RESOLVED_SUITE.error().isBlank()
          ? new AutoRun(REPORT.resumeIndex(SUITE.cases().size()))
          : null;

  private RtsGuiCompatProbe() {}

  /** 在 Fabric 客户端命令树注册探针入口；未配置报告路径时不暴露调试命令。 */
  public static void initialize() {
    ...
    ClientCommandRegistrationCallback.EVENT.register(
        (dispatcher, registryAccess) ->
            dispatcher.register(
                ClientCommandManager.literal("rtsbuilding_gui_compat_run")
                    .executes(context -> startFromCommand(CASE_ID))
                    .then(
                        ClientCommandManager.argument("caseId", StringArgumentType.word())
                            .executes(
                                context ->
                                    startFromCommand(
  ...
  /** 由 Fabric 的客户端 tick 尾部回调驱动探针状态机。 */
  public static void onClientTickPost() {
    ...
    if (!screenClass.equals(lastScreenClass)
        || !screenTitle.equals(lastScreenTitle)
        || !menuClass.equals(lastMenuClass)
  ...
  private static int startFromCommand(String requestedCaseId) {
    ...
    if (requestedCase == null) {
      writeRow(
          "run-start",
          "SKIP_SETUP",
          currentScreenClass(minecraft),
          currentScreenTitle(minecraft),
          currentMenuClass(minecraft),
    ...
    if (!target.trustedServerSetup()
        && !currentCase.blockId().isBlank()
    ...
    writeRow(
        ...
        "pos="
            ...
            + target.observedBlock()
            + " clientChunkLoaded="
            + target.clientChunkLoaded()
            + " trustedServerSetup="
            + target.trustedServerSetup());
  ...
  private static void closeStaleBuilderScreen(Minecraft minecraft) {
    ...
    if (screen != null
        && screen
            .getClass()
            .getName()
            .equals("com.rtsbuilding.rtsbuilding.client.screen.standalone.BuilderScreen")) {
      minecraft.setScreen(null);
      writeRow(
          "screen-close-stale-rts",
          "INFO",
          "",
          "",
          currentMenuClass(minecraft),
  ...
  /** 将测试清单里的点击面与方块内偏移应用到服务端真实交互射线。 默认仍命中方块中心；只有 Pipez 抽取臂等按局部碰撞体分派菜单的方块才需要覆盖。 */
  private static BlockHitResult applyCaseHitGeometry(
      BlockHitResult source, RtsGuiCompatCase guiCase) {
    BlockPos pos = source.getBlockPos();
    Vec3 location =
        Vec3.atCenterOf(pos).add(guiCase.hitOffsetX(), guiCase.hitOffsetY(), guiCase.hitOffsetZ());
  ...
  private static TargetResolution observedTarget(Minecraft minecraft, BlockHitResult hit) {
    ...
    return new TargetResolution(
        hit, loaded ? blockIdAt(minecraft, hit.getBlockPos()) : "<unloaded>", loaded, false);
  }
  ...
  private static boolean matchesTargetBlock(Minecraft minecraft, BlockPos pos) {
    ...
    return currentCase
        .blockId()
        .equals(BuiltInRegistries.BLOCK.getKey(state.getBlock()).toString());
  }
  ...
  private static TargetResolution findNearestTargetHit(Minecraft minecraft) {
    ...
    for (BlockPos pos :
        BlockPos.betweenClosed(
            playerPos.offset(-radius, -3, -radius), playerPos.offset(radius, 5, radius))) {
    ...
    return observedTarget(
        minecraft, new BlockHitResult(Vec3.atCenterOf(nearest), Direction.UP, nearest, false));
  }

  private record TargetResolution(
      BlockHitResult hit,
      String observedBlock,
      boolean clientChunkLoaded,
      boolean trustedServerSetup) {}

  private static void tickActiveRun(
      Minecraft minecraft,
      String screenClass,
      String screenTitle,
      String menuClass,
    ...
    if (activeRun.stage == SmokeStage.SEND_INTERACT) {
      ...
      writeRow(
          ...
          activeRun.guiCase.interactionItemId().isBlank()
              ...
              : "Sent RTS tool-slot right-click with "
                  + activeRun.guiCase.interactionItemId()
                  + ".");
      return;
    }
    ...
    if (activeRun.stage == SmokeStage.OBSERVE) {
      ...
      boolean menuMatches =
          hasMenu && matchesRegex(menu.getClass().getName(), activeRun.guiCase.expectedMenuRegex());
      boolean screenMatches =
          hasScreen && matchesRegex(screenClass, activeRun.guiCase.expectedScreenRegex());
      if (hasMenu && !menuMatches) {
        finishActiveRun(
            ...
            "Unexpected menu: "
                ...
                + " expected="
                + activeRun.guiCase.expectedMenuRegex());
        return;
      }
      if (hasMenu && hasScreen && !screenMatches) {
        finishActiveRun(
            ...
            "Unexpected screen: "
                + screenClass
                + " expected="
                + activeRun.guiCase.expectedScreenRegex());
        return;
      }
      ...
      if (hasMenu && hasContainerScreen && menuMatches && screenMatches) {
        ...
        if ("VANILLA_INTERACTION".equals(activeRun.guiCase.depth())
          ...
          if (activeRun.interactionDriver == null) {
            activeRun.interactionDriver =
                new RtsGuiCompatVanillaInteractionDriver(activeRun.guiCase);
          }
        ...
        boolean interactionReady =
            !"VANILLA_INTERACTION".equals(activeRun.guiCase.depth()) || activeRun.interactionPassed;
      ...
      if (activeRun.stageTicks > SUITE.openTimeoutTicks()) {
        finishActiveRun(
            ...
            menuClass,
            containerId,
            "Expected menu did not open within "
                + SUITE.openTimeoutTicks()
  ...
  private static void tickAutoRun(
    ...
    if (autoRun.stage == AutoStage.WAIT_WORLD) {
      ...
      writeRow(
          ...
          menuClass,
          containerId,
          "Player position stayed at "
              + minecraft.player.blockPosition().toShortString()
              + " for "
              + autoRun.worldStability.stableTicks()
              + " ticks before setup.");
    ...
    if (autoRun.stage == AutoStage.WAIT_SETUP) {
      int waitTicks = currentCase == null ? DEFAULT_AUTO_SETUP_DELAY : currentCase.setupWaitTicks();
  ...
  private static void finishAutoRun(Minecraft minecraft) {
    ...
    writeRow(
        ...
        currentContainerId(minecraft),
        "Completed "
            + Math.min(autoRun.caseIndex, SUITE.cases().size())
            + "/"
            + SUITE.cases().size()
            + " cases.");
    if (AUTO_EXIT) {
  ...
  private static void recordTransition(
      String screenClass, String screenTitle, String menuClass, int containerId) {
    if (!lastScreenClass.equals(screenClass)) {
      writeRow(
          screenClass.isEmpty() ? "screen-close" : "screen-open",
          "INFO",
          screenClass,
          screenTitle,
          menuClass,
    ...
    if (!lastMenuClass.equals(menuClass) || lastContainerId != containerId) {
      writeRow(
          menuClass.isEmpty() ? "menu-close" : "menu-open",
          "INFO",
          screenClass,
          screenTitle,
          menuClass,
  ...
  private static Path resolveReportPath() {
    return resolveOptionalPath(
        "rtsbuilding.guiCompatProbeReport", "RTSBUILDING_GUI_COMPAT_PROBE_REPORT");
  }
  ...
  private static ResolvedSuite resolveSuite() {
    RtsGuiCompatCase fallbackCase =
        new RtsGuiCompatCase(
            CASE_ID,
            TARGET_BLOCK,
            resolveInt(
                "rtsbuilding.guiCompatTargetDistance",
                "RTSBUILDING_GUI_COMPAT_TARGET_DISTANCE",
                20),
            "OPEN_STABLE",
            "single_block",
    ...
    try {
      ...
      RtsbuildingMod.LOGGER.error(
          "Failed to load RTS GUI compat suite {}; automatic probing is disabled.",
          SUITE_PATH,
          exception);
      return new ResolvedSuite(
          RtsGuiCompatSuiteLoader.single(fallbackCase, REQUIRED_STABLE_TICKS, 120),
  ...
  private static String resolveConfig(
      String propertyName, String environmentName, String fallback) {
  ...
  private static void writeRow(
    ...
    REPORT.append(
        tick, reportCase, event, status, screenClass, screenTitle, menuClass, containerId, note);
  }
  ...
  private record ResolvedSuite(RtsGuiCompatSuiteLoader.RtsGuiCompatSuite suite, String error) {}
}
