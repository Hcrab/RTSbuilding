...
public final class QuestService {
    ...
    public static void runQuestDetect(ServerPlayer player, RtsStorageSession session, boolean force) {
        ...
        if (!isQuestIntegrationAvailable()) {
            if (force) {
        ...
        QuestDetectResult result = runQuestDetection(player);
        if (force) {
    ...
    private static boolean isQuestIntegrationAvailable() {
        return !RtsCompatRegistry.getQuestIntegrations().isEmpty();
    }

    private static QuestDetectResult runQuestDetection(ServerPlayer player) {
        for (var qi : RtsCompatRegistry.getQuestIntegrations()) {
            if (qi.isAvailable()) return qi.detect(player);
        }
