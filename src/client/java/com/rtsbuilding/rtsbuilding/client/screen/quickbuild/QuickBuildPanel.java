 ...
 *
 * <p>继承 {@link RtsWindowPanel} 获得窗口能力。 向后兼容 {@code isQuickBuildOpen() / setQuickBuildOpen() /
 * toggleOpen()}。
 */
public final class QuickBuildPanel extends RtsWindowPanel {
  ...
  public boolean submitConvenienceDestroy(BlockHitResult hit) {
    return this.convenience.submit(isDestroyModeActive(), hit, this.screen.getSelectedToolSlot());
  }
  ...
  public boolean submitOrAnchorSmartFill(BlockHitResult hit, Vec3 rayOrigin, Vec3 rayDirection) {
    if (!isSmartFillMode()) {
      return false;
    }
    syncSmartFillSettings();
    return this.smartFill.submitOrAnchor(
        Minecraft.getInstance(), hit, rayOrigin, rayDirection, this.controller::confirmSmartFill);
  }
  ...
  protected void renderContent(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
    ...
    int x = this.windowX;
    int y = this.windowY;
    QuickBuildWindowLayout.Geometry sharedLayout = QuickBuildWindowLayout.geometry(x, y, core.mode);
    MinecraftUiCanvas canvas = new MinecraftUiCanvas(g, screen.font(), screen);
    this.controlSurface.render(
        g, canvas, screen, core, sharedLayout, this.windowWidth, mouseX, mouseY, partialTick);
  ...
  /**
   * Quick Build 本身不可缩放，因此持久化边界只保留玩家摆放的位置，尺寸始终以当前母版为准。 这也会自动迁移旧版本保存的 178×358 外框，避免新母版内容缩小后仍套着旧外壳。
   */
  @Override
  ...
  /** 所有生产按钮先经过纯 reducer，再由 1.21.1 adapter 执行副作用。 这样离屏输入回放与真实窗口不会再维护两套模式/形状切换规则。 */
  private QuickBuildUiTransition dispatchCore(QuickBuildUiAction action) {
    QuickBuildUiTransition transition =
        QuickBuildUiReducer.apply(QuickBuildUiAdapter.snapshot(this), action);
  ...
  public boolean isRangeDestroyChainMode() {
    return isRangeDestroyMode()
        && !isConvenienceDestroyMode()
        && effectiveRangeDestroyShape() == AreaMineShape.CHAIN;
  }

  /** 创造覆盖只在建造模式且本地玩家仍处于创造模式时生效。 偏好值可以保留，但切回生存后不会继续随请求发送。 */
   ...
   * 构造期安全地读取本地玩家模式。
   *
   * <p>{@link BuilderScreen} 构造函数初始化面板时，Minecraft 尚未把 Screen 的 {@code minecraft}
   * 字段挂上去，因此这里必须读取客户端单例，不能调用 {@code screen.getMinecraft()}。
   */
  ...
  private AreaMineShape effectiveRangeDestroyShape() {
    ...
    AreaMineShape fallback =
        QuickBuildUnlockPolicy.firstAvailableDestroyShape(
            this.controller.isProgressionEnabled(),
            hasPlugin(BuiltInRtsPluginCatalog.CHAIN_BREAK_PLUGIN),
            hasPlugin(BuiltInRtsPluginCatalog.AREA_DESTROY_PLUGIN));
    if (fallback == null) {
  ...
  private void applyActiveShapeToController() {
    ...
    BuildShape buildShape =
        effectiveMode() == QuickBuildMode.SMART_FILL
            ? BuildShape.BLOCK
            : this.preferences.buildShape();
  ...
  /** 解析当前用于形状建造的物品栈： 优先返回 RTS 存储中选中的物品，其次返回玩家手持工具槽位的物品。 */
