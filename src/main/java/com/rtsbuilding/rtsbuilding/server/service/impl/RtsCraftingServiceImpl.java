 ...
 * 静态工具方法的代理层，将请求委托给RTS储存合成系统处理。 负责管理合成终端GUI的打开、可合成物品列表的请求、 配方合成到链接存储、JEI 一键传输以及合成格填充等操作。
 */
public final class RtsCraftingServiceImpl implements CraftingService {
  ...
  public void requestCraftables(
    ...
    requestCraftables(
        player, search, showUnavailable, offset, limit, currentCraftPinyinSearchEnabled(player));
  }
  ...
  public void craftRecipeToLinked(ServerPlayer player, String recipeId, int craftCount) {
    ...
    RtsStorageCrafting.craftRecipeToLinked(
        player, registry.session().getOrCreate(player), recipeId, craftCount);
  }

  @Override
  public void refillCurrentCraftGridFromBlueprintIds(
    ...
    RtsStorageCrafting.refillCurrentCraftGridFromBlueprintIds(
        player, registry.session().getIfPresent(player), blueprintIds, craftedItemId, craftedCount);
  }

  @Override
  public void refillCurrentCraftGridFromBlueprintStacks(
      ServerPlayer player,
  ...
  public void refillCraftGridFromBlueprint(
    ...
    RtsStorageCrafting.refillCraftGridFromBlueprint(
        menu, handlers, player, blueprint, fillAll, includePlayerFallback);
  }

  @Override
  public void refillCraftGridFromLinked(
      ServerPlayer player, AbstractContainerMenu craftingMenu, ItemStack[] blueprint) {
    RtsStorageCrafting.refillCraftGridFromLinked(
        player, registry.session().getIfPresent(player), craftingMenu, blueprint);
  }

  @Override
  public void recordCraftedOutput(ServerPlayer player, ItemStack crafted) {
    RtsStorageCrafting.recordCraftedOutput(
        player, registry.session().getIfPresent(player), crafted);
  }
