 ...
 * <p>
 * Runs in {@link TickEvent.ClientTickEvent} before {@code aiStep()}, so the client's
...
public final class RtsClientPathfinding {
    ...
    private static void stopMovement() {
        target = null;
        targetYOffset = 0;
        if (previousMode != null && Minecraft.getInstance().player != null) {
            previousMode.onDeactivate(Minecraft.getInstance().player);
        }
        previousMode = null;
    }
   ...
   /**
     * Called from the Forge client tick start phase.
