...
public final class ShapeGeometryUtil {
    ...
    /**
     * 普通建造的显式几何入口。dimension 只约束直线/矩形轴和圆柱高度，radius
     * 只约束圆、圆柱底面和球半径；两者不能互相折算。
     */
     ...
     *
     * <p>调用方必须先通过 {@link ShapeSelectionLimiter#clampDimensionsAndVolume} 收紧包围盒总体积。
    ...
    /** 范围破坏的显式入口；输入已经由范围限制器按自身 X/Y/Z+体积规则收紧。 */
    ...
    /** 高级选区显式计划入口；box 的轴向尺寸由调用方按普通 dimension 先收紧。 */
    ...
    private static void addCircleTargets(Set<BlockPos> targets, BlockPos start, BlockPos end,
        ...
        if (degrees != 0) {
            ...
            for (BlockPos position : positions) {
                ...
                RotatedOffset offset = rotatePlaneOffset(toInt(cellA), toInt(cellB), 0.0D, 0.0D, degrees);
                rotated.add(offsetPos(start, axes[0], offset.a(), axes[1], offset.b()));
            }
            positions = rotated;
        }
    ...
    private static void addBoxTargets(Set<BlockPos> targets, BlockPos start, BlockPos end,
        ...
        if (fillMode == ShapeFillMode.FILL) {
            for (long iy = minY; iy <= maxY; iy++) {
                List<BlockPos> layer = new ArrayList<>((int) Math.min(width * depth, Integer.MAX_VALUE));
                for (long x = minX; x <= maxX; x++) {
                    for (long z = minZ; z <= maxZ; z++) {
                        layer.add(new BlockPos(toInt((long) start.getX() + x),
                                toInt((long) start.getY() + iy), toInt((long) start.getZ() + z)));
                    }
                }
                layer.sort(Comparator.comparingDouble(pos -> pos.distSqr(start)));
                targets.addAll(layer);
            }
            return;
        }
