...
/**
 * 将声明式便捷破坏请求接入既有范围拆除工作流。
 *
 * <p>本类只拥有“服务端重新规划并提交”的边界。工具借用、插件权限、分 Tick、掉落入库
 * 和 Ctrl+Z 仍完全由正式 AREA_DESTROY 管道负责，避免形成第二套破坏实现。</p>
 */
public final class RtsConvenienceDestroyService {
    ...
    public RtsConvenienceDestroyPlanner.Plan submit(ServerPlayer player,
        ...
        return submit(player, mode, anchor, face, settings, toolSlot, toolItemId,
                toolPrototype, toolProtectionEnabled,
    ...
    public RtsConvenienceDestroyPlanner.Plan submit(ServerPlayer player,
        ...
        RtsConvenienceDestroyPlanner.Plan plan = RtsConvenienceDestroyPlanner.plan(
                player.level(), mode, anchor, face, settings);
        if (!plan.ready()) {
