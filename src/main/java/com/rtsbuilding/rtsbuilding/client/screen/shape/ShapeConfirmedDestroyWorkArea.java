...
public final class ShapeConfirmedDestroyWorkArea {
    ...
    public ConfirmedDestroyPreviewState state() { return this.state; }
    public void clearChain() { this.state.clearChain(); }
    ...
    public void rememberChain(List<BlockPos> blocks) {
        ...
        if (bounded.isEmpty()) this.state.clearChain();
        else this.state.rememberChain(bounded);
    }
    ...
    public ConfirmedDestroyPreviewState.Progress progress() {
        ...
        return new ConfirmedDestroyPreviewState.Progress(
                progressPos, this.controller.getMineProgressStage(),
