...
/**
 * 26.1 底栏四类物品网格的生产绘制适配器。
 *
 * <p>Core 快照负责已选择状态，真实条目仅用于取得物品预览；Kit 网格负责容量、分页
 * 与半开命中范围。它不发包、不抽取物品，也不改变远程仓储的可用条件。</p>
 */
public final class BottomPanelGridRenderer {
    ...
    public static int renderStorage(
    ...
    public static int renderCreative(
            GuiGraphicsExtractor graphics,
            Font font,
            List<BottomBarUiEntry> entries,
    ...
    public static int renderRecent(
            GuiGraphicsExtractor graphics,
            Font font,
            List<BottomBarUiEntry> entries,
        ...
        for (int row = 0; row < view.rows; row++) {
            for (int column = 0; column < view.columns; column++) {
                ...
                if (entry.sourceIndex >= 0 && entry.sourceIndex < sourceEntries.size()
                    ...
                    graphics.item(sourceEntries.get(entry.sourceIndex).preview(),
                            slotX + 2, slotY + 2);
                }
    ...
    private static void renderEmptyState(
            GuiGraphicsExtractor graphics,
            Font font,
    ...
    private static BottomBarUiEntry entryAt(
            List<BottomBarUiEntry> entries,
            int index) {
