...
/**
 * Crafting grid filler, responsible for automatically filling the 3x3 crafting grid from linked storage.
 *
 * <p>Supports three filling modes:
 * <ul>
 *   <li><b>Blueprint fill</b> ({@link #refillCraftGridFromLinked}) — Fills the crafting grid slot by slot from linked storage
 *   based on a predefined item blueprint, supporting single fill and multi-stack fill (up to 64 passes)</li>
 *   <li><b>Network packet fill</b> ({@link #refillCurrentCraftGridFromBlueprintIds} / 
 *   {@link #refillCurrentCraftGridFromBlueprintStacks}) — Fills the current crafting grid from item IDs
 *   or item prototype stacks sent by the client</li>
 *   <li><b>JEI one-click fill</b> ({@link #applyJeiTransfer}) — Supports JEI recipe transfer integration,
 *   can clear existing grid, preferred prototype matching, multi-stack fill</li>
 * </ul>
 *
 * <p>When filling, preferentially matches exact prototypes, falling back to any matching material.
 * If the grid already contains items, automatically detects stack limits and attempts incremental filling.
 */
public final class RtsCraftingGridFiller {
    ...
    /**
     * Refills the open crafting grid from linked storage using a single-item blueprint.
     */
    ...
    /**
     * Refills the current crafting grid from item IDs sent by the client.
     */
    public static void refillCurrentCraftGridFromBlueprintIds(
        ...
        if (session != null && craftedItemId != null && !craftedItemId.isBlank() && craftedCount > 0) {
            RtsServer.get().page().recordRecentItem(session, craftedItemId,
    ...
    /**
     * Refills the current crafting grid from exact item prototypes sent by the client.
     */
    ...
    /**
     * Executes the low-level grid refill loop from linked storage/player fallback.
     */
