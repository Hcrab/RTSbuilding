...
final class ClientRtsLifecycleOwner {
  ...
  void applyServerCameraState(S2CRtsCameraStatePayload payload) {
    ...
    if (payload.enabled()) {
      ...
      if (freshEnable) {
        ...
        // Clear stale player input to prevent WASD presses from before entering RTS mode from
        // affecting movement
      ...
      controller.cameraOrbitService.setBounds(
          payload.anchorX(), payload.anchorY(), payload.anchorZ(), payload.maxRadius());
  ...
  void tick() {
    ...
    boolean hasRemoteMenuOpen =
        minecraft.player.containerMenu != null && minecraft.player.containerMenu.containerId != 0;

    if (hasRemoteMenuOpen
        && minecraft.screen == null
        && controller.pendingRemoteMenuOpenTicks <= 0) {
      controller.screenlessRemoteMenuTicks++;
      if (controller.screenlessRemoteMenuTicks
          >= ClientRtsController.SCREENLESS_REMOTE_MENU_RECOVERY_TICKS) {
        RtsRemoteMenuClientDiagnostics.screenlessRecovery(
            minecraft.player.containerMenu, controller.screenlessRemoteMenuTicks);
    ...
    if (hasRemoteMenuOpen) {
      controller.pendingRemoteMenuOpenTicks = 0;
      try {
        AbstractContainerMenu activeRemoteMenu =
            RtsClientRemoteMenuCompat.install(minecraft, minecraft.player.containerMenu);
    ...
    RtsRemoteMenuClientDiagnostics.observe(
        minecraft, controller.pendingRemoteMenuOpenTicks, controller.screenlessRemoteMenuTicks);
    ...
    controller.cameraOrbitService.tick(
        minecraft,
        controller.anchorX,
        controller.anchorY,
        controller.anchorZ,
        controller.maxRadius);
    boolean storageViewVisible =
      ...
      if (localPlayer.getAbilities().flying) {
        ...
        if (ctrlHeld && spaceHeld) {
          ...
          localPlayer.setDeltaMovement(
              localPlayer.getDeltaMovement().x, upSpeed, localPlayer.getDeltaMovement().z);
        } else if (ctrlHeld && shiftHeld) {
          double downSpeed = localPlayer.getAbilities().getFlyingSpeed() * 3.0;
          localPlayer.setDeltaMovement(
              localPlayer.getDeltaMovement().x, -downSpeed, localPlayer.getDeltaMovement().z);
        }
      }
    }
  }
