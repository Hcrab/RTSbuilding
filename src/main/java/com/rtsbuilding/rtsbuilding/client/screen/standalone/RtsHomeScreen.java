...
public final class RtsHomeScreen extends Screen {
    ...
    public void extractRenderState(GuiGraphicsExtractor g, int mouseX, int mouseY, float partialTick) {
        ...
        g.centeredText(this.font, Component.translatable("screen.rtsbuilding.home"), this.width / 2, 12,
        ...
        if (this.controller.isProgressionHomeSet()) {
            ...
            drawInfoRow(g, x, y, contentW, Component.translatable("screen.rtsbuilding.home.dimension_label"),
                    Component.translatable("screen.rtsbuilding.home.dimension", this.controller.getProgressionHomeDimension()),
                    StandaloneScreenStyle.INFO_DIMENSION);
        } else {
            drawInfoRow(g, x, y, contentW, Component.translatable("screen.rtsbuilding.home"),
                    Component.translatable("screen.rtsbuilding.home.not_set"), StandaloneScreenStyle.WARNING_TEXT);
            y += ROW_H + 4;
            drawInfoRow(g, x, y, contentW, Component.translatable("screen.rtsbuilding.home.dimension_label"),
        ...
        drawInfoRow(g, x, y, contentW, Component.translatable("screen.rtsbuilding.home.radius_label"),
                Component.translatable("screen.rtsbuilding.home.radius", this.controller.getProgressionRadiusBlocks()),
        ...
        g.fill(x, y, x + contentW, warningBottom, StandaloneScreenStyle.WARNING_BACKGROUND.toArgb());
        g.horizontalLine(x, x + contentW, y, StandaloneScreenStyle.WARNING_DIVIDER.toArgb());
        drawWrapped(g, warning, x + 10, y + 9, contentW - 20, StandaloneScreenStyle.WARNING_TEXT);
    ...
    private void drawWrapped(GuiGraphicsExtractor g, Component text, int x, int y, int width, UiColor color) {
        for (var line : this.font.split(text, width)) {
            g.text(this.font, line, x, y, color.toArgb());
            y += 10;
        }
    }

    private void drawInfoRow(GuiGraphicsExtractor g, int x, int y, int width,
        ...
        g.text(this.font, label, x + 10, y + 9, StandaloneScreenStyle.INFO_LABEL.toArgb());
        String valueText = this.font.plainSubstrByWidth(value.getString(), width - labelW - 24);
        g.text(this.font, Component.literal(valueText), x + labelW, y + 9, valueColor.toArgb());
    }
    ...
    private void renderPageBackground(GuiGraphicsExtractor g) {
        ...
        g.horizontalLine(0, this.width, 32, StandaloneScreenStyle.BAR_DIVIDER.toArgb());
