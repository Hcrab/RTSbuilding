 ...
 * 远程菜单服务端生命期回归测试。
 *
 * <p>这些测试只验证服务端的公共关门闸门，不假装替代真实客户端 Screen 测试。重点是确保
 * 任意第三方菜单自己的 {@code stillValid()} 即使始终失败，也不会在被 RTS 精确跟踪时闪退；
 * 同时证明普通菜单、其他玩家和复用 containerId 的新菜单不会被误放行。
 */
@GameTestHolder(RtsbuildingMod.MODID)
@PrefixGameTestTemplate(false)
public final class RtsRemoteMenuGameTests {
    ...
    public static void farRtsChestSurvivesServerDistanceValidation(GameTestHelper helper) {
        ...
        helper.assertTrue(opened instanceof ChestMenu,
                "RTS 生产交互链应当打开远距离箱子菜单");

        helper.runAfterDelay(20, () -> {
            helper.assertTrue(player.containerMenu == opened,
                    "远距离箱子菜单不应被服务端 stillValid 关闭");
    ...
    public static void farRtsEnchantingTableSurvivesServerDistanceValidation(GameTestHelper helper) {
        ServerPlayer player = RtsServerGameTests.startRtsPlayer(helper, GameType.SURVIVAL);
        openFarBlockThroughProductionRts(helper, player, new BlockPos(13, 1, 3), Blocks.ENCHANTING_TABLE);
        AbstractContainerMenu opened = player.containerMenu;
        helper.assertTrue(opened instanceof EnchantmentMenu,
                "RTS 生产交互链应当打开远距离附魔台菜单");

        helper.runAfterDelay(20, () -> {
            helper.assertTrue(player.containerMenu == opened,
                    "远距离附魔台菜单不应被服务端 stillValid 关闭");
            RtsServerGameTests.stopPlayers(player);
            helper.succeed();
        });
    }
    ...
    public static void clientEmptyHandPlacementPathOpensAndKeepsFarChestMenu(GameTestHelper helper) {
        ...
        AbstractContainerMenu opened = player.containerMenu;
        helper.assertTrue(opened instanceof ChestMenu,
                "客户端空手右键的完整生产路径应立即打开远距离箱子菜单");
        helper.runAfterDelay(20, () -> {
            helper.assertTrue(player.containerMenu == opened,
                    "完整客户端路径打开的箱子菜单也必须通过后续 stillValid 检查");
            RtsServerGameTests.stopPlayers(player);
            helper.succeed();
        });
    }
    ...
    public static void arbitraryInvalidModMenuSurvivesWhenTracked(GameTestHelper helper) {
        ...
        helper.assertTrue(!opened.stillValid(player),
                "测试夹具必须模拟一个拥有自定义、始终失败 stillValid 的第三方菜单");

        helper.runAfterDelay(20, () -> {
            helper.assertTrue(player.containerMenu == opened,
                    "被精确跟踪的任意第三方菜单应绕过服务端公共关门闸门");
            RtsServerGameTests.stopPlayers(player);
            helper.succeed();
        });
    }
    ...
    public static void trackedRemoteMenuRejectsThirdPartyServerClose(GameTestHelper helper) {
        ...
        helper.assertTrue(player.containerMenu == opened,
                "被 RTS 跟踪的第三方菜单不应被模组自己的服务端主动关窗立即关闭");
    ...
    public static void untrackedInvalidMenuKeepsVanillaCloseBehavior(GameTestHelper helper) {
        ...
        helper.runAfterDelay(5, () -> {
            helper.assertTrue(player.containerMenu == player.inventoryMenu,
                    "未经过 RTS 打开的菜单仍应由原版 stillValid 正常关闭");
            RtsServerGameTests.stopPlayers(player);
            helper.succeed();
        });
    }
    ...
    public static void reusedContainerIdDoesNotKeepDifferentMenuOpen(GameTestHelper helper) {
        ...
        helper.runAfterDelay(5, () -> {
            helper.assertTrue(player.containerMenu == player.inventoryMenu,
                    "复用 containerId 的新菜单不应继承旧 RTS 菜单的豁免");
            RtsServerGameTests.stopPlayers(player);
            helper.succeed();
        });
    }
    ...
    public static void remoteMenuTrackingIsIsolatedPerPlayer(GameTestHelper helper) {
        ...
        helper.runAfterDelay(10, () -> {
            helper.assertTrue(trackedPlayer.containerMenu == tracked,
                    "被跟踪玩家的远程菜单应保持打开");
            helper.assertTrue(localPlayer.containerMenu == localPlayer.inventoryMenu,
                    "另一个玩家的同 ID 菜单不应获得远程豁免");
