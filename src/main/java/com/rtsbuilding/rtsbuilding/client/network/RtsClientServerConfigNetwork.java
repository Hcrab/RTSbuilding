...
/** G06 设置页直接调用的 Forge 客户端查询/读取/保存 API；不写远端配置文件。 */
public final class RtsClientServerConfigNetwork {
    ...
    public static synchronized void beginSession() { SESSION.begin(); clearLastResults(); }
    public static synchronized void clearSession() { SESSION.clear(); clearLastResults(); }
    public static synchronized RtsServerConfigRequestToken requestCurrent() {
        if (SESSION.sessionId() == 0L) { SESSION.begin(); clearLastResults(); }
        PacketDistributor.sendToServer(new C2SRtsServerConfigQueryPayload(SESSION.query()));
        return new RtsServerConfigRequestToken(SESSION.sessionId(), SESSION.lastIssuedRequestId(), SESSION.current().revision());
    }
    public static synchronized RtsServerConfigRequestToken save(List<RtsServerConfigChange> changes) { return save(current().revision(), changes); }
    /** 使用 UI 已确认基准 revision 保存；服务端仍会重新做权限与 revision 校验。 */
    public static synchronized RtsServerConfigRequestToken save(int expectedRevision, List<RtsServerConfigChange> changes) {
        if (SESSION.sessionId() == 0L) { SESSION.begin(); clearLastResults(); }
        PacketDistributor.sendToServer(new C2SRtsServerConfigUpdatePayload(SESSION.update(expectedRevision, changes)));
        return new RtsServerConfigRequestToken(SESSION.sessionId(), SESSION.lastIssuedRequestId(), expectedRevision);
    }
    public static RtsServerConfigView current() { return SESSION.current(); }
    public static boolean canEdit() { return current().editable(); }
    /** 返回最近一次保存 ACK；查询/广播不会消耗 UI 等待的保存结果。 */
    public static synchronized RtsServerConfigUpdateResult lastResult() { return SAVE_ACK.result(); }
    public static synchronized long lastResultSessionId() { return SAVE_ACK.sessionId(); }
    public static synchronized int lastResultRequestId() { return SAVE_ACK.requestId(); }
    public static synchronized RtsServerConfigUpdateResult lastSaveResult() { return SAVE_ACK.result(); }
    public static synchronized long lastSaveResultSessionId() { return SAVE_ACK.sessionId(); }
    public static synchronized int lastSaveResultRequestId() { return SAVE_ACK.requestId(); }
    public static synchronized long sessionId() { return SESSION.sessionId(); }
