 ...
 *
 * <p>Holds all storage-related fields and provides methods for querying and updating storage pages,
 * craftables, feedback, funnel, quick slots, and GUI bindings.
 */
public final class StorageStateManager {
  ...
  public void updateStoragePanelLayout(
    ...
    this.storagePanelWidthNormalized =
        StorageUiValueSanitizer.clampPanelNormalized(widthNormalized);
    this.storagePanelHeightNormalized =
        StorageUiValueSanitizer.clampPanelNormalized(heightNormalized);
  }
  ...
  /** 由合成终端的明确排序按钮直接选择字段，避免循环切换产生多余请求。 */
  ...
  public void assignQuickSlotFromSelected(
      int index, String selectedItemId, ItemStack selectedItemPreview) {
   ...
   * @param afterPageApplied called after all state is updated but before {@code
   *     refreshSelectedItemPreviewFromStorage} (used by the controller for cross-cutting concerns)
   */
  public void applyStoragePage(S2CRtsStoragePagePayload payload, Runnable afterPageApplied) {
    ...
    StoragePagePayloadDecoder.DecodedPage decoded =
        StoragePagePayloadDecoder.decode(payload, this.linkedStorageName);
    ...
    this.bindingState.applyQuickSlots(
        payload.quickSlotItemIds(), payload.quickSlotPreviews(), this.storageEntries);
  ...
  /**
   * Returns the stored storage entries for internal use by controller (e.g., selected item
   * preview).
   */
