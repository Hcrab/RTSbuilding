...
/** durable task 持久化的只读诊断出口；所有入口 fail-open。 */
public final class RtsPersistenceDiagnostics {
  ...
  public static void saveScheduled(
      TaskPersistenceCoordinator.PreparationResult preparation, int dirtyAfterSchedule) {
    ...
    event(
        false,
        false,
        "PERSIST_SAVE_SCHEDULED",
        "ticket",
        shortTicket(commit.ticketId().toString()),
        "records",
        commit.recordCount(),
        "estimated_bytes",
        Math.max(0L, preparation.estimatedBytes()),
        "deferred",
        preparation.deferredTaskIds().size(),
        "dirty",
        Math.max(0, dirtyAfterSchedule));
  }

  public static void saveAck(
      TaskPersistenceCoordinator.CommitAckResult ack, long elapsedMs, int dirtyRemaining) {
    if (ack == null) return;
    boolean failed = ack.outcome() != TaskPersistenceCoordinator.AckOutcome.ACKNOWLEDGED;
    event(
        failed,
        failed,
        failed ? "PERSIST_SAVE_FAILED" : "PERSIST_SAVE_ACK",
        "outcome",
        ack.outcome(),
        "records",
        ack.acknowledgedRevisions().size(),
        "purged",
        ack.purgedReceipts().size(),
        "bytes",
        ack.bytesWritten(),
        "dirty_remaining",
