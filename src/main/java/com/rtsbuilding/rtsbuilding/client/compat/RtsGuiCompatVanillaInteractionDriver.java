 ...
 * 然后等待服务端槽位同步。它明确不直接修改客户端 {@link Slot}，避免把“本地画面变化”误判为远程 GUI 可用。
 * 反射只用于给铁砧原生文本框输入测试名称；反射失败会返回 {@code FAIL}，不会影响正常客户端。</p>
 */
final class RtsGuiCompatVanillaInteractionDriver {
    ...
    TickResult tick(Minecraft minecraft, AbstractContainerMenu menu) {
        ...
        if (minecraft == null || minecraft.player == null || minecraft.gameMode == null || menu == null) {
    ...
    private TickResult tickChest(Minecraft minecraft, AbstractContainerMenu menu) {
        ...
        return countPlayerItem(menu, minecraft, Items.STONE) >= 16 && menu.getSlot(0).getItem().isEmpty()
    ...
    private TickResult tickCrafting(Minecraft minecraft, AbstractContainerMenu menu) {
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.OAK_PLANKS, 1)) {
            this.step++;
        } else if (this.step == 1 && menu.getSlot(1).hasItem()
    ...
    private TickResult tickFurnace(Minecraft minecraft, AbstractContainerMenu menu) {
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.RAW_IRON, 0)) {
            ...
            this.step++;
        } else if (this.step == 2 && menu.getSlot(0).getItem().is(Items.RAW_IRON)
    ...
    private TickResult tickEnchanting(Minecraft minecraft, AbstractContainerMenu menu) {
        ...
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.DIAMOND_SWORD, 0)) {
            ...
            if (this.step == 2 && this.stepTicks >= 40
                    && menu.getSlot(0).getItem().is(Items.DIAMOND_SWORD)
                    && menu.getSlot(1).getItem().is(Items.LAPIS_LAZULI)) {
                return TickResult.pass(
                        "Enchanting inputs synchronized; this pack exposed no selectable vanilla enchant option");
            }
        } else if (this.step == 3 && this.selectedEnchant >= 0 && menu.getSlot(0).getItem().isEnchanted()) {
            return TickResult.pass("Enchant option " + this.selectedEnchant + " applied and synchronized");
        }
        return TickResult.running();
    }
    ...
    private TickResult tickAnvil(Minecraft minecraft, AbstractContainerMenu menu)
        ...
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.IRON_SWORD, 0)) {
            ...
            this.step++;
        } else if (this.step == 3 && playerHasNamedItem(menu, minecraft, Items.IRON_SWORD, "RTS Probe")) {
    ...
    private TickResult tickSmithing(Minecraft minecraft, AbstractContainerMenu menu) {
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.NETHERITE_UPGRADE_SMITHING_TEMPLATE, 0)) {
            this.step++;
        } else if (this.step == 1 && menu.getSlot(0).hasItem()
            ...
            this.step++;
        } else if (this.step == 3 && this.stepTicks >= 40
    ...
    private TickResult tickStonecutter(Minecraft minecraft, AbstractContainerMenu menu) {
        ...
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.STONE, 0)) {
            ...
            this.step++;
        } else if (this.step == 3 && this.stonecutterOutput != null
    ...
    private TickResult tickBrewing(Minecraft minecraft, AbstractContainerMenu menu) {
        ...
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.POTION, 0)) {
            ...
            this.step++;
        } else if (this.step == 2 && menu.getSlot(3).getItem().is(Items.NETHER_WART)
    ...
    private TickResult tickGrindstone(Minecraft minecraft, AbstractContainerMenu menu) {
        if (this.step == 0 && placePlayerItem(minecraft, menu, Items.IRON_SWORD, 0)) {
            ...
            quickMove(minecraft, menu, 2);
            this.step++;
        } else if (this.step == 1 && this.stepTicks >= 40
    ...
    private static boolean placePlayerItem(Minecraft minecraft, AbstractContainerMenu menu, Item item,
            int targetSlotId) {
    ...
    private static void click(Minecraft minecraft, AbstractContainerMenu menu, int slotId, ClickType clickType) {
        minecraft.gameMode.handleInventoryMouseClick(menu.containerId, slotId, 0, clickType, minecraft.player);
    }
    ...
    private static boolean playerHasNamedItem(AbstractContainerMenu menu, Minecraft minecraft, Item item, String name) {
        for (Slot slot : menu.slots) {
            ItemStack stack = slot.getItem();
            if (slot.container == minecraft.player.getInventory() && stack.is(item)
    ...
    private static boolean playerHasUnenchantedItem(AbstractContainerMenu menu, Minecraft minecraft, Item item) {
        for (Slot slot : menu.slots) {
            ItemStack stack = slot.getItem();
            if (slot.container == minecraft.player.getInventory() && stack.is(item) && !stack.isEnchanted()) {
                return true;
            }
        }
        return false;
