...
public final class RtsServerConfigNetwork {
    ...
    private static void handleQuery(ServerPlayer player, byte[] data) {
        ...
        } catch (RuntimeException malformed) { RtsbuildingMod.LOGGER.debug("忽略无效的服务端配置查询包", malformed); }
    }
    private static void handleUpdate(ServerPlayer player, byte[] data) {
        RtsServerConfigWire.Update update;
        try { RtsServerConfigWire.Request request = RtsServerConfigWire.decodeRequest(data); if (request.isQuery()) return; update = request.update(); }
        catch (RuntimeException malformed) { RtsbuildingMod.LOGGER.debug("忽略无效的服务端配置修改包", malformed); return; }
        long currentSession = SESSIONS.getOrDefault(player.getUUID(), 0L);
        if (currentSession == 0L || currentSession != update.sessionId()) {
            RtsServerConfigUpdateResult expired = new RtsServerConfigUpdateResult(RtsServerConfigUpdateResult.Status.INVALID,
                    Config.currentServerSettings(canEdit(player)).readOnly(), "configuration session expired");
    ...
    @SubscribeEvent public static void onPlayerLoggedIn(PlayerEvent.PlayerLoggedInEvent event) { if (event.getEntity() instanceof ServerPlayer p) SESSIONS.remove(p.getUUID()); }
    @SubscribeEvent public static void onPlayerLoggedOut(PlayerEvent.PlayerLoggedOutEvent event) { SESSIONS.remove(event.getEntity().getUUID()); }
    @SubscribeEvent public static void onServerStarted(ServerStartedEvent event) { Config.beginServerConfigRuntime(); SESSIONS.clear(); }
    @SubscribeEvent public static void onServerStopped(ServerStoppedEvent event) { SESSIONS.clear(); Config.endServerConfigRuntime(); Config.clearRawConfigSnapshots(); }
    ...
    private static long sessionFor(ServerPlayer player) { long value = player.getUUID().getMostSignificantBits() ^ player.getUUID().getLeastSignificantBits() ^ System.nanoTime(); return value == 0L ? 1L : value; }
    private static void broadcastCurrent(MinecraftServer server) {
        if (server == null) return;
        for (ServerPlayer p : server.getPlayerList().getPlayers()) { long session = SESSIONS.getOrDefault(p.getUUID(), 0L); if (session != 0L) send(p, session, 0, Config.currentServerSettings(canEdit(p))); }
    }
    private static void send(ServerPlayer p, long session, int requestId, com.rtsbuilding.rtsbuilding.common.config.RtsServerConfigView view) { send(p, session, requestId, view, "", RtsServerConfigUpdateResult.Status.NO_CHANGE); }
    private static void send(ServerPlayer p, long session, int requestId, com.rtsbuilding.rtsbuilding.common.config.RtsServerConfigView view, String message, RtsServerConfigUpdateResult.Status status) {
        // 配置会话只暴露客户端确实消费的规则；服务端技术预算留在服务端文件与运行时。
        RtsServerConfigUpdateResult result = new RtsServerConfigUpdateResult(status, view.clientProjection(), message);
        RtsClientboundPackets.sendToPlayer(p, new S2CRtsServerConfigResultPayload(RtsServerConfigWire.encodeResponse(session, requestId, result)));
    }
}
