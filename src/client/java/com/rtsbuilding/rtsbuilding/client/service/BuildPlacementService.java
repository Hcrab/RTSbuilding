...
public final class BuildPlacementService {
  ...
  public String getPlacementStatePreset() {
    return this.placementStatePreset;
  }
  ...
  public void selectStorageEntry(int index, List<StorageEntry> entries, Runnable setModeInteract) {
  ...
  public void selectFluidEntry(int index, List<FluidEntry> entries, Runnable setModeInteract) {
    if (index < 0 || index >= entries.size()) {
      return;
    }
  ...
  public void selectRecentEntry(int index, List<RecentEntry> entries, Runnable setModeInteract) {
    if (index < 0 || index >= entries.size()) {
      return;
    }
  ...
  public void selectQuickSlot(
      int index, String qsItemId, ItemStack qsPreview, String qsLabel, Runnable setModeInteract) {
  ...
  public void selectItemForPlacement(
      String itemId, String label, ItemStack preview, Runnable setModeInteract) {
    ...
    setSelectedItem(
        itemId,
        label == null || label.isBlank() ? safePreview.getHoverName().getString() : label,
        safePreview);
    clearSelectedFluid();
    setModeInteract.run();
  }
  ...
  public void placeSelected(
    ...
    long selectedCount =
        getSelectedItemCountForPlacement(
            itemId, isLocalPlayerCreative, storageTotalCount, hasStoragePageSnapshot);
    ...
    boolean clearAfterPlace =
        !payloadItemId.isBlank() && autoClearUnavailable && selectedCount <= 1L;
  ...
  public void confirmSmartFill(
    ...
    String itemId = this.selectedItemId == null ? "" : this.selectedItemId;
    ItemStack prototype =
        this.selectedItemPreview == null ? ItemStack.EMPTY : this.selectedItemPreview.copy();
    if (itemId.isBlank()) {
      ...
      if (minecraft.player != null) {
        ItemStack held =
            minecraft
                .player
                .getInventory()
                .getItem(Mth.clamp(minecraft.player.getInventory().selected, 0, 8));
  ...
  public void interactEmpty(
      BlockHitResult hit, Vec3 rayOrigin, Vec3 rayDir, Runnable beginRemoteMenuOpenGrace) {
    beginRemoteMenuOpenGrace.run();
    RtsClientPacketGateway.sendEmptyHandPlace(hit, rayOrigin, rayDir);
  }

  public void interactEntityEmpty(
      int entityId,
      Vec3 hitLocation,
      Vec3 rayOrigin,
      Vec3 rayDir,
  ...
  public void interactEntityWithToolSlot(
    ...
    beginRemoteMenuOpenGrace.run();
    RtsClientPacketGateway.sendInteractEntityWithToolSlot(
  ...
  public void rotateBlockStep(BlockPos pos, Direction axisDirection, int quarterTurns) {
    if (pos != null && axisDirection != null && quarterTurns != 0) {
      RtsClientPacketGateway.sendRotateBlockStep(pos, axisDirection, quarterTurns);
    }
  }
  ...
  public void setPlacementStateProperty(String propertyName, String valueName) {
    this.placementStatePreset =
        PlacementStatePreset.withValue(this.placementStatePreset, propertyName, valueName);
  ...
  private long getSelectedItemCountForPlacement(
      String itemId,
      boolean isLocalPlayerCreative,
      long storageTotalCount,
      boolean hasStoragePageSnapshot) {
  ...
  public void syncSelectedPreviewFromStorage(
      List<StorageEntry> entries, boolean hasStoragePageSnapshot, long storageTotalCount) {
  ...
  private boolean shouldAutoClearSelectedItemWhenUnavailable(
      boolean hasStoragePageSnapshot, long storageTotalCount) {
