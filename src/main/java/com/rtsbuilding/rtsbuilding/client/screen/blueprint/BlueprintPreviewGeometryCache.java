 ...
 * 内容边界只在蓝图身份变化时扫描；旋转只变换边界八个角，方块状态按调色板惰性旋转。
 * 渲染器分批取局部方块，因此选择巨型蓝图不会先同步分配全量预览列表。
 */
final class BlueprintPreviewGeometryCache {
    ...
    void prepare(RtsBlueprint blueprint, int requestedY, int requestedX, int requestedZ, int requestedLimit) {
        ...
        if (source != blueprint || limit != safeLimit) {
            ...
            for (int i = 0; i < blueprint.blocks().size(); i++) {
                ...
                if (i < safeLimit) {
                    previewBounds.include(block.relativePos());
                }
                if (block.isMissingBlock() || (block.state() != null && !block.state().isAir())) {
        ...
        localBlocks = new RotatedBlocks(source.blocks(), Math.min(limit, source.blockCount()), y, x, z, center);
        preview = null;
    }
    ...
    BlueprintGhostPreview preview(BlockPos anchor, boolean materialsReady) {
        if (preview == null || !preview.anchor().equals(anchor) || preview.materialsReady() != materialsReady) {
            preview = BlueprintGhostPreview.at(localBlocks, anchor, localBounds,
                    materialsReady, source.blockCount() > localBlocks.size());
        }
        return preview;
    }
    ...
    private static final class BoundsAccumulator {
        ...
        AABB build() {
            return minX == Integer.MAX_VALUE ? null : new AABB(minX, minY, minZ, maxX + 1D, maxY + 1D, maxZ + 1D);
        }
    }

    /** 直角旋转为轴置换，变换内容边界的八角与逐块扫描得到的极值完全一致。 */
    private static AABB rotateBounds(AABB bounds, int y, int x, int z, BlockPos center) {
        ...
        for (int corner = 0; corner < 8; corner++) {
            ...
            rotated = union(rotated, new AABB(BlueprintTransform.rotateAroundCenter(pos, y, x, z, center)));
        }
        return rotated;
    }

    private static final class RotatedBlocks extends AbstractList<BlueprintGhostBlock> implements RandomAccess {
        ...
        private RotatedBlocks(List<RtsBlueprintBlock> source, int size, int y, int x, int z, BlockPos center) {
            this.source = source;
            this.size = size;
            this.y = y;
            this.x = x;
        ...
        public BlueprintGhostBlock get(int index) {
            ...
            BlockState state = block.isMissingBlock() ? Blocks.AIR.defaultBlockState()
                    : states.computeIfAbsent(block.state(), value -> BlueprintTransform.rotateState(value, y, x, z));
            return new BlueprintGhostBlock(BlueprintTransform.rotateAroundCenter(block.relativePos(), y, x, z, center),
