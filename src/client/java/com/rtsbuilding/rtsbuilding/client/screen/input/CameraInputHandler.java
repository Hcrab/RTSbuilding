 ...
 *
 * <p>Manages mouse dragging (right-click rotation, middle-click pan), mining actions, keyboard
 * camera control, and keyboard pan-drag states. All state is used in BuilderScreen event methods;
 * this class stores and manages these states and provides helper methods for input detection and
 * action execution.
 *
 * <p>Default mouse bindings:
 *
 * <ul>
 *   <li>Right-click drag -> camera rotation
 *   <li>Middle-click drag -> camera pan (movement)
 *   <li>Middle-click without drag -> pick block for placement
 * </ul>
 */
public final class CameraInputHandler {
  ...
  public static boolean isKeyboardPanDragActionHeld() {
    InputConstants.Key key = ((KeyMappingAccessor) ClientKeyMappings.CAMERA_PAN_DRAG).getBoundKey();
    return key.getType() == InputConstants.Type.KEYSYM
        && ClientKeyMappings.CAMERA_PAN_DRAG.isDown();
  }
   ...
   * 取消尚未结束的鼠标点击/拖动判定。
   *
   * <p>模式轮盘等模态界面接管鼠标时必须调用本方法，避免打开轮盘前按下的 右键在轮盘关闭后又被解释成一次世界交互或旋转操作。
   */
  ...
  public void beginRightPress(
      double mouseX, double mouseY, int button, boolean primaryMouse, boolean rotateMouse) {
   ...
   *
   * <ul>
   *   <li>Right button -> camera rotation by default
   *   <li>Middle button -> camera pan by default
   * </ul>
   *
   * The specific action is determined dynamically via {@link
   * CameraInputHandler#isPanDragActionMouse(int)} and {@link
   * CameraInputHandler#isRotateDragActionMouse(int)}.
   */
   ...
   *
   * <ul>
   *   <li>If rotation/pan occurred during drag, returns false (no primary action).
   *   <li>If middle-click (pick block) without drag, triggers {@link
   *       #tryPickHoveredBlockForPlacement()}.
   *   <li>Otherwise returns true iff a primary action should be triggered at the release position.
   * </ul>
   */
  ...
  public boolean handleMiddleDrag(
      double mouseX, double mouseY, int button, double dragX, double dragY) {
  ...
  /** 结束中键拖拽，返回 true 表示事件已处理。 如果中键按下时未发生拖拽且可拾取，则触发 tryPickHoveredBlockForPlacement。 */
  ...
  public boolean startMiningAt(double mouseX, double mouseY, int mouseButton, boolean keyboard) {
    ...
    if (screen.isQuickBuildRangeDestroyMode() && !screen.isQuickBuildRangeDestroyChainMode()) {
      return screen.handleQuickBuildRangeDestroyClick(
          mouseX, mouseY, keyboard ? RtsTraceInputKind.KEYBOARD : RtsTraceInputKind.MOUSE);
    }
    if (!screen.isQuickBuildRangeDestroyMode()
        && screen.getShapeController().hasConfirmedDestroyWorkArea()) {
      return false;
    }
    if (screen.isQuickBuildRangeDestroyMode()
  ...
  public void stopActiveMining() {
    stopActiveMining(RtsMiningStopOrigin.EXPLICIT_CANCEL);
  }
  ...
  public boolean tryPickHoveredBlockForPlacement() {
    ...
    if (mc.player != null) {
      ...
      RtsPickBlockPlacementSelector.Selection selection =
          RtsPickBlockPlacementSelector.resolve(
    ...
    this.controller.selectItemForPlacement(
        itemId.toString(), preview.getHoverName().getString(), preview);
    this.controller.copyPlacementState(state);
    return true;
  }
