 ...
 *
 * <p>放置、拆除、挖掘与掉落缓存回写共享同一个全局数量/时间预算。 新功能必须直接提交真正的 TaskRecord，不得再新增 Session 内的独立 Tick 驱动器。
 */
public final class RtsTaskEngine {
  ...
  private final Map<com.rtsbuilding.rtsbuilding.server.task.identity.TaskId, Long>
      projectedDurableStates = new java.util.HashMap<>();
  private final DurableBlueprintTaskBridge durableBlueprintBridge =
      new DurableBlueprintTaskBridge(
          this,
  ...
  public TaskScheduler.TickStats tick(MinecraftServer server) {
    ...
    java.util.LinkedHashMap<
            com.rtsbuilding.rtsbuilding.server.task.identity.TaskId,
            com.rtsbuilding.rtsbuilding.server.task.persistence.TaskSnapshot>
        durableCandidates = new java.util.LinkedHashMap<>();
    for (var player : server.getPlayerList().getPlayers()) {
      coordinator
          .query()
          .runnableFor(player.getUUID(), player.serverLevel().dimension().location().toString())
          .stream()
          .filter(
              snapshot ->
                  snapshot.type() == TaskType.PLACEMENT
    ...
    DurableTaskScheduler.TickStats durableStats =
        durableRuntime.tick(
            coordinator,
            durableCandidates.values(),
            Config.taskEngineMaxNanosPerTick(),
            Config.taskEngineMaxUnitsPerTick(),
            Config.taskEngineMaxUnitsPerSlice());
    long remainingNanos =
        Math.max(0L, Config.taskEngineMaxNanosPerTick() - durableStats.elapsedNanos());
    int remainingUnits =
        Math.max(0, Config.taskEngineMaxUnitsPerTick() - durableStats.processedUnits());
    TaskScheduler.TickStats legacyStats =
        scheduler.tick(remainingNanos, remainingUnits, Config.taskEngineMaxUnitsPerSlice());
    TaskScheduler.TickStats stats =
        new TaskScheduler.TickStats(
  ...
  public DurableBlueprintTaskBridge.QueueResult queueLegacyDurableBlueprint(
  ...
  public QueueDiagnostics queueDiagnostics() {
    int runnable = 0;
    int waiting = 0;
    if (com.rtsbuilding.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
        .isStarted()) {
      for (var snapshot :
          com.rtsbuilding.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
              .coordinator()
  ...
  public void detachPlayer(UUID playerId) {
    ...
    blueprintRecords.values().stream()
        .filter(record -> record.ownerId().equals(playerId))
        .map(TaskRecord::id)
        .forEach(removedTaskIds::add);
  ...
  /**
   * 登出/停用前把所有仍留在 Session 的旧迁移源提交给 TaskStore。 调用方随后必须执行 owner 定向 flush，并再次调用 {@link
   * #reconcilePlayerDetach}。
   */
  ...
  public boolean setWorkflowPaused(
      net.minecraft.server.level.ServerPlayer player, int workflowEntryId, boolean paused) {
    if (player == null || workflowEntryId < 0) return false;
    WorkflowTaskKey key =
        new WorkflowTaskKey(player.getUUID(), player.serverLevel().dimension(), workflowEntryId);
    var workflowEngine =
    ...
    var durable =
        coordinator
            ...
            .orElse(null);
    if (durable != null
        && (durable.type() == TaskType.PLACEMENT
  ...
  /** 收口指定维度的真实任务。工作流超时可能扫描到玩家当前维度以外的旧投影， 因此不能用玩家当前维度偷偷替换任务身份。 */
  public boolean cancelWorkflowTask(
    ...
    WorkflowTaskKey key = new WorkflowTaskKey(player.getUUID(), dimension, workflowEntryId);
    TaskRecord record = findWorkflowTask(key);
    var coordinator =
        com.rtsbuilding.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
            .coordinator();
    var durable =
        coordinator
            .query()
            .findByWorkflow(player.getUUID(), dimension.location().toString(), workflowEntryId)
            .orElse(null);
    ...
    if (durable != null
      ...
      if (currentDimension) {
        try {
          if (durable.type() == TaskType.PLACEMENT) {
            RtsPlacementBatch.recordDetachedHistory(
                player, durableRuntime.currentPlacementState(durable));
          } else if (durable.type() == TaskType.DESTRUCTION) {
            RtsDestructionBatch.recordDetachedHistory(
                player, durableRuntime.currentDestructionState(durable));
          } else {
            RtsMiningStateMachine.finalizeDetachedCancellation(
                player,
          ...
          RtsbuildingMod.LOGGER.warn(
              "[TaskEngine] 取消工作流 #{} 时无法恢复历史，将直接终止任务", workflowEntryId, malformedHistory);
        }
      }
      ...
      try {
        cancelled =
            durable.type() == TaskType.PLACEMENT
                ? durableRuntime.transitionPlacementSnapshot(
                    durable,
                    com.rtsbuilding.rtsbuilding.server.task.persistence.TaskLifecycleState
                        .CANCELLED,
      ...
      try {
        RtsServerTraceRegistry.externalTaskTerminal(cancelled, "CANCELLED", "WORKFLOW_CANCELLED");
      } catch (RuntimeException diagnosticFailure) {
        RtsbuildingMod.LOGGER.debug(
            "RTS external task terminal diagnostic failed", diagnosticFailure);
      }
    ...
    if (record != null) {
      ...
      if (record.payload() instanceof BlueprintTaskPayload) {
        com.rtsbuilding.rtsbuilding.server.pipeline.blueprint.BlueprintPersistence.clearFromEntry(
            player, workflowEntryId);
      }
  ...
  public void resumeWaitingPlacementItems(
      net.minecraft.server.level.ServerPlayer player, java.util.Collection<String> changedItemIds) {
    ...
    for (String itemId : new java.util.LinkedHashSet<>(changedItemIds)) {
      ...
      for (var snapshot : coordinator.query().waitingFor(key)) {
        if (!snapshot.ownerId().equals(player.getUUID())
            || snapshot.type() != TaskType.PLACEMENT
            || !snapshot
                .dimensionId()
                .equals(player.serverLevel().dimension().location().toString())) continue;
        coordinator.replace(
            snapshot.nextRevision(
                com.rtsbuilding.rtsbuilding.server.task.persistence.TaskLifecycleState.QUEUED,
                null,
                gameTime,
                snapshot.cursorUnits(),
  ...
  public PendingPlacementTaskView findWaitingPlacement(
    ...
    PlacementTaskPayload payload =
        com.rtsbuilding.rtsbuilding.server.task.placement.PlacementTaskCodec.decode(
            snapshot.payload());
  ...
  /** 把显式恢复策略先写入 TaskStore 新 revision，再交给 durable executor。 覆盖策略包含世界破坏，因此绝不能在本方法中直接修改世界。 */
  public boolean resumeWaitingPlacementWithStrategy(
    ...
    if (player == null
        ...
        || expectedTaskId == null
        || expectedRevision < 1L) return false;
    var coordinator =
        com.rtsbuilding.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
            .coordinator();
    var snapshot = coordinator.query().get(expectedTaskId).orElse(null);
    if (snapshot == null
        || snapshot.type() != TaskType.PLACEMENT
    ...
    var nextState =
        RtsPlacementBatch.applyDetachedResumeStrategy(player, payload.state(), strategy);
    ...
    coordinator.replace(
        snapshot.nextRevision(
            ...
            null,
            player.serverLevel().getGameTime(),
            nextState.cursorUnits(),
            nextState.succeededUnits(),
            nextState.failedUnits(),
            com.rtsbuilding.rtsbuilding.server.task.placement.PlacementTaskCodec.encode(
                nextPayload)));
    return true;
  }
   ...
   *
   * <p>查询时必须读取 durable 执行镜像，而不能只看最近一次落盘快照；首块可能已经在本 tick 完成并切换到批处理，但新的 checkpoint 仍在等待持久化 ACK。
   */
  public boolean hasCommittedMiningBatch(net.minecraft.server.level.ServerPlayer player) {
    ...
    return !states.isEmpty()
        && states.stream()
            .allMatch(
                com.rtsbuilding.rtsbuilding.server.task.mining.MiningTaskState::committedBatch);
  }
  ...
  private void resumeAvailableToolWaits(
    ...
    if (player == null
        ...
        || (!player.isCreative()
            ...
            && (session.mining.miningToolLease == null || session.mining.miningToolLease.isEmpty()))
        || com.rtsbuilding.rtsbuilding.server.service.mining.RtsMiningValidator.isToolNearBreak(
            player, session)) return;
    resumeWaitKey(
        coordinator,
        player.getUUID(),
        new com.rtsbuilding.rtsbuilding.server.task.persistence.TaskWaitKey(
            "tool", "usable_mining_tool"),
        player.serverLevel().getGameTime());
  ...
  public void resumeLoadedChunk(
      net.minecraft.server.level.ServerLevel level, net.minecraft.world.level.ChunkPos chunkPos) {
  ...
  public void submitBlueprint(
      BlueprintContext context, java.util.LinkedList<Integer> restoredRemaining) {
  ...
  TaskRecord activateDurableBlueprint(
    ...
    long now = System.nanoTime();
    BlueprintTaskPayload payload =
        new BlueprintTaskPayload(
            context, context.isPreparing() ? null : context.getRemainingQueue(), sourceDimension);
    TaskRecord record =
        new TaskRecord(
            taskId.value(),
            ...
            snapshot.totalUnits(),
            now);
    record.restoreDurableSnapshot(
        snapshot.cursorUnits(),
        snapshot.succeededUnits(),
        snapshot.failedUnits(),
        DurableBlueprintTaskBridge.runtimeState(snapshot.state()),
        now);
  ...
  public void submitBlueprint(
      BlueprintContext context,
      java.util.LinkedList<Integer> restoredRemaining,
      net.minecraft.resources.ResourceKey<net.minecraft.world.level.Level> sourceDimension) {
    if (context == null || context.player() == null) return;
    Integer entryId =
        context.getData(
    ...
    long now = System.nanoTime();
    BlueprintTaskPayload payload =
        new BlueprintTaskPayload(context, restoredRemaining, sourceDimension);
    TaskRecord record =
        new TaskRecord(
            UUID.nameUUIDFromBytes(
                (context.player().getUUID()
                        + ":blueprint:"
                        + sourceDimension.location()
                        + ":"
                        + entryId)
            ...
            TaskType.BLUEPRINT,
            payload,
            context.getBlueprint().blockCount(),
            now);
   ...
   *
   * <p>工作流面板与 durable task 分别持久化，旧世界恢复时两边的 nextId 可能短暂不同步。新工作流创建前必须避开仍由任务根占用的编号，不能等到
   * 任务提交时再以异常形式拒绝玩家操作。
   */
  ...
  public boolean submitPlacementJob(
      net.minecraft.server.level.ServerPlayer player, RtsPlacementBatch.PlaceBatchJob job) {
    ...
    if (job.quickBuild()
      ...
      player.displayClientMessage(
          net.minecraft.network.chat.Component.translatable(
              "message.rtsbuilding.quick_build.queue_full", Config.buildBatchMaxQueuedJobs()),
          true);
      return false;
    }
    PlacementTaskPayload payload =
    ...
    var taskId =
        com.rtsbuilding.rtsbuilding.server.task.identity.TaskId.fromSubmission(
            player.getUUID(), submission);
    long gameTime = player.serverLevel().getGameTime();
    var snapshot =
        new com.rtsbuilding.rtsbuilding.server.task.persistence.TaskSnapshot(
            taskId,
            submission,
            player.getUUID(),
            player.serverLevel().dimension().location().toString(),
   ...
   * 为新的玩家工作流任务整理对应任务族容量。
   *
   * <p>工作流槽位允许新任务淘汰未保护的旧任务；各持久化任务族不能再用另一套硬上限 推翻这条规则。只有当前维度、仍未终结且符合该任务族谓词的任务才占用容量。
   * 达到上限时优先终止最旧的未保护任务；缺失工作流投影的孤儿任务没有可保留的 “钉住”状态，因此同样必须允许新任务将其回收。
   */
      ...
      var replaceable =
          queued.stream()
              .filter(
                  snapshot -> {
                    var entry =
                        workflowEngine.findEntryByPlayer(player, snapshot.workflowEntryId());
              ...
              .min(
                  java.util.Comparator.comparingLong(
                          com.rtsbuilding.rtsbuilding.server.task.persistence.TaskSnapshot
                              ::createdGameTime)
                      .thenComparing(
                          com.rtsbuilding.rtsbuilding.server.task.persistence.TaskSnapshot::id))
              .orElse(null);
   ...
   *
   * <p>玩家无法在面板中钉住、暂停或删除一个不可见任务，因此这种孤儿任务不能享有保护权， 更不能继续占用任何任务族容量。放置、破坏和挖掘都在同一入口执行这项对账。
   */
  ...
  private void cancelDurableSnapshot(
    ...
    if (snapshot.workflowEntryId() >= 0
        && cancelWorkflowTask(
            player, player.serverLevel().dimension(), snapshot.workflowEntryId())) {
      return;
    }
  ...
  private static boolean occupiesQuickBuildSlot(
      com.rtsbuilding.rtsbuilding.server.task.persistence.TaskSnapshot snapshot) {
    try {
      return com.rtsbuilding.rtsbuilding.server.task.placement.PlacementTaskCodec.decode(
              snapshot.payload())
          .state()
          .definition()
          .getBoolean("quickBuild");
    } catch (RuntimeException malformed) {
      return true;
    }
  }

  public boolean submitDestructionJob(
      net.minecraft.server.level.ServerPlayer player, RtsDestructionBatch.DestructionJob job) {
    if (player == null || job == null) return false;
    var coordinator =
        com.rtsbuilding.rtsbuilding.server.task.persistence.TaskPersistenceRuntime.INSTANCE
            .coordinator();
    ...
    if (!makeRoomForDurableTaskFamily(
        player,
        coordinator,
        TaskType.DESTRUCTION,
        RtsDestructionBatch.DESTROY_MAX_QUEUED_JOBS,
        ignored -> true)) return false;
    ...
    var snapshot =
        new com.rtsbuilding.rtsbuilding.server.task.persistence.TaskSnapshot(
            ...
            job.successfulCount(),
            job.failedCount(),
            com.rtsbuilding.rtsbuilding.server.task.destruction.DestructionTaskCodec.encode(
                payload));
    coordinator.submit(snapshot);
    return true;
  }
  ...
  public boolean submitMiningTargets(
    ...
    java.util.List<net.minecraft.core.BlockPos> immutableTargets =
        targets.stream()
    ...
    var state =
        new com.rtsbuilding.rtsbuilding.server.task.mining.MiningTaskState(
            progressiveSingle
                ? com.rtsbuilding.rtsbuilding.server.task.mining.MiningTaskState.Mode
                    .PROGRESSIVE_SINGLE
                : com.rtsbuilding.rtsbuilding.server.task.mining.MiningTaskState.Mode.BATCH,
            workflowEntryId,
            immutableTargets,
            immutableTargets.size(),
            0,
            0,
            0,
            face,
  ...
  /** 旧 MiningJob 的唯一迁移入口。 */
  public boolean submitMiningJob(
      net.minecraft.server.level.ServerPlayer player,
      com.rtsbuilding.rtsbuilding.server.storage.session.RtsStorageSession session,
      RtsMiningStateMachine.MiningJob job) {
    if (player == null || session == null || job == null) return false;
    return submitMiningState(
        player, RtsMiningStateMachine.snapshotDetachedQueued(session, job, player.isCreative()));
  }

  private boolean submitMiningState(
      net.minecraft.server.level.ServerPlayer player,
  ...
  private TaskStepResult executeBlueprint(TaskRecord task, TaskBudget budget) {
    ...
    var token =
        com.rtsbuilding.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance()
            .from(payload.player(), payload.workflowEntryId())
            .orElse(null);
    if (token == null) {
      com.rtsbuilding.rtsbuilding.server.pipeline.blueprint.BlueprintPersistence.clearFromEntry(
          payload.player(), payload.workflowEntryId());
      return TaskStepResult.fail("rtsbuilding.task.error.workflow_missing");
    }
    if (!payload.ready()) {
      int processed = payload.prepare(budget);
      if (payload.ready()
          && payload.shouldCheckpoint(true)
  ...
  private void syncFunnelTask(
      net.minecraft.server.level.ServerPlayer player,
      com.rtsbuilding.rtsbuilding.server.storage.session.RtsStorageSession session) {
    TaskRecord existing = funnelRecords.get(player.getUUID());
    boolean active =
        session.funnel.funnelEnabled
    ...
    TaskRecord record =
        new TaskRecord(
            ...
            player.getUUID(),
            TaskType.FUNNEL,
            new FunnelTaskPayload(player, session),
            0,
            now);
  ...
  private void syncPlacedRecoveryTask(
      net.minecraft.server.level.ServerPlayer player,
      com.rtsbuilding.rtsbuilding.server.storage.session.RtsStorageSession session) {
    ...
    if (session.placement.recoveryJobs.isEmpty()) {
      if (existing != null && existing.status().terminal())
        recoveryRecords.remove(player.getUUID());
      return;
    }
    if (existing != null && !existing.status().terminal()) return;
    long now = System.nanoTime();
    TaskRecord record =
        new TaskRecord(
            UUID.randomUUID(),
            player.getUUID(),
            TaskType.PLACED_RECOVERY,
            new PlacedRecoveryTaskPayload(player, session),
            0,
            now);
  ...
  private TaskStepResult executePlacedRecovery(TaskRecord task, TaskBudget budget) {
    ...
    var result =
        RtsPlacedRecoveryService.tickBudgeted(
            payload.player(),
            payload.session(),
            budget.maxUnits(),
            System.nanoTime() + budget.remainingNanos());
  ...
  private void applyInitialPause(
      net.minecraft.server.level.ServerPlayer player,
      int workflowEntryId,
      TaskRecord record,
      long now) {
  ...
  private void projectWorkflowLifecycles() {
    ...
    for (TaskRecord record : records) {
      ...
      if (token == null) {
        if (record.payload() instanceof BlueprintTaskPayload payload) {
          ...
          RtsbuildingMod.LOGGER.warn(
              "[TaskEngine] 已终止缺失可见工作流投影的蓝图任务 {}（工作流 #{}）", record.id(), entryId);
        }
        continue;
      }
    ...
    java.util.Set<UUID> activeIds =
        records.stream().map(TaskRecord::id).collect(java.util.stream.Collectors.toSet());
    projectedTaskStatuses.keySet().removeIf(id -> !activeIds.contains(id));
  }

  /** TaskStore 生命周期到 Workflow 的单向投影；Workflow 永远不能反向成为执行权威。 */
  private void projectDurableWorkflowLifecycles(
    ...
    for (var player : server.getPlayerList().getPlayers()) {
      for (var snapshot : coordinator.query().ownedBy(player.getUUID())) {
        ...
        if (!snapshot.dimensionId().equals(player.serverLevel().dimension().location().toString()))
          continue;
        ...
        if (snapshot.workflowEntryId() < 0) {
          if (snapshot.state().terminal()) {
            ...
            RtsbuildingMod.LOGGER.warn("[TaskEngine] 已终止没有合法工作流 ID 的隐藏任务 {}", snapshot.id());
          }
          continue;
        }
        ...
        var workflowEngine =
            com.rtsbuilding.rtsbuilding.server.workflow.core.RtsWorkflowEngine.getInstance();
        var token = workflowEngine.from(player, snapshot.workflowEntryId()).orElse(null);
        if (snapshot.state().terminal()) {
   ...
   *
   * <p>旧版可能写入了没有建材 ID 的快速建造任务。这类任务既无法执行，也无法被 任意具体物品唤醒，必须标记为 poison 并经统一取消入口回收。
   */
  private DurableWorkflowProjection inspectDurableProjection(
    ...
    try {
      return switch (snapshot.type()) {
          ...
          var type =
              job.quickBuild()
                  ...
                  : snapshot.totalUnits() <= 1
                      ? com.rtsbuilding.rtsbuilding.server.workflow.model.RtsWorkflowType
                          .PLACE_SINGLE
                      : com.rtsbuilding.rtsbuilding.server.workflow.model.RtsWorkflowType
                          .PLACE_BATCH;
        ...
        case DESTRUCTION ->
            DurableWorkflowProjection.restore(
                com.rtsbuilding.rtsbuilding.server.workflow.model.RtsWorkflowType.AREA_DESTROY);
        case MINING -> {
          var state =
              com.rtsbuilding.rtsbuilding.server.task.mining.MiningTaskCodec.decode(
                      snapshot.payload())
                  .state();
      ...
      RtsbuildingMod.LOGGER.warn("[TaskEngine] 持久任务 {} 无法重建工作流投影", snapshot.id(), malformed);
      return DurableWorkflowProjection.poison();
    }
  }

  private record DurableWorkflowProjection(
      com.rtsbuilding.rtsbuilding.server.workflow.model.RtsWorkflowType type, boolean poisoned) {
  ...
  private void releaseTerminalWorkflow(TaskRecord record) {
    ...
    workflowPauseOverrides.remove(
        new WorkflowTaskKey(record.ownerId(), taskDimension(record), entryId));
    projectedTaskStatuses.put(record.id(), record.status());
  }
  ...
  private net.minecraft.resources.ResourceKey<net.minecraft.world.level.Level> taskDimension(
      TaskRecord record) {
  ...
  private int workflowEntryId(TaskRecord record) {
    ...
    if (record.payload() instanceof DestructionTaskPayload payload)
      return payload.workflowEntryId();
  ...
  public record TaskDiagnostics(
      Map<TaskType, Integer> activeByType, Map<TaskType, Integer> waitingByType) {}
