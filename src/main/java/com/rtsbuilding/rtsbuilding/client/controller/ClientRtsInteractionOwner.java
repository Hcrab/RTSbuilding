...
import static com.rtsbuilding.rtsbuilding.client.controller.ClientRtsController.*;
...
final class ClientRtsInteractionOwner {
    ...
    void applyDamageFeedback(S2CRtsDamageFeedbackPayload payload) {
        ...
        if (RtsClientUiStateStore.isRtsSoundsEnabled() && controller.damageSoundEnabled) {
        ...
        if (payload.lowHealth() && controller.damageAutoReturnEnabled && controller.enabled) {
            RtsClientPacketGateway.sendToggleCamera(controller.startCameraAtPlayerHead);
        }
    }
    ...
    void applyQuestDetectStatus(S2CRtsQuestDetectStatusPayload payload) {
        ...
        controller.questDetectExpiryMs = now + QUEST_DETECT_RESULT_VISIBLE_MS;
        controller.questDetectScannedTasks = Math.max(0, payload.scannedTasks());
        controller.questDetectTotalTasks = Math.max(0, payload.totalTasks());
        controller.questDetectCompletedTasks = Math.max(0, payload.completedTasks());
    }
    ...
    void confirmSmartFill(BlockHitResult hit, int maxBlocks, int detectionDiameter,
    ...
    void startMining(BlockPos pos, int face, int toolSlot, RtsTraceInputKind inputKind) {
        controller.miningOperationService.startMining(pos, face, toolSlot,
                controller.buildPlacementService.getSelectedItemId(),
                controller.buildPlacementService.getSelectedItemPreview(),
                controller.allowPlacedBlockRecovery, controller.toolProtectionEnabled,
                inputKind, inputOrigin(inputKind));
    }
    ...
    void startUltimine(BlockPos pos, int face, int toolSlot, int limit, byte mode,
        ...
        controller.miningOperationService.startUltimine(pos, face, toolSlot, limit, mode,
                controller.buildPlacementService.getSelectedItemId(),
                controller.buildPlacementService.getSelectedItemPreview(),
                controller.toolProtectionEnabled, inputKind, inputOrigin(inputKind));
    }
    ...
    static AreaMineBounds computeAreaMineBounds(BlockPos pointA, BlockPos pointB, int heightOffset) {
    ...
    void confirmAreaMine(int toolSlot, ShapeFillMode fillMode, RtsTraceInputKind inputKind) {
        controller.miningOperationService.confirmAreaMine(toolSlot, fillMode,
                controller.buildPlacementService.getSelectedItemId(),
                controller.buildPlacementService.getSelectedItemPreview(),
                controller.toolProtectionEnabled, inputKind);
    }
    ...
    private static String inputOrigin(RtsTraceInputKind inputKind) {
        if (inputKind == RtsTraceInputKind.KEYBOARD) return "KEY_PRESS";
        if (inputKind == RtsTraceInputKind.MOUSE) return "POINTER_PRESS";
