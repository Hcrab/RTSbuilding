 ...
 *
 * <p>该实现类通过 {@link com.rtsbuilding.rtsbuilding.server.pipeline.core.PipelineRegistry} 执行挖掘流程：
 *
 * <ul>
 *   <li>单方块挖掘（{@code MINE_SINGLE}）
 *   <li>连锁挖掘（{@code ULTIMINE}）
 *   <li>范围挖掘（{@code AREA_MINE}）
 *   <li>范围破坏（{@code AREA_DESTROY}）
 *   <li>停止挖掘（{@code STOP_MINING}）
 * </ul>
 *
...
public final class RtsMiningServiceImpl implements MiningService {
  ...
  public void mine(
    ...
    if (session != null && !committedBatch) {
      PipelineRegistry.execute(
          RtsWorkflowType.STOP_MINING, MiningContext.builder(player).operationTrace(trace).build());
  ...
  public void areaDestroy(
    ...
    areaDestroy(
        ...
        toolPrototype,
        toolProtectionEnabled,
        RtsOperationTraceContext.legacy("AREA_DESTROY"));
  }
  ...
  private static int getAreaDestroyMetric(
      ServerPlayer player, Function<RtsWorkflowStatus, Integer> metric) {
