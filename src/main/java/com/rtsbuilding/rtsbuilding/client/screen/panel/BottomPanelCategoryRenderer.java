...
/**
 * 底栏分类树的 26.1 Extractor 绘制适配器。
 *
 * <p>本类只将 Core 分类行、Kit 几何和主题翻译为绘制调用；分类来源、滚动、选择和
 * 展开动作仍由 BottomPanel 与生产适配器编排。</p>
 */
public final class BottomPanelCategoryRenderer {
    ...
    public static void render(
        ...
        int titleX = layout.panel.x + (layout.panel.width - font.width(title)) / 2;
        graphics.text(font, title, titleX,
                layout.panel.y + BottomPanelCategoryLayout.TITLE_TOP,
                BottomPanelCategoryStyle.TITLE_TEXT.toArgb(), false);
        ...
        for (int categoryIndex = layout.scroll; categoryIndex < to; categoryIndex++) {
            ...
            double rowHover = hover("row." + visibleRow,
                    row.contains(mouseX, mouseY), category.selected);
            fill(graphics, row,
                    BottomPanelCategoryStyle.rowBackground(category.selected, rowHover).toArgb());
            ...
            if (category.expandable) {
                ...
                double toggleHover = hover("toggle." + visibleRow,
                        toggle.contains(mouseX, mouseY), category.expanded);
                fill(graphics, toggle,
                        BottomPanelCategoryStyle.buttonBackground(
                                BottomPanelCategoryStyle.TOGGLE_BACKGROUND, toggleHover).toArgb());
                drawCenteredNoShadow(graphics, font, category.expanded ? "-" : "+", toggle);
                labelRight = toggle.x - BottomPanelCategoryLayout.LABEL_TOGGLE_GAP;
            }
            drawScaledLabel(graphics, font, category.label, labelX, labelRight, row,
    ...
    private static double hover(String id, boolean hovered, boolean selected) {
        UiControlState state = new UiControlState(true, selected, false, false, "")
                .withInteraction(hovered, false, false);
        return ANIMATIONS.update(id, state, Config.isUiAnimationsEnabled()).hover();
    }
    ...
    private static void drawScaledLabel(
        ...
        try {
            ...
            graphics.pose().scale(BottomPanelCategoryLayout.TEXT_SCALE,
                    BottomPanelCategoryLayout.TEXT_SCALE);
            graphics.text(font, label, 0, 0, color, false);
        } finally {
