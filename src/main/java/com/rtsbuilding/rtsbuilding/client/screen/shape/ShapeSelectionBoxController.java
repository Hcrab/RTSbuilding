...
public final class ShapeSelectionBoxController {
    ...
    public void init(BuilderScreen screen, Supplier<ShapeBuildTypes.Session> sessionReader,
    ...
    public Direction hoveredHandle() { return this.handles.hoveredDirection(); }
    public Direction activeHandle() { return this.handles.activeDirection(); }

    public Set<Direction> allowedDirections() {
        if (!hasEditableSession()) return Set.of();
        ShapeBuildTypes.Session session = current();
    ...
    public boolean releaseIfDragged() { return this.handles.releaseActiveHandleIfDragged(); }

    private boolean resize(Direction direction, int delta) {
        if (direction == null || delta == 0 || !hasEditableSession()) return false;
        ShapeBuildTypes.Session session = current();
        ...
        if (!withinCaps(after)) return true;
    ...
    public boolean nudge(int dx, int dy, int dz) {
        ...
        RtsCullingBox oldBox = hasEditableSession()
                ? AdvancedShapeSelectionGeometry.boxFromSession(session) : null;
    ...
    private boolean withinCaps(RtsCullingBox box) {
        ...
        if (isRangeDestroy()) return RangeDestroySelectionLimiter.contains(box, currentRangeDestroyLimits());
        int maxDimension = Math.max(1, Config.maxShapeDimension());
    ...
    private void update(ShapeBuildTypes.Session session) {
        if (this.sessionWriter != null) this.sessionWriter.accept(session);
    }
}
