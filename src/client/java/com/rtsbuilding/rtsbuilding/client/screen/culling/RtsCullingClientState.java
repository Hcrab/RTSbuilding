 ...
 * 范围剔除的客户端全局状态桥。
 *
 * <p>Mixin 和世界渲染器不能持有 BuilderScreen 实例，因此通过这里查询当前 打开的 RTS 页面。它只转发只读判断，不主动创建或修改剔除区域。
 */
public final class RtsCullingClientState {
  ...
  public static void requestCurrentWorldState() {
    ...
    ClientPlayNetworking.send(new C2SRtsRequestCullingStatePayload());
  }
  ...
  private static void saveCurrentWorldState() {
    ...
    List<RtsCullingBoxSnapshot> boxes =
        PERSISTENT_MANAGER.boxes().stream()
            .map(box -> new RtsCullingBoxSnapshot(box.min(), box.max()))
            .toList();
    ClientPlayNetworking.send(
        new C2SRtsSaveCullingStatePayload(
  ...
  public static double distanceAfterCulledBlock(
