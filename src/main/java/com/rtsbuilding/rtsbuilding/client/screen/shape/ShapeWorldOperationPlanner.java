...
public final class ShapeWorldOperationPlanner {
    @FunctionalInterface
    public interface TargetFilter { List<BlockPos> filter(ShapeBuildTypes.Input input, List<BlockPos> rawPositions); }
    @FunctionalInterface
    public interface TargetExecutor { void execute(List<BlockPos> validPositions); }
    ...
    public void init(BuilderScreen screen, ClientRtsController controller, ShapeModeState modeState,
            ShapeSelectionSession session, ShapeSelectionBoxController selectionBox) {
        this.screen = screen;
    ...
    public void clear() { this.generationPlans.clear(); }
    public RtsCullingBox generatedBounds() { return this.generationPlans.bounds(); }
    public List<BlockPos> generate(ShapeBuildTypes.Input input) { return generationPlan(input).positions(); }
    ...
    private ShapeGenerationPlanCache.Request requestFor(ShapeBuildTypes.Input input) {
        ...
        RtsCullingBox advancedBox = this.selectionBox.hasEditableSession()
                ? this.selectionBox.box() : null;
        return new ShapeGenerationPlanCache.Request(
                ...
                Config.maxShapeDimension(),
                Config.maxShapeRadius());
    }

    public List<BlockPos> filterToBounds(List<BlockPos> blocks) {
        if (!this.controller.hasBounds() || blocks == null) return blocks;
    ...
    public boolean isBreakable(BlockPos pos) {
        ...
        return state.getFluidState().isEmpty()
                && !state.isAir() && state.getDestroySpeed(mc.level, pos) >= 0.0F;
    }
    ...
    public ItemStack placementStack() {
        if (this.controller.hasSelectedItem()) return this.controller.getSelectedItemPreview();
        Minecraft mc = this.screen.getMinecraft();
    ...
    public BlockState pendingGhostState(BlockPos targetPos) {
        ...
        if (targetPos == null) return blockItem.getBlock().defaultBlockState();
        BlockState state = BuildGhostBlockStateResolver.resolveStateWithCamera(mc, blockItem, stack, targetPos);
        if (state == null) return null;
        int degrees = this.modeState.activeRotateDegrees();
        return degrees == 0 ? state
    ...
    public BlockHitResult templateHit(ShapeBuildTypes.Input input) {
        if (this.session.templateHit() != null) return this.session.templateHit();
        if (input == null || input.pointA() == null || input.placementFace() == null) return null;
        return ShapeGeometryUtil.createShapePlacementHit(input.pointA(), input.placementFace());
    }

    public boolean execute(ShapeBuildTypes.Input input, TargetFilter filter,
            TargetExecutor executor, Runnable clearSession) {
        ...
        if (!bounded.isEmpty()) executor.execute(bounded);
        return true;
    }
    ...
    public static List<BlockHitResult> wrapPlacementHits(List<BlockPos> positions, Direction face) {
        ...
        for (BlockPos pos : positions) hits.add(ShapeGeometryUtil.createShapePlacementHit(pos, face));
        return hits;
    }
}
