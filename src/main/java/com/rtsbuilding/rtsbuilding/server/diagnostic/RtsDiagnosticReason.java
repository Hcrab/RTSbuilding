 ...
 * 服务端 RTS 操作的稳定诊断原因代码。
 *
 * <p>该枚举面向日志和外部排障工具，不承载玩家提示文案，也不参与玩法判断。 分类器优先使用统一 Pipe 的职责名称，再谨慎读取现有结果文本；这样业务代码无需
 * 到处埋日志，同时未来调整人类可读文案也不会轻易改变常见原因代码。
 */
public enum RtsDiagnosticReason {
  ...
  public static RtsDiagnosticReason classify(
    ...
    if (pipe.contains("sessionvalidatepipe")
        || message.contains("no storage session")
