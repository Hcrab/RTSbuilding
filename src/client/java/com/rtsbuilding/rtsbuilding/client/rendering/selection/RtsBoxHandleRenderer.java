 ...
 * 世界空间盒子编辑手柄的统一渲染器。
 *
 * <p>范围剔除和蓝图框选共用同一组六向箭头：轴颜色、悬停高亮和锁定金色都必须保持一致。 这里不关心盒子的业务含义，也不绘制外框颜色；调用方只传入当前用于渲染的 AABB。
 */
public final class RtsBoxHandleRenderer {
  private RtsBoxHandleRenderer() {}

  public static void renderAxisHandles(
      PoseStack poseStack,
    ...
    renderAxisHandles(
        poseStack, lineBuffer, fillBuffer, box, hoveredDirection, activeDirection, null);
  }
  ...
  public static void renderAxisHandles(
    ...
    for (RtsCullingAxisHandle.Handle handle :
      ...
      AxisColor color =
          active
              ? axisColor(UiThemeWorldColors.HANDLE_ACTIVE)
      ...
      if (hovered && !active) {
        renderHandleBox(
            poseStack, lineBuffer, fillBuffer, handle.shaft().inflate(0.05D), color, 0.10F, 0.30F);
        renderHandleBox(
            poseStack, lineBuffer, fillBuffer, handle.head().inflate(0.07D), color, 0.12F, 0.38F);
      }
      if (active) {
        renderHandleBox(
            poseStack, lineBuffer, fillBuffer, handle.shaft().inflate(0.06D), color, 0.16F, 0.42F);
        renderHandleBox(
            poseStack, lineBuffer, fillBuffer, handle.head().inflate(0.08D), color, 0.20F, 0.54F);
      }
      renderHandleBox(
          poseStack, lineBuffer, fillBuffer, handle.shaft(), color, fillAlpha, lineAlpha);
      renderHandleBox(
          poseStack, lineBuffer, fillBuffer, handle.head(), color, fillAlpha, lineAlpha);
    }
  }

  private static void renderHandleBox(
      PoseStack poseStack,
      VertexConsumer lineBuffer,
      VertexConsumer fillBuffer,
      AABB box,
