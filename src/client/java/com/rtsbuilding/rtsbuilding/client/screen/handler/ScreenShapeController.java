...
public final class ScreenShapeController implements ShapeGhostPreviewProvider.Runtime {
  ...
  private final ShapeConfirmedDestroyWorkArea confirmedDestroyWorkArea =
      new ShapeConfirmedDestroyWorkArea();
  ...
  public void init(BuilderScreen screen, ClientRtsController controller) {
    ...
    this.worldOperations.init(
        screen, controller, this.modeState, this.selectionSession, this.selectionBox);
  ...
  public void rotateShapeByStep(int step) {
    int raw = this.modeState.activeRotateDegrees() + (step * SHAPE_ROTATE_STEP_DEGREES);
  ...
  /** 从 BUILD 切换到 DESTROY：保存当前活跃状态到 BUILD 独立字段， 然后将之前保存的 DESTROY 独立状态恢复到活跃字段。 */
  ...
  /** 从 DESTROY 切换到 BUILD：保存当前活跃状态到 DESTROY 独立字段， 然后将之前保存的 BUILD 独立状态恢复到活跃字段。 */
  ...
  /** 初始化时调用：将持久化的 BUILD 独立状态直接复制到活跃字段， 不覆盖独立字段中的值。 */
  public void applyBuildStateAsActive() {
    this.modeState.applyBuildState();
  }

  /** 初始化时调用：将持久化的 DESTROY 独立状态直接复制到活跃字段， 不覆盖独立字段中的值。 */
  ...
  public void placeWithShape(
      BlockHitResult hit,
      boolean forcePlace,
      Vec3 rayOrigin,
      Vec3 rayDir,
      double mouseY,
      boolean fluidPlacement,
    ...
    if (shape == BuildShape.BLOCK) {
      ...
      if (fluidPlacement) {
        ...
        BlockPos placePos =
            ShapePlacementTargetResolver.resolveClickedTarget(
                hit.getBlockPos(),
                hit.getDirection(),
  ...
  public void selectRangeDestroyShape(BlockHitResult hit, double mouseY, Vec3 rayDir) {
    selectRangeDestroyShape(hit, mouseY, rayDir, RtsTraceInputKind.UNKNOWN);
  }
    ...
    if (shape == BuildShape.BLOCK) {
      ...
      List<BlockPos> breakable =
          ShapeDestroyTargetClassifier.breakableTargets(
              List.of(hit.getBlockPos().immutable()), this::isBreakableDestroyTarget);
  ...
  private void advanceShapeSession(
      BlockHitResult hit, Vec3 rayDir, double mouseY, BuildShape shape) {
  ...
  public boolean tryConfirmPendingRangeDestroy() {
    return tryConfirmPendingRangeDestroy(RtsTraceInputKind.UNKNOWN);
  }
  ...
  public boolean tryConfirmPendingRangeDestroy(RtsTraceInputKind inputKind) {
    ...
    List<BlockPos> boundedEnvelope =
        filterToBounds(ShapeDestroyTargetClassifier.envelopeTargets(raw, boundedBreakable));
    clearShapeBuildSession();
  ...
  public boolean dragAdvancedRangeDestroyHandle(
    ...
    return this.worldOperations.execute(
        ...
        bounded -> {
          List<BlockHitResult> hits =
              ShapeWorldOperationPlanner.wrapPlacementHits(bounded, input.placementFace());
          if (useFluid) {
  ...
  @Override
  public List<BlockPos> filterPlacementTargets(
      ShapeBuildTypes.Input input, List<BlockPos> targets) {
  ...
  public void recordSinglePlacementForUndo(
      BlockHitResult hit,
      InteractionTypes.PlacementReplayKind replayKind,
      String itemId,
      int toolSlot) {}
  ...
  public boolean adjustShapeDimensionNudge(
      int delta, boolean adjustSecondaryAxis, boolean adjustHeight) {
  ...
  public String currentShapeCostText() {
    ...
    if (this.screen.isQuickBuildRangeDestroyMode()) {
      return ShapeSelectionTextPresenter.countText(
          ShapeDestroyTargetClassifier.breakableTargets(
                  generateShapePositions(input), this::isBreakableDestroyTarget)
              .size());
    }
  ...
  private String confirmKeyLabel(boolean destroyMode) {
    ...
    return this.screen.text(
        destroyMode ? "screen.rtsbuilding.input.lmb" : "screen.rtsbuilding.input.rmb");
  }
  ...
  private ShapeBuildTypes.Input resolveCurrentShapeBuildInput(
      BlockHitResult cursorHit, boolean requireReady) {
  ...
  private void rememberConfirmedRangeDestroyPreview(
      ShapeDestroyTargetClassifier.Selection preview) {
