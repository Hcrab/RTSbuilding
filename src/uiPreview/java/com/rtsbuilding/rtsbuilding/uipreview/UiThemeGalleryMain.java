...
/**
 * Renders the five built-in themes from one complete RTS scenario and a review contact sheet.
 *
 * <p>The entry point is report-only: it does not create a Minecraft client or alter player
 * configuration. Each image still passes through the production shared geometry, chrome and
 * theme resolution path used by {@link UiPreviewRenderer}.</p>
 *
 * <p>用同一个完整 RTS 场景离屏渲染五套内建主题，并额外生成便于审阅的联系表。该入口只用于
 * 开发报告，不创建 Minecraft 客户端或桌面窗口，也不会修改玩家配置；每张图仍经由
 * {@link UiPreviewRenderer} 所使用的生产共享几何、chrome 和主题解析路径。</p>
 */
