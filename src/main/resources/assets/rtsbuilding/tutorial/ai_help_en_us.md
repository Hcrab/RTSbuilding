...
### Forge 1.19.2 adapter note

Use this line's Forge 1.19.2 event/network adapters; there is no prior same-line release baseline.
...
## Version-specific technical appendix: Forge 1.19.2

This line is being prepared as `0.0.2` and has no previously published Forge 1.19.2 release to roll back to. Keep world and configuration backups. If the candidate is unstable, stop using it and restore the backup while waiting for a published release for this line.

### Task cancellation and diagnosis

When a new single mine, chain mine, or area-destruction replacement action starts, non-terminal `MINING` and area-destruction `DESTRUCTION` tasks for the same player and dimension must use this line's real task-cancellation path to reach `CANCELLED`, release runtime resources, preserve cursor/succeeded, write a tombstone, and close the workflow. `WAITING_RESOURCE` may continue without a replacement action; PLACE, QUICK_BUILD, and BLUEPRINT_BUILD must not be cancelled accidentally.

### Log-analysis quick reference

The prompt includes the last 200 lines of `latest.log` and the latest 200 RTSBuilding records. Find the earliest abnormal event, correlate `trace` across `seq/op/workflow/task`, and inspect `[RTS-TRACE]`, `[RTS-DIAG]`, `[RTS-SERVER-HEALTH]`, `BEGIN/RESULT/FILTER/TERMINAL`, and the first relevant stack trace. Do not treat the last error as the root cause; separate evidence, conclusions, and inferences.

### Forge 1.19.2 differences

This is the Forge 1.19.2 compatibility/landing line and uses its Forge event and networking adapters. There is no previous same-line release for a clean binary comparison. Capture the exact Minecraft/Forge/mod versions, server logs, and a recoverable backup before deciding whether the durable snapshot and workflow projection agree.
