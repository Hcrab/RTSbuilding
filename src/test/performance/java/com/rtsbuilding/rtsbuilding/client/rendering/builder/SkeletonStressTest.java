...
class SkeletonStressTest {
    ...
    @Test
    void worldSpawnAreaUsesNoDepthBackstopForContinuousVisibleEdges() {
        SkeletonScene scene = SkeletonSceneFixtures.worldSpawnAreaTenK();
        int edgeCount = SkeletonEdgeInspector.actualVisibleEdges(scene.blocks()).size();

        assertTrue(MergedSkeletonRenderer.shouldRenderNoDepthBackstop(edgeCount),
                "large natural range-destroy skeletons need the no-depth backstop so terrain depth does not cut visible edges");
    }
