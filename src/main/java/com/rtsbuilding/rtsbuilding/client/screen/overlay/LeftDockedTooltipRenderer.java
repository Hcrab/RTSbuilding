...
public final class LeftDockedTooltipRenderer {
    ...
    public void render(GuiGraphicsExtractor graphics, ItemStack stack) {
        LeftDockedTooltipLayout.Geometry geometry = geometry();
        graphics.setTooltipForNextFrame(this.screen.font(), stack,
                geometry.anchorX(), geometry.anchorY());
    }

    public void render(GuiGraphicsExtractor graphics, Component text) {
        LeftDockedTooltipLayout.Geometry geometry = geometry();
        graphics.setTooltipForNextFrame(this.screen.font(), text,
                geometry.anchorX(), geometry.anchorY());
    }

    public void renderDetail(GuiGraphicsExtractor graphics, String detail, UiColor color) {
        if (detail == null || detail.isBlank() || color == null) {
            return;
        }
        LeftDockedTooltipLayout.Geometry geometry = geometry();
        graphics.text(this.screen.font(), detail,
