    ...
    public static void render(PoseStack poseStack, VertexConsumer lineBuffer, VertexConsumer fillBuffer,
        ...
        for (RtsCullingBox box : manager.boxes()) {
            ...
            if (box.id() == manager.hoveredId()) {
                renderBox(poseStack, lineBuffer, fillBuffer, renderBox,
                        UiThemeWorldColors.CULLING_HOVER, 0.22F, 0.95F);
            } else if (box.id() == manager.selectedId()) {
                renderBox(poseStack, lineBuffer, fillBuffer, renderBox,
                        UiThemeWorldColors.CULLING_SELECTED, 0.18F, 0.98F);
            } else {
                renderBox(poseStack, lineBuffer, fillBuffer, renderBox,
                        UiThemeWorldColors.CULLING_IDLE, 0.12F, 0.82F);
            }
        }
        ...
        if (preview != null) {
            renderBox(poseStack, lineBuffer, fillBuffer, preview.asAabb(),
                    UiThemeWorldColors.CULLING_HOVER, 0.16F, 0.92F);
        }
    }

    private static void renderBox(PoseStack poseStack, VertexConsumer lineBuffer,
            VertexConsumer fillBuffer, AABB box, UiColor color,
            float fillAlpha, float lineAlpha) {
        renderBox(poseStack, lineBuffer, fillBuffer, box,
                UiThemeWorldColors.red(color), UiThemeWorldColors.green(color),
                UiThemeWorldColors.blue(color), fillAlpha, lineAlpha);
    }
