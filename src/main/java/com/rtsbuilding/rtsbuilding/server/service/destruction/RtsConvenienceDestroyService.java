 ...
 * 便捷破坏的服务端权威边界。
 *
 * <p>请求不携带客户端预览的坐标集合；此处从锚点、工具和受限设置重新规划，再提交到正式 AREA_DESTROY 管道。
 * 它不新建第二套破坏执行器，因此工具借用、分 tick、领地校验、掉落收纳与撤销语义保持一致。</p>
 */
public final class RtsConvenienceDestroyService {
    ...
    public RtsConvenienceDestroyPlanner.Plan submit(
        ...
        if (player == null || !RtsProgressionManager.canUse(player, RtsFeature.AREA_DESTROY)) {
            return invalidPlan();
        }
