...
// ======================== 坐标/向量工具 ========================
    ...
    /** 高级选区几何全部使用有界目标集合；HOLLOW/SKELETON 不扫描完整内部体积。 */
    static void addAdvancedSquareTargets(Set<BlockPos> targets, RtsCullingBox box, ShapeFillMode fillMode) {
        long minX = box.min().getX(), maxX = box.max().getX();
        long minZ = box.min().getZ(), maxZ = box.max().getZ();
        long width = span(toInt(minX), toInt(maxX));
        long depth = span(toInt(minZ), toInt(maxZ));
        long expected = fillMode == ShapeFillMode.FILL
                ? saturatedProduct(width, 1L, depth) : perimeterCount(width, depth);
        if (expected > targetLimit(targets)) throw new ShapeGeometryBudget.LimitExceeded(expected);
        long y = box.min().getY();
    ...
    static void addAdvancedBoxLikeTargets(Set<BlockPos> targets, RtsCullingBox box, ShapeFillMode fillMode) {
        long minX = box.min().getX(), maxX = box.max().getX();
        long minY = box.min().getY(), maxY = box.max().getY();
        long minZ = box.min().getZ(), maxZ = box.max().getZ();
        long width = span(toInt(minX), toInt(maxX));
    ...
    static void addAdvancedWallTargets(Set<BlockPos> targets, RtsCullingBox box, ShapeFillMode fillMode) {
        ...
        long height = span(toInt(minY), toInt(maxY));
        long expected = fillMode == ShapeFillMode.FILL
                ? saturatedProduct(horizontalLength, height, 1L) : perimeterCount(horizontalLength, height);
        if (expected > targetLimit(targets)) throw new ShapeGeometryBudget.LimitExceeded(expected);
        long fixed = useX ? box.min().getZ() : box.min().getX();
        if (fillMode != ShapeFillMode.FILL) {
            for (long h = horizontalMin; h <= horizontalMax; h++) {
                targets.add(useX ? new BlockPos(toInt(h), toInt(minY), toInt(fixed))
                        : new BlockPos(toInt(fixed), toInt(minY), toInt(h)));
                if (maxY != minY) {
                    targets.add(useX ? new BlockPos(toInt(h), toInt(maxY), toInt(fixed))
                            : new BlockPos(toInt(fixed), toInt(maxY), toInt(h)));
                }
            }
            for (long y = minY + 1L; y < maxY; y++) {
                targets.add(useX ? new BlockPos(toInt(horizontalMin), toInt(y), toInt(fixed))
                        : new BlockPos(toInt(fixed), toInt(y), toInt(horizontalMin)));
                if (horizontalMax != horizontalMin) {
                    targets.add(useX ? new BlockPos(toInt(horizontalMax), toInt(y), toInt(fixed))
                            : new BlockPos(toInt(fixed), toInt(y), toInt(horizontalMax)));
                }
            }
            return;
        }
        for (long y = minY; y <= maxY; y++) for (long h = horizontalMin; h <= horizontalMax; h++) {
            targets.add(useX ? new BlockPos(toInt(h), toInt(y), toInt(fixed))
                    : new BlockPos(toInt(fixed), toInt(y), toInt(h)));
        }
    }
    ...
    static void addAdvancedEllipseTargets(Set<BlockPos> targets, RtsCullingBox box, ShapeFillMode fillMode,
        ...
        addEllipseTargetsOnAxes(targets, box, axes[0], axes[1], ShapeGeometryUtil.normalizePlaneFace(planeFace), fillMode);
    }

    static void addEllipseTargetsOnAxes(Set<BlockPos> targets, RtsCullingBox box,
            Direction axisA, Direction axisB, Direction normal, ShapeFillMode fillMode) {
        long minA = minCoordLong(box, axisA.getAxis()), maxA = maxCoordLong(box, axisA.getAxis());
        long minB = minCoordLong(box, axisB.getAxis()), maxB = maxCoordLong(box, axisB.getAxis());
        long fixed = minCoordLong(box, normal.getAxis());
        ...
        if (lowerBound > targetLimit(targets)) throw new ShapeGeometryBudget.LimitExceeded(lowerBound);
        ...
        for (long a = activeA[0]; a <= activeA[1]; a++) {
            ...
            if (fillMode == ShapeFillMode.FILL) {
                for (long b = interval[0]; b <= interval[1]; b++) {
                    targets.add(positionAtAxes(box, axisA, a, axisB, b, normal, fixed));
                }
            } else {
                addEllipseBoundaryRow(targets, box, axisA, axisB, normal, fixed,
    ...
    static void addAdvancedEllipticCylinderTargets(Set<BlockPos> targets, RtsCullingBox box,
        ...
        long normalMin = minCoordLong(box, normal.getAxis()), normalMax = maxCoordLong(box, normal.getAxis());
        long minA = minCoordLong(box, axes[0].getAxis()), maxA = maxCoordLong(box, axes[0].getAxis());
        long minB = minCoordLong(box, axes[1].getAxis()), maxB = maxCoordLong(box, axes[1].getAxis());
    ...
    static void addEllipseBoundaryRow(
            Set<BlockPos> targets, RtsCullingBox box,
            Direction axisA, Direction axisB, Direction normal, long normalCoordinate,
            long a, long[] interval, long minA, long maxA, long minB, long maxB) {
        long lower = interval[0], upper = interval[1];
    ...
    static void addEllipsoidBoundaryRow(
            Set<BlockPos> targets,
            long x, long y, long[] interval,
            long[] previousX, long[] nextX, long[] previousY, long[] nextY) {
        long lower = interval[0], upper = interval[1];
    ...
    static void addAdvancedLineTargets(Set<BlockPos> targets, long min, long max,
            long fixedA, long fixedB, ShapeFillMode fillMode, Direction.Axis axis) {
        long length = span(toInt(min), toInt(max));
          long expected = length;
        if (expected > targetLimit(targets)) throw new ShapeGeometryBudget.LimitExceeded(expected);
          for (long value = min; value <= max; value++) {
            targets.add(axis == Direction.Axis.X ? new BlockPos(toInt(value), toInt(fixedA), toInt(fixedB))
                    : axis == Direction.Axis.Y ? new BlockPos(toInt(fixedA), toInt(value), toInt(fixedB))
                    : new BlockPos(toInt(fixedA), toInt(fixedB), toInt(value)));
        }
    }

    static void addAbsoluteRectangleBoundary(Set<BlockPos> targets,
            long minX, long maxX, long minZ, long maxZ, long y) {
        for (long x = minX; x <= maxX; x++) {
            targets.add(new BlockPos(toInt(x), toInt(y), toInt(minZ)));
            if (maxZ != minZ) targets.add(new BlockPos(toInt(x), toInt(y), toInt(maxZ)));
        }
        for (long z = minZ + 1L; z < maxZ; z++) {
            targets.add(new BlockPos(toInt(minX), toInt(y), toInt(z)));
            if (maxX != minX) targets.add(new BlockPos(toInt(maxX), toInt(y), toInt(z)));
        }
    }

    static void addAbsoluteHollowLayer(Set<BlockPos> targets,
            long minX, long maxX, long minZ, long maxZ, long y, boolean yBoundary) {
        if (yBoundary) {
            for (long x = minX; x <= maxX; x++) for (long z = minZ; z <= maxZ; z++)
                targets.add(new BlockPos(toInt(x), toInt(y), toInt(z)));
            return;
    ...
    static void addAbsoluteSkeletonLayer(Set<BlockPos> targets,
            long minX, long maxX, long minZ, long maxZ, long y, boolean yBoundary) {
        if (yBoundary) {
            addAbsoluteRectangleBoundary(targets, minX, maxX, minZ, maxZ, y);
            return;
        }
        targets.add(new BlockPos(toInt(minX), toInt(y), toInt(minZ)));
        targets.add(new BlockPos(toInt(minX), toInt(y), toInt(maxZ)));
        targets.add(new BlockPos(toInt(maxX), toInt(y), toInt(minZ)));
        targets.add(new BlockPos(toInt(maxX), toInt(y), toInt(maxZ)));
    }

    /** 椭圆填充目标的保守下界：中心半轴矩形中的单元一定在椭圆内。 */
    static long ellipseInteriorLowerBound(long width, long height) {
        if (width <= 0L || height <= 0L) return 0L;
        if (width == 1L) return height;
        if (height == 1L) return width;
    ...
    static long ellipsoidInteriorLowerBound(long width, long height, long depth) {
        if (width <= 0L || height <= 0L || depth <= 0L) return 0L;
        if (width == 1L) return ellipseInteriorLowerBound(height, depth);
        if (height == 1L) return ellipseInteriorLowerBound(width, depth);
        if (depth == 1L) return ellipseInteriorLowerBound(width, height);
        return Math.max(Math.max(width, Math.max(height, depth)),
                saturatedProduct(Math.max(1L, width / 2L),
                        Math.max(1L, height / 2L), Math.max(1L, depth / 2L)));
    }
    ...
    /** 椭球边界至少覆盖三个轴中最长轴数量的格。 */
      ...
      static long[] axisIntervalForRemaining(long min, long max, double remaining) {
          if (remaining < -1.0E-9D) return null;
          if (min >= max) return new long[] {min, max};
    ...
    static long[] ellipseRowInterval(long a, long minA, long maxA, long minB, long maxB) {
        ...
        if (remaining < -1.0E-9D) return null;
        if (minB >= maxB) return new long[] {minB, maxB};
        double center = ((double) minB + maxB + 1.0D) * 0.5D;
        double radius = ((double) maxB - minB + 1.0D) * 0.5D;
        double extent = Math.sqrt(Math.max(0.0D, remaining)) * radius;
    ...
    static boolean insideEllipseCoordinate(long a, long b,
            long minA, long maxA, long minB, long maxB) {
        return normalizedCellDistance(a, minA, maxA) + normalizedCellDistance(b, minB, maxB) <= 1.0D;
    }

    static long[] ellipsoidRowInterval(double remaining, long minZ, long maxZ) {
        if (remaining < -1.0E-9D) return null;
        if (minZ >= maxZ) return new long[] {minZ, maxZ};
        double center = ((double) minZ + maxZ + 1.0D) * 0.5D;
        double radius = ((double) maxZ - minZ + 1.0D) * 0.5D;
        double extent = Math.sqrt(Math.max(0.0D, remaining)) * radius;
    ...
    static boolean insideEllipsoidCoordinate(long z, double remaining, long minZ, long maxZ) {
        return remaining >= -1.0E-9D && normalizedCellDistance(z, minZ, maxZ) <= remaining + 1.0E-9D;
    }

    static BlockPos positionAtAxes(RtsCullingBox box, Direction axisA, long a,
            Direction axisB, long b, Direction normal, long normalCoordinate) {
        long[] coordinates = {box.min().getX(), box.min().getY(), box.min().getZ()};
    ...
    static long minCoordLong(RtsCullingBox box, Direction.Axis axis) {
        return switch (axis) { case X -> box.min().getX(); case Y -> box.min().getY(); case Z -> box.min().getZ(); };
    }

    static long maxCoordLong(RtsCullingBox box, Direction.Axis axis) {
        return switch (axis) { case X -> box.max().getX(); case Y -> box.max().getY(); case Z -> box.max().getZ(); };
    }
