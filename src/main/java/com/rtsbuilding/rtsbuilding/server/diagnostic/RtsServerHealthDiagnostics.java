 ...
 * 低成本服务端 tick 健康采样器。
 *
 * <p>这里只陈述墙钟间隔与 RTS 已测调度量，不做 CPU 归因，也不改变任务预算。</p>
 */
public final class RtsServerHealthDiagnostics {
    ...
    public static synchronized void beginTick(MinecraftServer server) {
        ...
        currentServerTick = gameTime(server);
        if (lastTickNanos != 0L) {
    ...
    public static synchronized void completeTick(
            ...
            RtsTaskEngine.QueueDiagnostics queue) {
        if (level() == RtsDiagnosticLevel.OFF || server == null || stats == null || queue == null) {
            return;
        }
        long tick = gameTime(server);
        boolean summaryDue = queue.runnable() + queue.waiting() > 0
                && tick - lastSummaryTick >= SUMMARY_INTERVAL_TICKS;
        ...
        String message = "[RTS-SERVER-HEALTH] schema=2 run={} event={} server_tick={} "
                + "gap_ms={} severity={} rts_runnable={} rts_waiting={} "
                + "rts_processed_units={} rts_slice_nanos={} persistence_dirty={} "
                + "persistence_inflight={} persistence_asset_pending={}";
        Object[] values = {
                RtsTraceIds.runId(), event, tick, gapMs,
                pendingGapMs > 0L ? severity : "NORMAL",
                queue.runnable(), queue.waiting(), stats.processedUnits(), stats.elapsedNanos(),
                persistence.dirty(), persistence.inFlight(), persistence.pendingAssetAdmissions()
        };
        if (pendingGapMs > 0L) {
            RtsbuildingMod.LOGGER.warn(message, values);
        } else {
            RtsbuildingMod.LOGGER.info(message, values);
        }
        RtsStructuredDiagnostics.appendServer(
                ...
                "server_tick", tick,
                "gap_ms", gapMs,
                "severity", pendingGapMs > 0L ? severity : "NORMAL",
                "rts_runnable", queue.runnable(),
                "rts_waiting", queue.waiting(),
                "rts_processed_units", stats.processedUnits(),
                "rts_slice_nanos", stats.elapsedNanos(),
                "persistence_dirty", persistence.dirty(),
                "persistence_inflight", persistence.inFlight(),
                "persistence_asset_pending", persistence.pendingAssetAdmissions());
        pendingGapMs = 0L;
