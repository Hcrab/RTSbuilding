...
final class RtsGuiScaleCoordinator {
    ...
    boolean renderScaled(
        ...
        graphics.pose().scale((float) frame.scale(), (float) frame.scale());
        try {
            renderer.render(
                    graphics,
    ...
    int toNativeGuiCoordinate(int coordinate) {
        double scale = this.renderPass ? this.activeRenderScale : 1.0D;
        return (int) Math.round(coordinate * (Double.isFinite(scale) && scale > 0.0D ? scale : 1.0D));
    }
