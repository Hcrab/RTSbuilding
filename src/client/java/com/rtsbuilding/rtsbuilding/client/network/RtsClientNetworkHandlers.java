...
public final class RtsClientNetworkHandlers {
  ...
  public static void handleCameraState(
      S2CRtsCameraStatePayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyServerCameraState(payload));
  }

  public static void handleCameraAnchor(
      S2CRtsCameraAnchorPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyServerCameraAnchor(payload));
  }

  public static void handleStoragePage(
      S2CRtsStoragePagePayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
          ClientRtsController.get().applyStoragePage(payload);
          RtsDeveloperScenarioTracker.getInstance()
              .record("storage_page_received", "page=" + payload.page());
        });
  }

  public static void handleStorageDirty(
      S2CRtsStorageDirtyPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyStorageDirty(payload));
  }

  public static void handleRemoteMenuHint(
      S2CRtsRemoteMenuHintPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyRemoteMenuHint(payload));
  }

  public static void handleCraftables(S2CRtsCraftablesPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyCraftables(payload));
  }

  public static void handleCraftFeedback(
      S2CRtsCraftFeedbackPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyCraftFeedback(payload));
  }

  public static void handleCullingState(
      S2CRtsCullingStatePayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> RtsCullingClientState.applyCurrentWorldState(payload));
  }

  public static void handleDamageFeedback(
      S2CRtsDamageFeedbackPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyDamageFeedback(payload));
  }

  public static void handleQuestDetectStatus(
      S2CRtsQuestDetectStatusPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyQuestDetectStatus(payload));
  }

  public static void handleMineProgress(
      S2CRtsMineProgressPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyMineProgress(payload));
  }

  public static void handleUltimineProgress(
      S2CRtsUltimineProgressPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyUltimineProgress(payload));
  }

  public static void handleOperationTerminal(
      S2CRtsOperationTerminalPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> RtsClientOperationDiagnostics.serverTerminal(payload));
  }

  public static void handleHarvestTierSkipped(
      S2CRtsHarvestTierSkippedPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
  ...
  public static void handlePlaceAnimation(
      S2CRtsPlaceAnimationPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
          PlacementAnimationRenderer.confirmPlacement(payload.pos(), payload.state());
          RtsDeveloperScenarioTracker.getInstance()
              .record("place_confirmed", "pos=" + payload.pos().toShortString());
        });
  }

  public static void handleBreakAnimation(
      S2CRtsBreakAnimationPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
          ClientFakeAirBlocks.hideUntilServerState(
              payload.pos(), payload.state(), payload.resultState());
          PlacementAnimationRenderer.addDestroy(payload.pos(), payload.state());
          ShapeGhostRenderer.markDestroyed(payload.pos());
          RtsDeveloperScenarioTracker.getInstance()
              .record("break_confirmed", "pos=" + payload.pos().toShortString());
        });
  }

  public static void handleBlockActionSound(
      S2CRtsBlockActionSoundPayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> RtsBlockActionSoundPlayer.play(payload));
  }

  public static void handleProgressionState(
      S2CRtsProgressionStatePayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyProgressionState(payload));
  }

  public static void handlePluginState(
      S2CRtsPluginStatePayload payload, RtsPayloadContext context) {
    context.enqueueWork(() -> ClientRtsController.get().applyPluginState(payload));
  }

  public static void handleHistorySync(
      S2CRtsHistorySyncPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> PlacementHistoryManager.syncHistoryState(payload.undoSize(), payload.redoSize()));
  }

  public static void handleWorkflowProgress(
      S2CRtsWorkflowProgressPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
          ClientRtsController.get().applyWorkflowProgress(payload);
          RtsDeveloperScenarioTracker.getInstance()
              .record(
                  "workflow_update_received",
                  "completed="
  ...
  public static void handleWorkflowProgressBatch(
      S2CRtsWorkflowProgressBatchPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
          ClientRtsController.get().applyWorkflowProgressBatch(payload);
          RtsDeveloperScenarioTracker.getInstance()
              .record("workflow_update_received", "entries=" + payload.entries().size());
        });
  }

  public static void handleResumePlacementScan(
      S2CRtsResumePlacementScanPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
  ...
  public static void handleBlueprintResumeScan(
      S2CRtsBlueprintResumeScanPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> {
          if (Minecraft.getInstance().screen instanceof BuilderScreen bs) {
  ...
  public static void handleBlueprintStatus(
      S2CBlueprintStatusPayload payload, RtsPayloadContext context) {
    context.enqueueWork(
        () -> BlueprintPanel.setStatus(payload.status(), payload.messageKey(), payload.detail()));
  }
}
