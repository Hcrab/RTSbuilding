...
public final class RtsPersistenceDiagnostics {
    ...
    public static void saveScheduled(
        ...
        event(false, false, "PERSIST_SAVE_SCHEDULED",
                ...
                "estimated_bytes", Math.max(0L, estimatedBytes),
    ...
    private static void event(boolean latestLog, boolean warning, String name, Object... fields) {
        try {
            ...
            if (latestLog) {
                ...
                if (warning) {
                    RtsbuildingMod.LOGGER.warn(message, RtsTraceIds.runId(), name, suffix);
                } else {
                    RtsbuildingMod.LOGGER.info(message, RtsTraceIds.runId(), name, suffix);
                }
            }
            ...
            if (fields != null) {
                System.arraycopy(fields, 0, structured, 2, fields.length);
            }
