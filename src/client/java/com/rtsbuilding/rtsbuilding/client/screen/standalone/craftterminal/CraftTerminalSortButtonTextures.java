 ...
 * 合成终端排序按钮的双轨纹理目录。
 *
 * <p>四个语义各自对应一张已经离线合成完毕的 24×24 PNG。生产端只选择完整纹理并 以 1:1 绘制；不会在运行时拼字符、叠图标或缩放素材。Palette 只替换五种精确 索引色，不改变
 * v2 图标的像素排布。
 */
final class CraftTerminalSortButtonTextures {
  ...
  private static ResourceLocation resolve(Entry entry, UiTextureState state) {
    return ThemedStateTextureResolver.resolve(
        entry.legacy, entry.source, state, UiIndexedTextureSpec.V2_TERMINAL_SORT_BUTTON);
  }

  private static Entry entry(String name) {
    ResourceLocation source =
        ResourceLocation.fromNamespaceAndPath("rtsbuilding", "textures/gui/ui/" + name);
