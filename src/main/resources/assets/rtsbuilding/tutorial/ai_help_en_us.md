...
- RTS Camera: RTS view, basic interaction, and a base camera view radius of 16 blocks.
...
- Range I / II / III provide 16 / 32 / 48 blocks of camera view radius; Maximum Range uses the server's maximum allowed camera radius.
...
## RTS Home and camera view range
...
After RTS opens, the camera view area is centered on the player block where that session opened. The radius extends in both directions on X and Z, so the boundary is a square, not a circle. Home decides where RTS may open; it is not intersected with the session camera square. Reopen RTS elsewhere to move the action center. This boundary only limits camera movement and visibility: a remote operation that has a valid RTS session, target dimension and chunk, permission, and legal payload is not rejected only for being outside this view radius.
...
### NeoForge 26.1 adapter note

Use the early 26.1 compatibility wiring and verify actual packet/task adapter names; 1.21.1 APIs are not interchangeable.
...
`diagnostics.level` defaults to `BASIC`, which records bounded lifecycle events and percentage progress checkpoints. `VERBOSE` also permits one task-progress sample per second; `OFF` disables these diagnostics. Common rejection reasons include `FEATURE_LOCKED`, `HARVEST_TIER_TOO_LOW`, `TOOL_CANNOT_HARVEST`, `TARGET_CHUNK_UNAVAILABLE`, and `CLAIM_DENIED`. Supply both client and server logs for the reproduction window. CPU attribution still requires Spark, JFR, or another profiler.

## Stable responsibility chains

### Remote interaction

The client ray selects a block, face, or entity. The server recreates a bounded remote interaction context and invokes normal block/item interaction behavior. Claim protection, the active RTS session, target dimension/chunk, and permission still apply; camera view range is visual only. Shift-natural interaction and ordinary RTS interaction are distinct intents; troubleshooting should record the actual input used.

### Placement and batch building

The server validates the RTS session, target dimension/chunk, progression/plugins, claims, and placement legality, then obtains the real `ItemStack` from inventory or linked storage. Capability, NBT, durability, and energy mutations must remain on the real extracted stack. Large shapes become task slices. Only confirmed changes trigger placement animation and material/storage refresh.

### Single, chain, and area mining

Targets pass active-session/target-chunk validation, soft-block classification, plugin harvest tier, real-tool suitability, tool protection, claim, and recovery rules. Chain requests obey target-count and per-tick limits; area requests additionally obey per-axis and total-volume limits.
...
|---|---:|---|
| `enableSurvivalProgression` | `false` | Enables plugins, survival progression, and RTS-home opening gates. |
| `shareSurvivalProgressionWithTeams` | `false` | Shares home/plugins via FTB Teams, OpenPAC party, or scoreboard team. |
| `maxActionRadiusBlocks` | `128` (48–512) | Final server ceiling for the RTS camera view radius; not a server-side remote-operation rejection gate. |
...
| Preview appears, confirm does nothing | Confirm key, packet, session/dimension/chunk, plugin, claim, task admission | Press configured confirm key; check toast/actionbar and whether `latest.log` contains a request/workflow. |
...
## Version-specific technical appendix: NeoForge 26.1

The release-preparation version for this line is `0.0.2`; the latest release is `0.0.1`. This is an early experimental branch. If the test build is unstable, return to `0.0.1` or restore a backup before reporting the issue.

### Task cancellation and diagnosis

A new mining replacement action must cancel non-terminal durable `MINING` and area-destruction `DESTRUCTION` tasks for the same player and dimension through `RtsTaskEngine.cancelWorkflowTask(...)`. That path writes `CANCELLED`, releases leases, preserves cursor/succeeded, writes a tombstone, and closes the workflow. Without a replacement action, `WAITING_RESOURCE` may continue waiting; unrelated PLACE, QUICK_BUILD, and BLUEPRINT_BUILD tasks are unaffected.

### Log-analysis quick reference

Use the last 200 lines of `latest.log` and the latest 200 RTSBuilding records included in the prompt. Find the earliest abnormal event, correlate `trace` across `seq/op/workflow/task`, and check `[RTS-TRACE]`, `[RTS-DIAG]`, `[RTS-SERVER-HEALTH]`, `BEGIN/RESULT/FILTER/TERMINAL`, and the first relevant stack trace. Separate input/network, server rejection, resource waiting, terminal state, and cascade errors; mark missing evidence as unknown.

### NeoForge 26.1 differences

This branch uses the NeoForge 26.1 API and its own early compatibility wiring, so conclusions from the 1.21.1 mainline must not be copied blindly. Confirm the actual loader/mod versions and the server durable snapshot; a restored client UI does not prove that a durable task terminated.
