...
public final class RtsAiChatSession {
        ...
        this.activeRequest = this.client.ask(prompt,
                ...
                error -> runOnClient(generation, () -> {
                    ...
                    this.notices.add(new Notice(Component.translatable(
                            "screen.rtsbuilding.ai_chat.error", error).getString(), 0xFFFF8D8D));
                }),
                () -> runOnClient(generation, () -> {
                    String answer = this.streamingAnswer.toString().strip();
                    if (!answer.isEmpty()) {
                        this.conversation.add(this.pendingQuestion, answer);
                    } else {
                        this.notices.add(new Notice(Component.translatable(
                                "screen.rtsbuilding.ai_chat.empty").getString(), 0xFFFFC46A));
                    }
                    this.pendingQuestion = "";
                    this.streamingAnswer.setLength(0);
                    this.waiting = false;
    ...
    public void reset() {
        ...
        this.streamingAnswer.setLength(0);
        this.notices.add(new Notice(Component.translatable(
                "screen.rtsbuilding.ai_chat.cleared").getString(), 0xFF8ED6A7));
    }
    ...
    public String streamingAnswer() {
        return this.streamingAnswer.toString();
    }
