    ...
    static WorkflowUiTransition dispatch(ClientRtsController controller,
        ...
        switch (transition.command) {
            case TOGGLE_PROTECTED:
                ...
                if (row != null) {
                    ClientPlayNetworking.send(new C2SRtsSetWorkflowProtectedPayload(
                            row.entryId, !row.protectedWorkflow));
                }
                break;
            case TOGGLE_PAUSED:
                ClientPlayNetworking.send(new C2SRtsPauseWorkflowPayload(action.entryId));
                break;
            case SCAN_RESUME_PLACEMENT:
                ClientPlayNetworking.send(new C2SRtsScanResumePlacementPayload(action.entryId));
                break;
            case SCAN_RESUME_BLUEPRINT:
                ClientPlayNetworking.send(new C2SRtsScanBlueprintResumePayload(action.entryId));
                break;
            case DELETE:
                ClientPlayNetworking.send(new C2SRtsDeleteWorkflowPayload(action.entryId));
                break;
            default:
                break;
        }
