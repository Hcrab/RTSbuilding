 ...
 *
 * <p>它只保存玩家的点选意图和预览状态；候选储存端点由服务端在确认后重新扫描，客户端不会
 * 把自己发现的坐标列表当成权威结果。确认后保持批量模式，方便连续框选；Ctrl 或 Esc 可退出。</p>
 */
public final class StorageBatchSelectionSession {
    ...
    public boolean confirm(Minecraft minecraft) {
        ...
        RtsClientPacketGateway.sendBatchLinkStorage(first, second, true);
