...
/**
 * schemaVersion 1 的严格 Palette 主题 JSON 编解码器。
 *
 * <p>缺项、未知项、错误颜色、伪造 Legacy、过深或过大的输入全部硬失败。该类只处理内存字符串，
 * 文件选择与原子写入由 {@link UiThemeStorage} 负责。</p>
 */
public final class UiThemeJsonCodec {
    ...
    private static final Set<String> ROOT_FIELDS = set(
            "schemaVersion", "id", "name", "author", "description",
            "renderMode", "textureSet", "tokens", "components");
    ...
    public UiThemeDefinition decode(String json) {
        ...
        JsonElement parsed;
        try {
            // Forge 1.12 bundled Gson 使用实例 parse；不要引入较新 Gson 的静态 API。
        ...
        String mode = requiredText(root, "renderMode", 32);
        if (!"palette".equals(mode)) {
            throw new IllegalArgumentException("user theme renderMode must be palette");
        }
        String name = requiredText(root, "name", MAX_SHORT_TEXT);
        String author = requiredText(root, "author", MAX_SHORT_TEXT);
        String description = requiredText(root, "description", MAX_DESCRIPTION);
        String textureSet = requiredText(root, "textureSet", MAX_SHORT_TEXT);
        ...
        UiThemeDefinition definition = new UiThemeDefinition(
                id, name, author, description, UiThemeRenderMode.PALETTE,
                textureSet, true, tokens, components);
    ...
    public String encode(UiThemeDefinition definition) {
        ...
        if (definition.renderMode() != UiThemeRenderMode.PALETTE) {
            throw new IllegalArgumentException("Legacy Direct is not exported as Palette JSON");
        }
        ...
        for (UiThemeCoverageCatalog.ComponentFamily family
            ...
            for (UiThemeToken token : UiThemeCoverageCatalog.required(family)) {
                colors.addProperty(token.serializedId(),
                        color(definition.componentColor(family, token)));
            }
    ...
    private static EnumMap<UiThemeToken, UiColor> readTokens(JsonObject object) {
        Set<String> expected = UiThemeToken.serializedCatalog().keySet();
        requireExactFields(object, expected, "tokens");
        EnumMap<UiThemeToken, UiColor> tokens = new EnumMap<UiThemeToken, UiColor>(UiThemeToken.class);
        for (UiThemeToken token : UiThemeToken.values()) {
            tokens.put(token, parseColor(requiredText(object, token.serializedId(), 9),
    ...
    readComponents(JsonObject object, Map<UiThemeToken, UiColor> tokens) {
        ...
        for (UiThemeCoverageCatalog.ComponentFamily family
            ...
            for (UiThemeToken token : UiThemeCoverageCatalog.required(family)) {
                expected.add(token.serializedId());
            }
            requireExactFields(colors, expected, "components." + familyId);
            EnumMap<UiThemeToken, UiColor> resolved = new EnumMap<UiThemeToken, UiColor>(UiThemeToken.class);
            for (UiThemeToken token : UiThemeCoverageCatalog.required(family)) {
                ...
                if (value.startsWith("$")) {
                    ...
                    if (reference == null) throw new IllegalArgumentException(
                            path + " references unknown core token: " + value);
    ...
    private static UiColor parseColor(String value, String path) {
        if (!COLOR.matcher(value).matches()) {
            throw new IllegalArgumentException(path + " must be #AARRGGBB");
        }
        long parsed = Long.parseUnsignedLong(value.substring(1), 16);
        return new UiColor((int) parsed);
    }
    ...
    private static void requireExactFields(JsonObject object, Set<String> expected, String path) {
        ...
        if (!actual.equals(expected)) {
            ...
            Set<String> unknown = new HashSet<String>(actual);
            unknown.removeAll(expected);
            throw new IllegalArgumentException(path + " fields mismatch; missing=" + missing
                    + ", unknown=" + unknown);
        }
    }
    ...
    private static void requireDepth(JsonElement element, int depth) {
        ...
        if (element.isJsonObject()) {
            for (Map.Entry<String, JsonElement> entry : element.getAsJsonObject().entrySet()) {
                requireDepth(entry.getValue(), depth + 1);
            }
