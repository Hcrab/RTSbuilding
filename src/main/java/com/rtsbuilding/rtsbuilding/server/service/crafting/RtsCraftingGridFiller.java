 ...
 *
 * <ul>
 *   <li><b>蓝图填充</b>（{@link #refillCraftGridFromLinked}）— 根据预定义的物品蓝图， 从链接存储逐槽填充合成网格，支持单次填充和多次堆叠填充（最多
 *       64 轮）
 *   <li><b>网络包填充</b>（{@link #refillCurrentCraftGridFromBlueprintIds} / {@link
 *       #refillCurrentCraftGridFromBlueprintStacks}）— 从客户端发送的物品 ID 或物品原型栈列表填充当前合成网格
 *   <li><b>JEI 一键填充</b>（{@link #applyJeiTransfer}）— 支持 JEI 配方传输集成， 可清除现有网格、首选原型匹配、多次堆叠填充
 * </ul>
 *
 * <p>填充时优先匹配精确原型，回退到任意匹配的材料。 若网格中已有物品，会自动检测堆叠上限并尝试增量填充。
 */
public final class RtsCraftingGridFiller {
  ...
  public static void refillCraftGridFromLinked(
      ...
      AbstractContainerMenu craftingMenu,
      ItemStack[] blueprint) {
  ...
  public static void refillCurrentCraftGridFromBlueprintIds(
    ...
    if (session != null && craftedItemId != null && !craftedItemId.isBlank() && craftedCount > 0) {
      ServiceRegistry.getInstance()
          .page()
          .recordRecentItem(
              session, craftedItemId, S2CRtsStoragePagePayload.RECENT_ITEM_CRAFTED, craftedCount);
  ...
  public static void applyJeiTransfer(
    ...
    CraftIngredientPlan plannedFallback =
        RtsCraftingAvailability.resolveCraftIngredientPlan(
            craftingRecipe,
            RtsCraftingAvailability.snapshotAvailable(player, extractHandlers, true));
    ...
    for (int pass = 0; pass < maxPasses; pass++) {
      ...
      for (int i = 0; i < 9; i++) {
        ...
        if (!existing.isEmpty()) {
          ...
          ItemStack extracted =
              RtsCraftingExecutor.extractOneMatchingIngredientCombined(
  ...
  public static void refillCraftGridFromBlueprint(
    ...
    refillCraftGridFromBlueprint(
        menu, handlers, player, blueprint, null, fillAll, includePlayerFallback);
  }
    ...
    for (int pass = 0; pass < maxPasses; pass++) {
      ...
      for (int i = 0; i < 9; i++) {
        ...
        Ingredient ingredient =
            ingredients != null && i < ingredients.length ? ingredients[i] : Ingredient.EMPTY;
        ...
        if (!current.isEmpty()) {
          if (hasIngredient
              ...
              : !ItemStack.isSameItemSameComponents(current, blueprintStack)) {
            continue;
          }
          ...
          ItemStack extracted =
              includePlayerFallback
                  ? RtsTransferExtractor.extractOneMatchingPrototypeCombined(
                      handlers, player, current)
  ...
  private static ItemStack extractCraftGridRefillStack(
    ...
    if (hasIngredient) {
      ItemStack extracted =
          includePlayerFallback
              ? RtsCraftingExecutor.extractOneMatchingIngredientCombined(
                  handlers, player, ingredient, preferred)
   ...
   *
   * <p>普通材料从 10 变 9 时只补 1；原样留在槽中的催化剂从 1 仍为 1 时不补。 这避免把未消耗工具越补越多，同时让 Shift 合成后也能恢复点击前的堆叠数量。
   */
  ...
  /** 把桶、瓶等占住原材料槽的配方剩余物移到玩家背包，再回退到 linked storage。 两处都没有容量时，剩余物留在原合成槽，宁可暂时无法补料也绝不丢失。 */
  ...
  /** 安全清空终端合成格。首选目标放不下时尝试另一个目标；两边都满的剩余物 会原样留在槽中，因此该操作不会通过丢弃或客户端预测造成物品损失。 */
  ...
  private static ItemStack[] sanitizeIngredientPrototypes(
      Ingredient[] required, List<ItemStack> prototypes) {
