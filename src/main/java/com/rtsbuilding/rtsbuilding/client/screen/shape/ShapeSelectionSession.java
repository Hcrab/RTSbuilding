 ...
 *
 * <p>本类不生成方块集合、不发送网络请求，也不决定放置或破坏；这些副作用仍由顶层控制器
...
public final class ShapeSelectionSession {
    ...
    public ShapeBuildTypes.Session current() { return this.session; }
    public void replace(ShapeBuildTypes.Session session) { this.session = session; }
    public BlockHitResult templateHit() { return this.templateHit; }
    ...
    public boolean shouldSubmitAfterSelection(boolean keyboardConfirmEnabled) {
        return this.session != null
                && ShapeConfirmationPolicy.shouldSubmitAfterSelection(
                        keyboardConfirmEnabled, this.session.phase());
    }
    ...
    private Direction resolveBuildFace(BuildShape shape, Direction clickedFace, Vec3 rayDir) {
        ...
        if (this.screen == null || !this.screen.isRoundShapeVertical(shape)) return Direction.UP;
        if (rayDir != null && (Math.abs(rayDir.x) > 1.0E-5D || Math.abs(rayDir.z) > 1.0E-5D)) {
            ...
            if (nearest.getAxis() != Direction.Axis.Y) return nearest;
        }
        return clickedFace != null && clickedFace.getAxis() != Direction.Axis.Y
                ? clickedFace : Direction.NORTH;
    }

    private void advanceLine(BlockHitResult hit) {
        if (this.session.phase() != ShapeBuildTypes.Phase.NEED_SECOND_POINT) return;
        ...
        ShapeBuildTypes.Session ready = new ShapeBuildTypes.Session(
                ...
                this.boxes.isAdvanced(base.shape()) && pointB != null
                        ? AdvancedShapeSelectionGeometry.initialHeightOffset(
                                base.shape(), base.pointA(), pointB) : 0,
                mouseY);
        return this.boxes.isAdvanced(base.shape())
                ? AdvancedShapeSelectionGeometry.sessionFromBox(
                        ready,
                        this.boxes.clamp(
                                AdvancedShapeSelectionGeometry.initialBox(ready), base.pointA()))
                : ready;
    }

    private BlockPos resolveAdvancedSecondPoint(ShapeBuildTypes.Session base, BlockHitResult hit) {
        if (hit == null) return resolvePlanePoint(base, null);
    ...
    public ShapeBuildTypes.Input resolveInput(
            ...
            BuildShape currentShape, boolean lineConnected) {
        if (this.session == null || this.session.shape() != currentShape) return null;
        Minecraft mc = this.screen.getMinecraft();
        Vec3 rayOrigin = mc != null && mc.gameRenderer != null
                ? mc.gameRenderer.getMainCamera().getPosition() : null;
    ...
    public boolean isAwaiting(BuildShape currentShape) {
        return currentShape != BuildShape.BLOCK
                && this.session != null && this.session.shape() == currentShape
    ...
    public boolean adjustDimension(int delta, boolean secondary, boolean height) {
        if (delta == 0 || this.session == null) return false;
        if (height && supportsHeight(this.session.shape())) return adjustHeight(delta);
        if (this.session.shape() == BuildShape.BLOCK
                ...
                && this.session.phase() != ShapeBuildTypes.Phase.READY_CONFIRM)) return false;
        if (secondary) {
    ...
    public boolean canAdjustHeight(BuildShape currentShape) {
        return this.session != null && this.session.shape() == currentShape
    ...
    public boolean adjustHeight(int delta) {
        if (delta == 0 || this.session == null || !supportsHeight(this.session.shape())) return false;
        if (this.session.shape() == BuildShape.LINE && !isVerticalLine(BuildShape.LINE)) return false;
        if ((this.session.shape() == BuildShape.BOX || this.session.shape() == BuildShape.CYLINDER
                || this.session.shape() == BuildShape.WALL)
                && this.session.phase() != ShapeBuildTypes.Phase.NEED_THIRD_POINT) return false;
    ...
    private boolean isVerticalLine(BuildShape shape) {
        return shape == BuildShape.LINE && this.screen != null
