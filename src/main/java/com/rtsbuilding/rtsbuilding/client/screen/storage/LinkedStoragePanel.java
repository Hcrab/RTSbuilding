...
/**
 * 查看、编辑和解绑 RTS 已绑定仓库的浮动窗口。
 *
 * <p>窗口拥有文本焦点、滚动和平台命令；Core 提供有界可见状态，Kit 提供行、控件和
 * 滚动条几何与 chrome。真实 ItemStack、BlockPos、EditBox 和网络副作用不会进入 Core/Kit。</p>
 */
public final class LinkedStoragePanel extends RtsWindowPanel {
    ...
    public void openNear(int anchorX, int anchorY) {
        if (!hasUserBoundsPreference()) {
            ...
            int y = Mth.clamp(anchorY, TOP_H + 2,
                    Math.max(TOP_H + 2, this.screen.getBottomY() - PANEL_H - 4));
    ...
    protected void renderContent(
        ...
        StorageWindowLayout.Geometry geometry = storageGeometry(state);

        LinkedStoragePanelRenderer.renderHeader(graphics, this.screen.font(), geometry);
        if (state.status != StorageUiStatus.READY) {
            LinkedStoragePanelRenderer.renderStatus(graphics, this.screen.font(), geometry, state);
            return;
        }

        MinecraftUiCanvas canvas = new MinecraftUiCanvas(
                graphics, this.screen.font(), this.screen);
        for (int rowIndex = 0; rowIndex < geometry.rows.size(); rowIndex++) {
            int platformIndex = state.scroll + rowIndex;
            if (platformIndex < 0 || platformIndex >= entries.size()) {
                break;
            }
            LinkedStorageEntry platformEntry = entries.get(platformIndex);
            StorageUiEntry coreEntry = state.visibleEntry(rowIndex);
            if (platformEntry == null || coreEntry == null) {
                continue;
            }
            boolean priorityEditing = isEditingPriority(
                    platformEntry.dimensionId(), platformEntry.pos());
            StorageWindowLayout.RowGeometry rowGeometry = geometry.rows.get(rowIndex);
            double rowHover = animateContentControl("storage.row." + rowIndex, true,
                    rowGeometry.row.contains(mouseX, mouseY), false).hover();
            double priorityHover = animateContentControl("storage.priority." + rowIndex,
                    !priorityEditing, !priorityEditing
                            && rowGeometry.priority.contains(mouseX, mouseY),
                    priorityEditing).hover();
            double extractHover = animateContentControl("storage.extract." + rowIndex, true,
                    rowGeometry.extract.contains(mouseX, mouseY),
                    coreEntry.extractOnly).hover();
            double unlinkHover = animateContentControl("storage.unlink." + rowIndex, true,
                    rowGeometry.unlink.contains(mouseX, mouseY), false).hover();
            LinkedStoragePanelRenderer.renderRow(graphics, this.screen.font(), canvas,
                    rowGeometry, platformEntry, coreEntry, priorityEditing, rowHover,
                    priorityHover, extractHover, unlinkHover);
            if (priorityEditing) {
                placePriorityInput(rowGeometry);
                this.priorityInput.render(graphics, mouseX, mouseY, partialTick);
            }
        }
        StorageWindowChromeRenderer.renderScrollbar(canvas, geometry);
    }
    ...
    protected void handleContentClick(double mouseX, double mouseY, int button) {
        ...
        StorageUiState state = snapshot();
        StorageWindowLayout.Geometry geometry = storageGeometry(state);
        StorageWindowLayout.Hit hit = geometry.hitAt(mouseX, mouseY);
        if (hit == null) {
        ...
        StorageWindowLayout.RowGeometry rowGeometry = geometry.rows.get(hit.rowIndex);
        if (hit.control == StorageWindowLayout.Control.PRIORITY) {
            beginPriorityEdit(entry, rowGeometry.priority);
            return;
        }

        // 离开优先级框时先提交草稿，再执行目标动作。
        commitPriorityEdit();
        if (hit.control == StorageWindowLayout.Control.EXTRACT) {
            StorageUiAdapter.dispatch(this.controller, state,
                    StorageUiAction.key(StorageUiAction.Type.TOGGLE_EXTRACT, core.stableKey));
    ...
    protected boolean handleContentScroll(
        ...
        StorageUiTransition transition = StorageUiAdapter.dispatch(
                this.controller, snapshot(), StorageUiAction.scroll(delta));
    ...
    private StorageUiState snapshot() {
        return StorageUiAdapter.snapshot(this.controller, isOpen(), this.scroll, visibleRows());
    }

    private StorageWindowLayout.Geometry storageGeometry(StorageUiState state) {
        return StorageWindowLayout.geometry(contentX(), contentY(), contentWidth(),
                contentHeight(), state.visibleEntries.size(), state.totalRows, state.scroll);
    }
    ...
    private WindowTextBox createPriorityInput() {
        WindowTextBox input = new WindowTextBox(this.screen.font(), 0, 0,
                StorageWindowLayout.PRIORITY_W, StorageWindowLayout.CONTROL_H);
    ...
    private void placePriorityInput(StorageWindowLayout.RowGeometry geometry) {
    ...
    private void commitPriorityEdit() {
        ...
        int priority = parsePriorityDraft(this.priorityInput.getValue(),
        ...
        if (entry == null) {
            this.controller.updateLinkedStorageSettings(
                    dimensionId, position, extractOnly, priority);
        } else if (entry.priority() != priority) {
            StorageUiAdapter.dispatch(this.controller, snapshot(),
    ...
    private boolean isEditingPriority(String dimensionId, BlockPos position) {
        return position != null && position.equals(this.editingPriorityPos)
