 ...
 *
 * <p>本类负责把一次已归属世界区域的主操作，按既有优先级分发给 GUI 绑定、蓝图捕获、 范围剔除、储存绑定、蓝图放置、选中物品、工具槽或空手交互。它不拥有模式、选择、预览
 * 或网络状态，也不改变任何判定规则；实际副作用仍由 Controller、ShapeController 与 BlueprintPanel 执行。
 *
 * <p>这条优先级链曾埋在 BuilderScreen 生命周期中。独立后，Screen 只负责确定“何时触发”， 本类负责确定“交给谁”，便于后续版本共用同一顺序并对交互优先级做集中回归。
 */
final class BuilderScreenPrimaryActionRouter {
  ...
  BuilderScreenPrimaryActionRouter(
    ...
    this.itemActions = new BuilderScreenItemActionHandler(host, controller, shapeController);
  }
  ...
  boolean run(double mouseX, double mouseY, int mouseButton) {
    ...
    if (this.bottomPanel.bottomPanelTab == BottomPanelLayoutTypes.BottomPanelTab.BLUEPRINTS
      ...
      if (mouseButton == GLFW.GLFW_MOUSE_BUTTON_LEFT && host.isWorldArea(mouseX, mouseY)) {
    ...
    if ((mouseButton == GLFW.GLFW_MOUSE_BUTTON_LEFT || mouseButton < 0)
        ...
        && this.shapeController.clickAdvancedRangeDestroyHandle(
            this.cursorPicker.currentRayOrigin(), this.cursorPicker.computeCursorRayDirection())) {
      return true;
    }
    ...
    if (this.bottomPanel.bottomPanelTab == BottomPanelLayoutTypes.BottomPanelTab.BLUEPRINTS
      ...
      if (blueprintHit != null) {
        BlockPos anchor =
            BlueprintPanel.anchorForCursorTarget(
    ...
    if (this.controller.hasSelectedItem()) {
      return this.itemActions.runSelectedItem(target, forcePlace, rangeDestroyMode, mouseY);
    }
    if (target.blockHit() != null
