...
/** 客户端 RTS 操作边界的统一诊断出口；只观察，不影响输入或网络请求。 */
public final class RtsClientOperationDiagnostics {
  ...
  public static TraceStart begin(
    ...
    info(
        ...
        input,
        "binding",
        input == RtsTraceInputKind.MOUSE ? "MOUSE" : "KEYSYM",
        "physical",
        input == RtsTraceInputKind.MOUSE ? "MOUSE_LEFT" : "ACTION_BREAK",
        "origin",
        origin,
        "screen",
        screenName(),
  ...
  public static int inputRelease(
      long traceId, int heldMs, RtsTraceInputKind input, RtsMiningStopOrigin origin) {
  ...
  public static void serverTerminal(S2CRtsOperationTerminalPayload payload) {
    long now = System.nanoTime();
    RtsClientOperationTraceTracker.Completion completion =
        TRACKER
            .finish(payload.traceId(), payload.outcome(), now)
            .orElse(
                new RtsClientOperationTraceTracker.Completion(
                    payload.traceId(), "UNKNOWN", payload.outcome(), "UNKNOWN", -1L));
    info(
        "SERVER_TERMINAL_RECEIVED",
        payload.traceId(),
        payload.sequence(),
        "outcome",
        payload.outcome(),
        ...
        payload.serverTick(),
        "ever_executed",
        payload.everExecuted(),
        "first_slice_wait_ticks",
        payload.firstSliceWaitTicks(),
        "elapsed_ms",
  ...
  private static void finish(long traceId, String outcome) {
    TRACKER
        .finish(traceId, outcome, System.nanoTime())
        .ifPresent(
            completion ->
                info(
                    "TRACE_END",
  ...
  public record TraceStart(
      long traceId, long startedNanos, long clientTick, RtsTraceInputKind inputKind) {}
}
