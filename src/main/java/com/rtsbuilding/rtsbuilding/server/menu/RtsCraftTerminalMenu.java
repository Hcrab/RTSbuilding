...
/**
 * Forge 1.20.1 一体化合成终端的服务端权威菜单。
 *
 * <p>本类拥有终端专用槽位坐标、远程有效性和合成后 linked storage 补料。
 * 它保留 1.20.1 原版的 {@link RecipeBookMenu}、{@link TransientCraftingContainer}
 * 与 {@link ResultSlot} 合成语义，但不再继承原版 {@code CraftingMenu} 的旧坐标。
 * 结果 0、合成格 1~9、背包 10~36、快捷栏 37~45 的索引契约保持不变。</p>
 */
public final class RtsCraftTerminalMenu extends RecipeBookMenu<CraftingContainer> {
    ...
    public void slotsChanged(Container inventory) {
        this.access.execute((level, ignored) -> updateCraftingResult(
                this, level, this.player, this.craftSlots, this.resultSlots));
    }
    ...
    private static void updateCraftingResult(
        ...
        Optional<CraftingRecipe> match = level.getRecipeManager()
    ...
    /**
     * 点击前记录真实网格；只有原版结果槽确实消耗材料后才记录产出并补料。
     * Shift+左键玩家槽位则交给 linked storage 的窄入口，失败时物品仍留在原槽。
     */
    @Override
