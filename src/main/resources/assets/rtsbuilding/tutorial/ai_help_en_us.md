...
### Fabric 1.21.1 adapter note

Use Fabric client/common sources, callbacks, loader metadata, and Fabric networking; do not infer NeoForge event behavior.
...
## Version-specific technical appendix: Fabric 1.21.1

The release-preparation version for this line is `0.0.2`; the latest known published build for this line is `1.1.6-patch2-fabric-alpha.1`. If the test build is unstable, return to that latest release for the line. Fabric APIs, logs, and class paths must not be inferred from NeoForge labels.

### Task cancellation and diagnosis

When a new single mine, chain mine, or area-destruction replacement action starts, non-terminal `TaskType.MINING` and area-destruction `TaskType.DESTRUCTION` tasks for the same player and dimension must use the real task-cancellation path to reach `CANCELLED`, release leases, preserve cursor/succeeded, write a tombstone, and close the workflow. `WAITING_RESOURCE` may continue waiting when no replacement action is requested; PLACE, QUICK_BUILD, and BLUEPRINT_BUILD are outside the scope.

### Log-analysis quick reference

The prompt includes the last 200 lines of `latest.log` and the latest 200 RTSBuilding records. Find the earliest abnormal event, correlate one `trace` across `seq/op/workflow/task`, and inspect `[RTS-TRACE]`, `[RTS-DIAG]`, `[RTS-SERVER-HEALTH]`, `BEGIN/RESULT/FILTER/TERMINAL`, and the first relevant stack trace. Separate client input/network, server rejection, resource waiting, terminal state, and cascade errors. State logged facts and supported conclusions separately from inferences.

### Fabric 1.21.1 differences

Client entry points live under `src/client/java` and use Fabric client callbacks and Fabric loader metadata; shared/server code follows this branch's Fabric adapters. Confirm the Fabric loader, Fabric API, and installed package when diagnosing a report. NeoForge names or mappings in another line are not evidence here.
