...
public final class RtsMiningDropBufferState {
    ...
    private int enqueueMerged(ItemStack prototype, int requestedCount, int stackLimit,
        ...
        int safeBufferedItems = Math.max(0, Math.min(MAX_BUFFERED_ITEMS, bufferedItems));
        ...
        for (ItemStack existing : stacks) {
            ...
            if (!ItemStack.isSameItemSameTags(existing, prototype)) continue;
        ...
        int safeStackLimit = Math.max(1, Math.min(MAX_STACKS, stackLimit));
