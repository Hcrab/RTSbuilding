...
final class BuilderScreenPointerActionOwner {
  ...
  boolean handleLeftClickInteractions(double mouseX, double mouseY, int button) {
    ...
    if (screen.isWorldArea(mouseX, mouseY) && screen.cancelQuickBuildSmartFillAnchor()) {
    ...
    if (screen.isWorldArea(mouseX, mouseY) && screen.controller.getMode() == BuilderMode.ROTATE) {
      ...
      PlacedBlockRotationGesture gesture =
          screen.rotationHandles.hitGesture(
      ...
      BlockHitResult hit = screen.cursorPicker.pickBlockHit();
      if (hit == null
          || !screen.rotationHandles.select(
              screen.getMinecraft().level, hit.getBlockPos(), cameraForward)) {
        ...
        if (hit != null && screen.getMinecraft().player != null) {
          screen
              ...
              .displayClientMessage(
                  Component.translatable("screen.rtsbuilding.rotation_wheel.unsupported"), true);
        }
      }
  ...
  boolean handleWorldClickActions(double mouseX, double mouseY, int button) {
    ...
    if (panMouse || pickMouse) {
      screen.cameraInput.beginMiddlePress(
          screen.isWorldArea(mouseX, mouseY), button, panMouse, pickMouse);
      return true;
    }
    return false;
  }
  ...
  boolean handleBatchConfirmMouse(double mouseX, double mouseY, int button) {
    ...
    if (screen.shapeController.isAwaitingBatchDestroyConfirm()
        && ClientKeyMappings.CONFIRM_BATCH_DESTROY.matchesMouse(button)) {
    ...
    if (screen.shapeController.isAwaitingBatchPlaceConfirm()
        && ClientKeyMappings.CONFIRM_BATCH_PLACE.matchesMouse(button)) {
