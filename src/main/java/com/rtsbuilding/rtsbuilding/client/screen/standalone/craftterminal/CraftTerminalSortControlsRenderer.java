 ...
 *
 * <p>它拥有两个稳定按钮的短时视觉动画和指针按下状态，但不执行排序、翻页或
 * 网络请求。四种显示语义各自读取一张预处理完成的 24×24 纹理；生产路径不拼接
 * 字符与图标，也不缩放素材。短时动画只在完整状态纹理之间交叉淡化。</p>
 */
public final class CraftTerminalSortControlsRenderer {
    ...
    private final UiControlAnimationRegistry<CraftTerminalUiAction> animations =
            new UiControlAnimationRegistry<CraftTerminalUiAction>(
                    SystemUiClock.INSTANCE, ACTIONS.length);
    ...
    public void render(
        ...
        renderButton(graphics, left, top, layout.field,
                CraftTerminalUiAction.SORT, field, ascending, mouseX, mouseY);
        renderButton(graphics, left, top, layout.direction,
                CraftTerminalUiAction.SORT_DIRECTION, field, ascending, mouseX, mouseY);
    }
    ...
    private void renderButton(
        ...
        UiControlAnimationState.Snapshot animation = this.animations.update(
                action,
        ...
        drawFrame(graphics, action, field, ascending,
                x, y, UiTextureState.INACTIVE, idleWeight);
        drawFrame(graphics, action, field, ascending,
                x, y, UiTextureState.HOVER, hoverWeight);
        drawFrame(graphics, action, field, ascending,
                x, y, UiTextureState.PRESSED, pressedWeight);
    }
    ...
    private static void drawFrame(
        ...
        ResourceLocation texture = action == CraftTerminalUiAction.SORT
    ...
    private static boolean isSortAction(CraftTerminalUiAction action) {
        return action == CraftTerminalUiAction.SORT
