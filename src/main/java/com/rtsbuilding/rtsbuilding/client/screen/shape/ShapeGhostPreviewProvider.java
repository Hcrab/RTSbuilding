 ...
 *
 * <p>本 owner 只组装渲染快照，不修改会话、不发送网络请求。形状生成与世界资格判断通过
...
public final class ShapeGhostPreviewProvider {
    public interface Runtime {
        ...
        /** 正式规划器可返回 TOO_LARGE；旧兼容端口仍可只提供坐标。 */
        default ShapeGenerationResult generationPlan(ShapeBuildTypes.Input input) {
            ...
            return new ShapeGenerationResult(
                    positions == null || positions.isEmpty()
                            ? ShapeGenerationStatus.EMPTY : ShapeGenerationStatus.READY,
    ...
    public void init(BuilderScreen screen, ClientRtsController controller,
            ConfirmedDestroyPreviewState confirmedPreviews, Runtime runtime) {
        this.screen = screen;
    ...
    public ShapeDataRecords.GhostPreview snapshot() {
        ...
        return this.screen.isQuickBuildRangeDestroyMode() ? destroyPreview() : buildPreview();
    }
    ...
    private ShapeDataRecords.GhostPreview destroyPreview() {
        ...
        if (this.screen.isQuickBuildRangeDestroyChainMode()) {
            ...
            if (confirmed != ShapeDataRecords.GhostPreview.EMPTY) return confirmed;
        ...
        if (this.controller.getBuildShape() == BuildShape.BLOCK) {
            ...
            if (hit == null) return ShapeDataRecords.GhostPreview.EMPTY;
        ...
        if (input == null) return ShapeDataRecords.GhostPreview.EMPTY;
    ...
    private ShapeDataRecords.GhostPreview buildPreview() {
        if (this.controller.getBuildShape() == BuildShape.BLOCK) return singleBlockPreview();
        ...
        return blocks.isEmpty()
                ? ShapeDataRecords.GhostPreview.EMPTY
                : new ShapeDataRecords.GhostPreview(blocks, ready());
    }
    ...
    private ShapeDataRecords.GhostPreview singleBlockPreview() {
        ...
        if (!hasSingleBlockSource(mc)) return ShapeDataRecords.GhostPreview.EMPTY;
        BlockHitResult hit = this.screen.pickBlockHit();
        if (hit == null || mc == null || mc.level == null || mc.player == null) {
            return ShapeDataRecords.GhostPreview.EMPTY;
        }
        ItemStack stack = this.controller.hasSelectedItem()
                ? this.controller.getSelectedItemPreview() : mc.player.getMainHandItem();
        if (stack.isEmpty()) return ShapeDataRecords.GhostPreview.EMPTY;
        BlockPos target = ShapePlacementTargetResolver.resolveSingleGhostTarget(mc, hit, stack);
        return target == null ? ShapeDataRecords.GhostPreview.EMPTY
                : new ShapeDataRecords.GhostPreview(List.of(target), true);
    }

    private boolean hasSingleBlockSource(Minecraft mc) {
        if (this.controller.hasSelectedItem() || this.screen.canUseToolSlotShapeSource()) return true;
