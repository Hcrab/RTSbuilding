 ...
 * 客户端远程 GUI 的低噪声诊断入口。
 *
 * <p>这里记录玩家真正需要排查的生命周期边界：收到远程提示、Menu 打开或替换、
 * Screen 缺失/恢复、校验字段放宽、菜单关闭和安全 fallback。它明确不记录每 tick 的
 * 完整快照；重复状态由 {@link RtsRemoteMenuTraceState} 压缩，避免整合包日志被淹没。</p>
 */
public final class RtsRemoteMenuClientDiagnostics {
    ...
    public static void receiveServerHint(BlockPos pos, int graceTicks) {
        ...
        RtsbuildingMod.LOGGER.debug("{} hint session={} target={} graceTicks={}",
                PREFIX, TRACE.activeSessionId(), TRACE.target(), graceTicks);
    }
    ...
    public static void observe(Minecraft minecraft, int pendingTicks, int screenlessTicks) {
        ...
        RtsRemoteMenuTraceState.Transition transition = TRACE.observe(
                containerId, menuClass, screenClass, pendingTicks, System.nanoTime());

        switch (transition.event()) {
            case MENU_OPENED -> RtsbuildingMod.LOGGER.info(
                    "{} opened session={} target={} containerId={} menu={} screen={} pendingTicks={}",
                    PREFIX, transition.sessionId(), transition.target(), containerId,
                    display(menuClass), display(screenClass), pendingTicks);
            case MENU_CHANGED -> RtsbuildingMod.LOGGER.info(
                    "{} changed session={} target={} containerId={} menu={} screen={}",
                    PREFIX, transition.sessionId(), transition.target(), containerId,
                    display(menuClass), display(screenClass));
            case SCREEN_MISSING -> RtsbuildingMod.LOGGER.warn(
                    "{} screen-missing session={} target={} containerId={} menu={} screenlessTicks={}",
                    PREFIX, transition.sessionId(), transition.target(), containerId,
                    display(menuClass), screenlessTicks);
            case SCREEN_RECOVERED -> RtsbuildingMod.LOGGER.info(
                    "{} screen-recovered session={} target={} containerId={} menu={} screen={} ageMs={}",
                    PREFIX, transition.sessionId(), transition.target(), containerId,
                    display(menuClass), display(screenClass), transition.ageMillis());
            case MENU_CLOSED -> RtsbuildingMod.LOGGER.info(
                    "{} closed session={} target={} ageMs={}",
                    PREFIX, transition.sessionId(), transition.target(), transition.ageMillis());
            case HINT_EXPIRED -> RtsbuildingMod.LOGGER.debug(
    ...
    public static void validationApplied(
            ...
            RtsClientRemoteMenuCompat.RelaxationReport report) {
    ...
    public static void compatFailure(Minecraft minecraft, Throwable throwable) {
        String menuClass = minecraft != null && minecraft.player != null && minecraft.player.containerMenu != null
        ...
        String screenClass = minecraft != null && minecraft.screen != null
                ...
                : "none";
        int containerId = minecraft != null && minecraft.player != null && minecraft.player.containerMenu != null
    ...
    public static void reset(String reason) {
        if (TRACE.activeSessionId() != 0L) {
            RtsbuildingMod.LOGGER.debug("{} reset session={} target={} reason={}",
                    PREFIX, TRACE.activeSessionId(), TRACE.target(), reason);
        }
        TRACE.reset();
    }
