    ...
    public static void render(PoseStack poseStack, VertexConsumer lineBuffer, VertexConsumer fillBuffer,
        ...
        for (RtsCullingBox box : manager.boxes()) {
            if (box.id() == manager.hoveredId()) {
                renderBox(poseStack, lineBuffer, fillBuffer, box, YELLOW_R, YELLOW_G, YELLOW_B, 0.22F, 0.95F);
            } else if (box.id() == manager.selectedId()) {
                renderBox(poseStack, lineBuffer, fillBuffer, box, SELECT_R, SELECT_G, SELECT_B, 0.18F, 0.98F);
            } else {
                renderBox(poseStack, lineBuffer, fillBuffer, box, BLUE_R, BLUE_G, BLUE_B, 0.12F, 0.82F);
            }
        }
        ...
        if (preview != null) {
            renderBox(poseStack, lineBuffer, fillBuffer, preview, YELLOW_R, YELLOW_G, YELLOW_B, 0.16F, 0.92F);
        }
    }

    private static void renderBox(PoseStack poseStack, VertexConsumer lineBuffer, VertexConsumer fillBuffer,
            RtsCullingBox box, float r, float g, float b, float fillAlpha, float lineAlpha) {
