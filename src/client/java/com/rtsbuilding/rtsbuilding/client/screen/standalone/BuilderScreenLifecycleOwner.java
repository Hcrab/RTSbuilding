...
final class BuilderScreenLifecycleOwner {
  ...
  void init() {
    ...
    // Enter the RTS scale frame so that clamps in applyStoredUiState use the virtual coordinate
    // space (rather than the GUI-scaled width)
    ...
    WindowTextBox storageSearchBox =
        new WindowTextBox(screen.font(), 8, screen.uiHeight() - 52, 150, 14);
    ...
    screen.craftSearchBox =
        new EditBox(
            screen.font(), 8, screen.uiHeight() - 52, 74, 10, Component.literal("Craft Search"));
    ...
    screen.craftSearchBox.setResponder(
        value -> screen.bottomPanel.craftSearchDraft = value == null ? "" : value);
  ...
  void tick() {
    ...
    // 只同步自由光标命中给仍读取原版 hitResult 的客户端预览，不修改服务端交互或权限判断。
    RtsVanillaCursorHitBridge.publish(screen);
    BlockHitResult storageBatchHit =
        screen.storageBatchSelection.isActive() ? screen.cursorPicker.pickBlockHit() : null;
