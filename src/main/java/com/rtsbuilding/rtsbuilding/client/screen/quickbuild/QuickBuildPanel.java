...
public final class QuickBuildPanel extends RtsWindowPanel {
    ...
    public void loadStoredShapes(BuildShape storedBuildShape, AreaMineShape storedDestroyShape) {
        ...
        // 不覆盖 rangeDestroyShape，由 BuilderScreen 的扁平 UiState 统一恢复。
        if (isOpen()) {
            applyActiveShapeToController();
        }
        rebuildFillModeButtons();
    ...
    public boolean isAdvancedShape(BuildShape shape) {
        return switch (shape == null ? BuildShape.BLOCK : shape) {
    ...
    public void setAdvancedShape(BuildShape shape, boolean value) {
    ...
    public void setRoundShapeVertical(BuildShape shape, boolean value) {
        switch (shape == null ? BuildShape.BLOCK : shape) {
    ...
    /** Forge 旧版扁平 UI 状态使用字符串保存模式。 */
    public String getQuickBuildModeName() {
        return this.quickBuildMode.name();
    }

    /** 从 Forge 旧版扁平 UI 状态恢复模式。 */
    public void setQuickBuildModeName(String modeName) {
        try {
            setMode(QuickBuildMode.valueOf(modeName));
    ...
    public boolean isDestroyChainSelected() {
        return effectiveRangeDestroyShape() == AreaMineShape.CHAIN;
    }

    public void setDestroyChainSelected(boolean selected) {
        if (selected) {
            setRangeDestroyShape(AreaMineShape.CHAIN);
        }
    }
