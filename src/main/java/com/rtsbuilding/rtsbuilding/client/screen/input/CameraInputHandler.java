...
public final class CameraInputHandler {
    ...
    public boolean startMiningAt(double mouseX, double mouseY, int mouseButton, boolean keyboard) {
        ...
        if (screen.isQuickBuildRangeDestroyMode() && !screen.isQuickBuildRangeDestroyChainMode()) {
            return screen.handleQuickBuildRangeDestroyClick(mouseX, mouseY, inputKind);
        }
        ...
        if (screen.isQuickBuildRangeDestroyMode()
            ...
            this.controller.confirmAreaMine(screen.getSelectedToolSlot(), screen.getShapeFillMode(), inputKind);
        } else {
            // 如果指示框当前选中实体，阻止方块破坏
            ...
            if (screen.isQuickBuildRangeDestroyMode() && !screen.isQuickBuildRangeDestroyChainMode()) {
                ...
                if (!UltimineUiAdapter.confirmPreview(screen, hit, preview, inputKind)) {
                    return false;
                }
            } else {
                ...
                this.controller.startMining(
                        hit.getBlockPos(), hit.getDirection().get3DDataValue(),
                        screen.getSelectedToolSlot(), inputKind);
            }
        }
