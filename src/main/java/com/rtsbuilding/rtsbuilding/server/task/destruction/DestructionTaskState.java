 ...
 * 只由此状态拥有。外部构造会校验并深复制可变 NBT；切片迁移复用冻结的 targets、历史前缀
 * 和增量预算，不在每个 slice 重算整个历史。</p>
 */
public final class DestructionTaskState {
    ...
    public List<CompoundTag> historyRecords() { return copyTags(historyRecords); }
    ...
    public int totalUnits() { return targets.size(); }
    ...
    public boolean complete() { return cursorUnits >= targets.size(); }
