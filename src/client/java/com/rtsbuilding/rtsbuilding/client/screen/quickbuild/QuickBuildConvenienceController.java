 ...
 *
 * <p>本类拥有页签/工具/参数偏好、短时世界预览缓存和声明式提交组装；它明确不拥有窗口 绘制、Minecraft 控件、服务端扫描、工具借用或实际破坏。抽离后 QuickBuildPanel
 * 仍只做 窗口生命周期与 Core action 接线，后续增加便捷工具也不会把扫描状态塞回面板大类。
 */
final class QuickBuildConvenienceController {
  ...
  void setPage(QuickBuildUiCatalogPage page, boolean destroyMode) {
    preferences.catalogPage(!destroyMode || page == null ? QuickBuildUiCatalogPage.SHAPES : page);
  ...
  boolean isActive(boolean destroyMode) {
    return destroyMode && preferences.catalogPage() == QuickBuildUiCatalogPage.CONVENIENCE_TOOLS;
  }
  ...
  boolean submit(boolean destroyMode, BlockHitResult hit, int toolSlot) {
    return submit(destroyMode, hit, toolSlot, RtsTraceInputKind.UNKNOWN);
  }

  boolean submit(
      boolean destroyMode, BlockHitResult hit, int toolSlot, RtsTraceInputKind inputKind) {
    if (!isActive(destroyMode) || hit == null || controller == null) {
      return false;
    }
    RtsConvenienceDestroyPlanner.Plan preview =
        previewPlanner.preview(screen.getMinecraft(), commonMode(), hit, commonSettings());
    if (!preview.ready()) {
      return true;
    }
