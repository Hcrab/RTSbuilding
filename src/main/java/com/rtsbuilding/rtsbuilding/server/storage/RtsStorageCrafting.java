 ...
 * RTS 合成操作的外观（Facade）。
 *
 * <p>所有方法委托给 {@code crafting} 包中的相应子模块。 本类仅用于保留 {@link
 * com.rtsbuilding.rtsbuilding.server.RtsStorageManager} 和网络层中现有的调用点，无需更改导入。
 *
 * <p>实际实现位于：
 *
 * <ul>
 *   <li>{@link RtsCraftingSearch}——可合成面板搜索与配方扫描
 *   <li>{@link RtsCraftingExecutor}——合成执行与终端打开
 *   <li>{@link RtsCraftingGridFiller}——合成网格填充与 JEI 转移
 * </ul>
 *
...
public final class RtsStorageCrafting {
  ...
  public static void recordCraftedOutput(
      ServerPlayer player, RtsStorageSession session, ItemStack crafted) {
  ...
  public static void requestCraftables(
      ServerPlayer player,
    ...
    RtsCraftingSearch.requestCraftables(
        player,
        session,
        search,
        showUnavailable,
