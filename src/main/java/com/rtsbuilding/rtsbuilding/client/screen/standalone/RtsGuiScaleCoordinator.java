 ...
 * 渲染递归、输入递归和 scissor 始终使用同一套缩放帧。</p>
 */
final class RtsGuiScaleCoordinator {
    ...
    /** 在非 1:1 缩放时开启虚拟渲染帧，并回调一次换算后的鼠标坐标。 */
    ...
    /** 开启一次输入坐标帧；返回对象必须关闭，递归分发期间不会重复换算。 */
    ...
    private RtsUiScaleFrame enterFrame() {
        ...
        // 信息密度高的完整设置页可声明临时最小虚拟视口；这里只收窄本帧渲染倍率，
        // 不修改玩家保存的 RTS UI Scale，关闭页面后会自然恢复。
        ...
        int virtualWidth = Math.max(1, (int) Math.round(currentWidth / renderScale));
        int virtualHeight = Math.max(1, (int) Math.round(currentHeight / renderScale));
