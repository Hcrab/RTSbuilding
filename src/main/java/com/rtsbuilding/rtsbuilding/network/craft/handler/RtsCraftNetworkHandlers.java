 ...
 * {@link com.rtsbuilding.rtsbuilding.server.service.api.CraftingService}; this layer should only unwrap payloads and
...
public final class RtsCraftNetworkHandlers {
    ...
    public static void handleCraftRecipe(C2SRtsCraftRecipePayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            if (context.player() instanceof ServerPlayer serverPlayer) {
                ServiceRegistry.getInstance().crafting().craftRecipeToLinked(
                        serverPlayer, payload.recipeId(), payload.craftCount());
            }
        });
    }
    ...
    public static void handleClearCraftingGrid(
            C2SRtsClearCraftingGridPayload payload, IPayloadContext context) {
        context.enqueueWork(() -> {
            if (context.player() instanceof ServerPlayer serverPlayer) {
