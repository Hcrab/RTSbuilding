 ...
 * RTS 一体化合成终端的服务端权威菜单。
 *
 * <p>合成语义直接沿用原版工作台的 {@link RecipeBookMenu}、 {@link TransientCraftingContainer} 与 {@link ResultSlot}
 * 组合：配方匹配、成就事件、 配方剩余物和 Shift 合成都不在这里另造一套。该类只拥有终端专用槽位坐标、远程 {@link #stillValid(Player)} 规则，以及合成后从
 * linked storage 按原槽位补料的桥接。
 *
 * <p>槽位索引必须保持原版工作台顺序：结果 0、合成格 1~9、背包 10~36、快捷栏 37~45。网络包、JEI 和批量转移都依赖这个稳定契约。
 */
public final class RtsCraftTerminalMenu extends RecipeBookMenu<CraftingInput, CraftingRecipe> {
  ...
  public RtsCraftTerminalMenu(int containerId, Inventory inventory, ContainerLevelAccess access) {
    ...
    this.addSlot(
        new ResultSlot(
            ...
            this.resultSlots,
            0,
            CraftTerminalLayout.RESULT_X,
            CraftTerminalLayout.RESULT_Y));
  ...
  @Override
  public void clicked(
      int slotId, int button, net.minecraft.world.inventory.ClickType clickType, Player player) {
    if (button == 0
        && clickType == net.minecraft.world.inventory.ClickType.QUICK_MOVE
        && slotId >= INVENTORY_SLOT_START
        && slotId < HOTBAR_SLOT_END
        && player instanceof ServerPlayer serverPlayer) {
      ServiceRegistry.getInstance().transfer().depositCraftTerminalPlayerSlot(serverPlayer, slotId);
      return;
    }
  ...
  public ItemStack quickMoveStack(Player player, int index) {
    ...
    if (slot != null && slot.hasItem()) {
      ...
      if (index == RESULT_SLOT) {
        this.access.execute(
            (level, ignored) -> source.getItem().onCraftedBy(source, level, player));
        ...
        if (!this.moveItemStackTo(source, CRAFT_SLOT_START, CRAFT_SLOT_END, false)) {
          if (index < HOTBAR_SLOT_START) {
            if (!this.moveItemStackTo(source, HOTBAR_SLOT_START, HOTBAR_SLOT_END, false)) {
              ...
              source, INVENTORY_SLOT_START, INVENTORY_SLOT_END, false)) {
            return ItemStack.EMPTY;
          }
        }
