...
class BlueprintGhostMeshCacheTest {
    private final List<BlueprintGhostBlock> geometry = List.of(new BlueprintGhostBlock(BlockPos.ZERO, null, false));
    ...
    void stationaryFramesMaterialChangesAndDraggingReuseFirstBakeWithoutStarvation() {
        ...
        for (int i = 0; i < 120; i++) {
            cache.prepare(world, preview(new BlockPos(i, 2, i), i % 2 == 0), clip, i * 20_000_000L);
    ...
    void changedWorldClipAndExplicitReleaseDisposeOwnedMeshes() {
        ...
        cache.prepare(world, preview(BlockPos.ZERO, true), new BlueprintPreviewClip(0, 8, 0, 9), 2);
        verify(second).close();
    ...
    void fullInteriorMovementHasSameClipButCrossingBoundaryDoesNot() {
        ...
        BlueprintPreviewClip a = BlueprintPreviewClip.of(bounds, BlockPos.ZERO, -100, 99, -100, 99);
        BlueprintPreviewClip b = BlueprintPreviewClip.of(bounds, new BlockPos(10, 70, 20), -100, 99, -100, 99);
        assertEquals(a, b);
        BlueprintPreviewClip edge = BlueprintPreviewClip.of(bounds, new BlockPos(98, 0, 0), -100, 99, -100, 99);
    ...
    private BlueprintGhostPreview preview(BlockPos anchor, boolean ready) {
        return new BlueprintGhostPreview(geometry, ready, false, geometry, anchor, new AABB(0, 0, 0, 10, 10, 10));
    }
}
